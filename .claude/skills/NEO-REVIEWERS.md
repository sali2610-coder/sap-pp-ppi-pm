...
|---|---|---|---|
| **enterprise-adaptive-ui-reviewer** | any UI/layout change; "responsive?", "presentation mode?", "wasted whitespace?", "does it scale on a projector?" | responsive + Phase 9 Adaptive Layout Engine, breakpoints, `.grid-adaptive`, whitespace, card sizing, type scaling, tables/diagrams layout, 6 viewport classes, presentation readability, layout a11y & hierarchy | Owns adaptive layout. Defers WCAG detail → accessibility; aesthetics → visual-designer. **Supersedes** old `neo-adaptive-ui-reviewer`. |
