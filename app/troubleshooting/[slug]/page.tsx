...
function Chips({ items }: { items: string[] }) {
  return <div className="flex flex-wrap gap-1.5">{items.map((t) => <SapTip key={t} name={t}><span className="tech rounded-lg border border-hairline bg-surface-2 px-2.5 py-1 text-xs font-bold text-ink-2" dir="ltr">{t}</span></SapTip>)}</div>;
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const i = incidentBySlug(decodeURIComponent(slug));
  if (!i) return <div className="py-20 text-center text-sm text-ink-3" dir="rtl">התקלה לא נמצאה.</div>;
  ...
  return (
    <div dir="rtl">
      ...
      {i.error && i.error !== "—" && <div className="mb-4 tech rounded-xl border border-red-200 bg-brand-soft px-4 py-2 text-sm font-bold text-brand" dir="ltr">{i.error}</div>}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Block title="Exits / BAdIs קשורים" accent="#7c3aed"><div className="flex flex-wrap gap-1.5">{i.exits.filter((e) => e !== "—").map((e) => <span key={e} className="tech rounded-lg border border-hairline bg-surface-2 px-2.5 py-1 text-xs font-bold text-ink-2" dir="ltr">{e}</span>)}</div></Block>
