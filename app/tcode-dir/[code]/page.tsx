...
function Chips({ items }: { items: { label: string; href: string }[] }) {
  ...
  return <div className="flex flex-wrap gap-1.5">{items.map((r) => <Link key={r.href + r.label} href={r.href} className="tech rounded-lg border border-hairline bg-surface-2 px-2.5 py-1 text-xs font-bold text-ink-2 hover:border-brand hover:text-brand" dir="ltr">{r.label}</Link>)}</div>;
}

export default async function Page({ params }: { params: Promise<{ code: string }> }) {
  const { code } = await params;
  const t = tcodeDirBySlug(decodeURIComponent(code));
  if (!t) return <div className="py-20 text-center text-sm text-ink-3" dir="rtl">טרנזקציה לא נמצאה.</div>;
  ...
  return (
    <div dir="rtl">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Block title="מילות מפתח / חיפוש" accent="#0891b2"><div className="flex flex-wrap gap-1.5">{t.keywords.map((k) => <span key={k} className="tech rounded bg-surface-2 px-2 py-0.5 text-[11px] font-bold text-ink-3" dir="ltr">{k}</span>)}</div></Block>
      ...
      <p className="mt-4 text-xs text-ink-3">רמת מדריך (Directory). לפירוט מלא (Input/Output/QA) ראה את ה-T-Codes המורחבים במרכז הטרנזקציות.</p>
