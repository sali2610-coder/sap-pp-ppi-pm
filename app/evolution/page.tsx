          ...
          <tbody className="divide-y divide-slate-100">
            {entries.map(([code, l]) => (
              <tr key={code} className="hover:bg-slate-50/60">
                ...
                <td className="p-3 align-top text-[11px] leading-snug text-slate-500">{l.migration}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <p className="mt-4 text-xs text-slate-400">דוגמאות: MB1A/MB1B/MB1C → MIGO · XK01/XD01 → BP · MD01 → MD01N · NACE → BRF+ Output. שאר ה-T-Codes (Active) — ראו מרכז הטרנזקציות.</p>
    </div>
  );
}
