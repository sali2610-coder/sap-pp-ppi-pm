  ...
  return (
    <div dir="rtl">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {SAP_NOTES.map((n) => (
          <section key={n.slug} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-2 flex items-center justify-between gap-2">
              <h2 className="text-base font-extrabold tracking-tight text-slate-900">{n.he}</h2>
            ...
            <p className="text-sm leading-relaxed text-slate-700"><b className="text-slate-900">תסמין:</b> {n.symptom}</p>
            <p className="mt-1.5 text-sm leading-relaxed text-slate-600"><b className="text-slate-800">שורש:</b> {n.cause}</p>

            <div className="mt-3">
              <p className="mb-1 text-xs font-bold text-slate-500">מילות חיפוש OSS</p>
              <div className="flex flex-wrap gap-1.5">
                {n.keywords.map((k) => (
                  <span key={k} className="tech rounded-md border border-slate-200 bg-slate-50 px-2 py-0.5 text-[11px] font-semibold text-slate-600" dir="ltr">{k}</span>
                ))}
              </div>
            </div>
            ...
            <ul className="mt-3 space-y-1">
              {n.resolution.map((r, i) => (
                <li key={i} className="flex gap-2 text-sm leading-relaxed text-slate-700"><span className="mt-1.5 size-1.5 shrink-0 rounded-full" style={{ background: ACC }} />{r}</li>
              ))}
            </ul>

            {n.relatedIncidents && n.relatedIncidents.length > 0 && (
              <div className="mt-3 border-t border-slate-100 pt-2.5">
                <p className="mb-1 text-xs font-bold text-slate-500">תקלות מקושרות</p>
                <div className="flex flex-wrap gap-1.5">
                  {n.relatedIncidents.map((slug) => {
                    ...
                    return inc
                      ...
                      : <span key={slug} className="tech rounded-lg border border-dashed border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-400" dir="ltr">{slug}</span>;
                  })}
                </div>
              </div>
            )}
