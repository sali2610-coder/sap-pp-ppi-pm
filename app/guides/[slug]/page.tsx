...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!g) return <div className="py-20 text-center text-sm text-ink-3" dir="rtl">המדריך לא נמצא.</div>;
  ...
  return (
    <div dir="rtl">
      ...
      {/* end-to-end flow */}
      <section className="mb-4 rounded-2xl border border-hairline bg-surface p-5 shadow-sm">
        <h2 className="mb-3 text-sm font-extrabold tracking-tight text-ink-1">זרימה עסקית מקצה-לקצה</h2>
        <div className="flex flex-wrap items-center gap-2">
          {g.flow.map((s, i) => (
            <span key={i} className="flex items-center gap-2">
              <span className="rounded-xl border px-3 py-2 text-xs font-bold text-ink-2" style={{ borderColor: c + "55" }}>{s}</span>
              {i < g.flow.length - 1 && <ArrowLeft className="size-4 shrink-0 rotate-180 text-ink-3" />}
            </span>
          ))}
        </div>
      </section>
      ...
      {/* step-by-step */}
      <section className="mb-4 rounded-2xl border border-hairline bg-surface p-5 shadow-sm">
        <h2 className="mb-3 text-sm font-extrabold tracking-tight text-ink-1">ביצוע שלב-אחר-שלב ב-SAP</h2>
            ...
            <li key={i} className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  {s.tcode && <SapTip name={s.tcode}><span className="tech rounded-md bg-surface-2 px-2 py-0.5 text-[11px] font-bold text-brand" dir="ltr">{s.tcode}</span></SapTip>}
                  {s.tables?.map((t) => <SapTip key={t} name={t}><span className="tech rounded-md bg-surface-2 px-1.5 py-0.5 text-[10px] font-bold text-ink-3" dir="ltr">{t}</span></SapTip>)}
                </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Block title="User Exits / BAdIs" accent="#7c3aed"><div className="flex flex-wrap gap-1.5">{g.exits.map((e) => <span key={e} className="tech rounded-lg border border-hairline bg-surface-2 px-2.5 py-1 text-xs font-bold text-ink-2" dir="ltr">{e}</span>)}</div></Block>
      </div>
