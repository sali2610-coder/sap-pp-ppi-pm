    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-2" dir="rtl">
        {(Object.keys(STATUS_HE) as (keyof typeof STATUS_HE)[]).map((k) => (
          <span key={k} className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-bold text-slate-600">
      ...
      {order.map((area) => {
        ...
        return (
          <section key={area} className="mb-8" dir="rtl">
            <h2 className="mb-3 text-lg font-extrabold tracking-tight text-slate-900">{AREA_HE[area]}</h2>
            <CardGrid>
