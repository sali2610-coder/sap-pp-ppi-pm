  ...
  return (
    <div>
      ...
      <details className="mt-8 rounded-2xl border border-hairline bg-surface p-2" dir="rtl">
        <summary className="cursor-pointer px-3 py-2 text-sm font-bold text-ink-2">חיפוש חכם מורחב (טבלה · שגיאה · תהליך · אובייקט)</summary>
        <div className="px-1 pt-2"><TxSearch index={index} /></div>
      </details>
      <div className="mt-8 mb-3 flex flex-wrap gap-1.5 text-[11px] font-bold text-ink-3" dir="rtl">
        {["MM", "SD", "FI", "CO", "PP", "PP-PI", "PM", "QM", "BASIS", "ABAP", "SECURITY", "INTEGRATION", "FIORI"].map((dm) => <span key={dm} className="rounded-full bg-surface-2 px-2 py-0.5">{dm} {byDomain(dm)}</span>)}
      </div>
      <h2 className="mb-3 mt-6 text-lg font-extrabold tracking-tight text-ink-1" dir="rtl">T-Codes מתועדים לעומק (טבלת 14 עמודות)</h2>
