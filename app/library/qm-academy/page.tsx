...
import { QM_TEXTBOOK, QM_TEXTBOOK_STATS } from "@/data/library/qm-textbook";
import { useI18n } from "@/lib/i18n";
const pad = (n: number) => String(n).padStart(2, "0");
export default function QMAcademyIndex() {
  const { lang } = useI18n();
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-4xl space-y-5 p-4">
      ...
      <section dir="rtl" className="glass rounded-2xl p-6 text-center">
        <span className="inline-flex items-center gap-2 rounded-full border border-brand/20 bg-brand-soft px-3 py-1 text-xs font-semibold text-brand"><GraduationCap className="size-3.5" />{lang === "he" ? "אקדמיית QM — ניהול איכות" : "QM Academy — Quality Management"}</span>
        <h1 className="mt-2 text-2xl font-bold tracking-tight">{lang === "he" ? "ניהול איכות ב-SAP S/4HANA" : "Quality Management with SAP S/4HANA"}</h1>
      ...
      <div className="grid gap-3">
        {keys.map((n) => {
          ...
          return (
            <Link key={n} href={`/library/qm-academy/chapter-${pad(n)}/`} dir="rtl" className="glass group flex items-center justify-between gap-3 rounded-2xl p-4 transition-shadow hover:shadow-lg">
              <span className="flex items-center gap-3">
                <span dir="ltr" className="tech grid size-9 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-700 text-sm font-bold text-white">{n}</span>
                <span><span className="block text-sm font-bold group-hover:text-brand">{ch.titleHe}</span><span dir="ltr" className="block text-[11px] text-muted-foreground">{ch.titleEn} · {st.totalNodes} {lang === "he" ? "יחידות" : "units"} · ~{st.readMin} {lang === "he" ? "דק'" : "min"}</span></span>
              </span>
