...
import { PMU_TEXTBOOK, PMU_TEXTBOOK_STATS } from "@/data/library/pmu-textbook";
import { useI18n } from "@/lib/i18n";
const pad = (n: number) => String(n).padStart(2, "0");
export default function PMUAcademyIndex() {
  const { lang } = useI18n();
  const keys = Object.keys(PMU_TEXTBOOK).map(Number).sort((a, b) => a - b);
  const totNodes = Object.values(PMU_TEXTBOOK_STATS).reduce((s, c) => s + c.totalNodes, 0);
  const totMin = Object.values(PMU_TEXTBOOK_STATS).reduce((s, c) => s + c.readMin, 0);
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      ...
      <section dir="rtl" className="glass rounded-2xl p-6 text-center">
        <span className="inline-flex items-center gap-2 rounded-full border border-brand/20 bg-brand-soft px-3 py-1 text-xs font-semibold text-brand"><GraduationCap className="size-3.5" />{lang === "he" ? "אקדמיית MM — תחזוקת מפעל — מדריך משתמש" : "PM-User Academy — PM Business User Guide"}</span>
        <h1 className="mt-2 text-2xl font-bold tracking-tight">{lang === "he" ? "תחזוקת מפעל — מדריך משתמש ב-SAP S/4HANA" : "Plant Maintenance Business User Guide"}</h1>
