...
import { LayoutGrid, Search, ExternalLink, X, Terminal, Sigma, ShieldCheck, Target, Briefcase, CheckCircle2, GraduationCap, ArrowLeft } from "lucide-react";
  ...
  return (
    <>
      ...
      <motion.div dir="rtl" role="dialog" aria-label={`פרטי ${app.id}`}
        ...
        className="fixed inset-x-0 bottom-0 z-[61] flex max-h-[88%] flex-col overflow-hidden rounded-t-[1.75rem] border-t border-hairline bg-surface shadow-2xl lg:inset-y-auto lg:top-[4.25rem] lg:bottom-0 lg:left-auto lg:right-0 lg:max-h-none lg:w-[460px] lg:max-w-[42vw] lg:rounded-none lg:rounded-s-[1.75rem] lg:border-s lg:border-t-0">
        <div className="mx-auto mt-2 h-1.5 w-11 shrink-0 rounded-full bg-hairline lg:hidden" />
        <div className="shrink-0 px-5 py-3.5 text-white" style={{ background: "linear-gradient(135deg,#7c3aed,#5b21b6)" }}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><LayoutGrid className="size-5" /><span className="tech font-mono text-2xl font-extrabold" dir="ltr">{app.id}</span><span className="rounded bg-surface/20 px-1.5 py-0.5 text-[10px] font-bold">{app.type}</span></div>
              <p className="mt-1 text-sm font-semibold text-white/90">{app.name}</p>
            </div>
            <button onClick={onClose} aria-label="סגור" className="tap grid size-9 shrink-0 place-items-center rounded-xl bg-surface/15 ring-1 ring-white/25 active:scale-90"><X className="size-5" /></button>
          </div>
        </div>
        <div className="min-h-0 flex-1 overflow-auto">
          ...
          <Row icon={<Terminal className="size-3.5" />} c="#475569" label="מקבילה / רקע ב-ECC (GUI)">
            {cf?.gui?.length ? <span className="flex flex-wrap gap-1.5">{cf.gui.map((g) => appObject(g) ? <Link key={g} href={`/apps/${encodeURIComponent(g)}/`} className="tech rounded-md bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2 hover:bg-brand/10 hover:text-brand" dir="ltr">{g}</Link> : <span key={g} className="tech rounded-md bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2" dir="ltr">{g}</span>)}</span> : NVspan}
          </Row>
          <Row icon={<ShieldCheck className="size-3.5" />} c="#dc2626" label="Business Role · Catalog">{cf ? <span className="space-y-0.5"><span className="block">Role: <span className="tech font-mono text-ink-2" dir="ltr">{cf.role || NV}</span></span><span className="block">Catalog: <span className="tech font-mono text-ink-2" dir="ltr">{cf.catalog || NV}</span></span></span> : NVspan}</Row>
          <Row icon={<Sigma className="size-3.5" />} c="#0891b2" label="OData · CDS · טבלאות">{cf || t ? <span className="space-y-0.5"><span className="block">OData: <span className="tech font-mono text-ink-2" dir="ltr">{cf?.odata || NV}</span></span><span className="block">CDS: <span className="tech font-mono text-ink-2" dir="ltr">{cf?.cds || (t?.cds?.join(", ")) || NV}</span></span>{t?.tables?.length ? <span className="block">טבלאות: <span className="tech font-mono text-ink-2" dir="ltr">{t.tables.slice(0, 6).join(", ")}</span></span> : null}</span> : NVspan}</Row>
          ...
          {refs.length > 0 && (
            <Row icon={<ExternalLink className="size-3.5" />} c="#0369a1" label="קורסי אקדמיה קשורים">
              <div className="flex flex-wrap gap-1.5">{refs.map((r, i) => <Link key={i} href={`${r.base}/chapter-${pad(r.ch)}/#sub-${r.id}`} className="inline-flex items-center gap-0.5 rounded-md bg-surface-2 px-2 py-0.5 text-[11px] font-bold text-ink-2 hover:text-brand">{r.base.split("/").pop()?.replace("-academy", "").toUpperCase()} {r.id}</Link>)}</div>
            </Row>
          )}
          {!cf && <div className="px-5 py-3 text-[11px] leading-relaxed text-ink-3">אפליקציה זו קיימת באינדקס אך טרם תועדה לעומק. הפרטים הטכניים המדויקים — <b>{NV}</b>. למידע מאומת ראה את קורסי האקדמיה או חפש את הטרנזקציה המקבילה במרכז האפליקציות.</div>}
        </div>
        {gui && appObject(gui) && (
