...
import { SOP_TEXTBOOK, SOP_TEXTBOOK_STATS } from "@/data/library/sop-textbook";
import { useI18n } from "@/lib/i18n";
const pad = (n: number) => String(n).padStart(2, "0");
export default function SOPAcademyIndex() {
  const { lang } = useI18n();
  const keys = Object.keys(SOP_TEXTBOOK).map(Number).sort((a, b) => a - b);
  const totNodes = Object.values(SOP_TEXTBOOK_STATS).reduce((s, c) => s + c.totalNodes, 0);
  const totMin = Object.values(SOP_TEXTBOOK_STATS).reduce((s, c) => s + c.readMin, 0);
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      ...
      <section dir="rtl" className="glass rounded-2xl p-6 text-center">
        <span className="inline-flex items-center gap-2 rounded-full border border-brand/20 bg-brand-soft px-3 py-1 text-xs font-semibold text-brand"><GraduationCap className="size-3.5" />{lang === "he" ? "אקדמיית MM — תכנון מכירות ותפעול" : "S&OP Academy — S&OP & IBP"}</span>
        <h1 className="mt-2 text-2xl font-bold tracking-tight">{lang === "he" ? "תכנון מכירות ותפעול ב-SAP S/4HANA" : "Sales & Operations Planning with SAP IBP"}</h1>
      ...
      <div className="grid-adaptive">
        {keys.map((n) => { const ch = SOP_TEXTBOOK[String(n)]; const st = SOP_TEXTBOOK_STATS[String(n)];
          return (<Link key={n} href={`/library/sop-academy/chapter-${pad(n)}/`} dir="rtl" className="glass group flex items-center justify-between gap-3 rounded-2xl p-4 transition-shadow hover:shadow-lg">
            <span className="flex items-center gap-3"><span dir="ltr" className="tech grid size-9 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-teal-500 to-teal-700 text-sm font-bold text-white">{n}</span>
