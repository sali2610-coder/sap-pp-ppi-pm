    ...
    <div dir="rtl">
      ...
      <div className="overflow-hidden rounded-2xl border border-hairline bg-surface shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-surface-2 text-xs font-bold text-ink-3"><tr><th className="p-3 text-start">מקור SAP</th><th className="p-3 text-start">אובייקט</th><th className="p-3 text-start">יעד ב-NEO</th><th className="p-3 text-center">סטטוס</th></tr></thead>
          <tbody className="divide-y divide-slate-100">
            {CONNECTOR_MAP.map((r) => (<tr key={r.source}><td className="tech p-3 font-bold text-ink-2" dir="ltr">{r.source}</td><td className="p-3 text-ink-2">{r.sapObject}</td><td className="tech p-3 text-[12px] text-ink-3" dir="ltr">{r.neoTarget}</td><td className="p-3 text-center"><span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700">planned</span></td></tr>))}
          </tbody>
        </table>
      </div>
      <p className="mt-4 text-[13px] leading-relaxed text-ink-3">המחבר מוגדר כ-<span className="tech" dir="ltr">SapConnector</span> interface ב-<span className="tech" dir="ltr">lib/sap-connector.ts</span> עם stub offline. מימוש עתידי דרך RFC / sc4sap MCP (GetTable/SearchObject/GetWhereUsed) / OData — read-only בלבד. הפעלה ע\"י <span className="tech" dir="ltr">USE_LIVE=true</span> + הזרקת adapter. ללא חיבור, NEO נשאר 100% offline.</p>
    </div>
  );
}
