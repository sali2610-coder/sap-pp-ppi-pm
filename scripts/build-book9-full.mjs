// Merge extracted English sections + authored Hebrew into data/library/book9-full.json.
...
const DIR = path.join(ROOT, "data", "library", "book9");
...
fs.writeFileSync(path.join(ROOT, "data", "library", "book9-full.json"), JSON.stringify(out, null, 2));

const index = out.chapters.flatMap((c) =>
  c.sections.map((s) => ({ ch: c.n, id: s.id, title: s.title, snippet: s.en.slice(0, 260), he: Boolean(s.he) })),
);
fs.writeFileSync(path.join(ROOT, "data", "library", "book9-index.json"), JSON.stringify(index, null, 2));
...
console.log(`✓ book9-full.json — ${out.chapters.length} chapters, ${total} sections (${heCount} with Hebrew), ${translated} chapters translated`);
