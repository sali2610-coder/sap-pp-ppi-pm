// Extract real figures (SAP screenshots/diagrams) from Book #3 via poppler's
// pdfimages. Filters out icons/rules by min size, names by page, copies to
// public/assets/library/book7/figures/, and writes a per-chapter manifest.
// Mirrors scripts/extract-book1-figures.mjs.
//
// Run: node scripts/extract-book7-figures.mjs
...
const PDF = path.join(ROOT, "docs", "SAP Fiori Apps for SAP S4HANA The Quick Reference Guide  __ Copy y2h3-b9ug-zf7r-54nk.pdf");
const OUT = path.join(ROOT, "public", "assets", "library", "book7", "figures");
const MANIFEST = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "library", "book7", "manifest.json"), "utf8"));
...
for (const ch of MANIFEST.chapters) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `b5fig-${ch.n}-`));
  ...
  for (const r of keep) {
    ...
    figs.push({ page: r.page, file: `/assets/library/book7/figures/${dest}`, w: r.w, h: r.h });
    total++;
  }
...
fs.writeFileSync(
  path.join(ROOT, "data", "library", "book7-figures.json"),
  JSON.stringify(figuresByChapter, null, 2),
);
const sz = execFileSync("du", ["-sh", OUT], { encoding: "utf8" }).split("\t")[0];
console.log(`\n✓ ${total} figures → public/assets/library/book7/figures/ (${sz})`);
