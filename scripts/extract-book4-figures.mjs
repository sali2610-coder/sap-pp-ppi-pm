...
// public/assets/library/book4/figures/, and writes a per-chapter manifest.
// Mirrors scripts/extract-book1-figures.mjs.
//
// Run: node scripts/extract-book4-figures.mjs
...
const PDF = path.join(ROOT, "docs", "PP-DS with SAP S4HANA __ Copy kysj-d27q-n3bt-whvm.pdf");
const OUT = path.join(ROOT, "public", "assets", "library", "book4", "figures");
const MANIFEST = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "library", "book4", "manifest.json"), "utf8"));
...
for (const ch of MANIFEST.chapters) {
  ...
  for (const r of keep) {
    ...
    figs.push({ page: r.page, file: `/assets/library/book4/figures/${dest}`, w: r.w, h: r.h });
    total++;
  }
...
fs.writeFileSync(
  path.join(ROOT, "data", "library", "book4-figures.json"),
  JSON.stringify(figuresByChapter, null, 2),
);
const sz = execFileSync("du", ["-sh", OUT], { encoding: "utf8" }).split("\t")[0];
console.log(`\n✓ ${total} figures → public/assets/library/book4/figures/ (${sz})`);
