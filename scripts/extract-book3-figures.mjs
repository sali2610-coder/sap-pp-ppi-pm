...
// public/assets/library/book3/figures/, and writes a per-chapter manifest.
// Mirrors scripts/extract-book1-figures.mjs.
//
// Run: node scripts/extract-book3-figures.mjs
...
const PDF = path.join(ROOT, "docs", "Sourcing and Procurement with SAP S4HANA __ Copy gz38-kvty-7wej-2i9b.pdf");
const OUT = path.join(ROOT, "public", "assets", "library", "book3", "figures");
const MANIFEST = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "library", "book3", "manifest.json"), "utf8"));
...
for (const ch of MANIFEST.chapters) {
  ...
  for (const r of keep) {
    ...
    figs.push({ page: r.page, file: `/assets/library/book3/figures/${dest}`, w: r.w, h: r.h });
    total++;
  }
...
fs.writeFileSync(
  path.join(ROOT, "data", "library", "book3-figures.json"),
  JSON.stringify(figuresByChapter, null, 2),
);
const sz = execFileSync("du", ["-sh", OUT], { encoding: "utf8" }).split("\t")[0];
console.log(`\n✓ ${total} figures → public/assets/library/book3/figures/ (${sz})`);
