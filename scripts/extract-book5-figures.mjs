// Extract real figures (SAP screenshots/diagrams) from Book #5 via poppler's
// pdfimages. Filters out icons/rules by min size, names by page, copies to
// public/assets/library/book5/figures/, and writes a per-chapter manifest.
// Mirrors scripts/extract-book1-figures.mjs.
//
// Run: node scripts/extract-book5-figures.mjs
...
const PDF = path.join(ROOT, "docs", "Quality Management with SAP S4HANA __ Copy bw76-fnz5-dphy-kgxt.pdf");
const OUT = path.join(ROOT, "public", "assets", "library", "book5", "figures");
const MANIFEST = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "library", "book5", "manifest.json"), "utf8"));
...
for (const ch of MANIFEST.chapters) {
  ...
  for (const r of keep) {
    ...
    figs.push({ page: r.page, file: `/assets/library/book5/figures/${dest}`, w: r.w, h: r.h });
    total++;
  }
...
fs.writeFileSync(
  path.join(ROOT, "data", "library", "book5-figures.json"),
  JSON.stringify(figuresByChapter, null, 2),
);
const sz = execFileSync("du", ["-sh", OUT], { encoding: "utf8" }).split("\t")[0];
console.log(`\n✓ ${total} figures → public/assets/library/book5/figures/ (${sz})`);
