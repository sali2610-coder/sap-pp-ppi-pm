// Merge extracted English sections + authored Hebrew into data/library/book5-full.json.
...
const DIR = path.join(ROOT, "data", "library", "book5");
...
fs.writeFileSync(path.join(ROOT, "data", "library", "book5-full.json"), JSON.stringify(out, null, 2));
...
fs.writeFileSync(path.join(ROOT, "data", "library", "book5-index.json"), JSON.stringify(index));
...
console.log(`✓ book5-full.json — ${out.chapters.length} chapters, ${secs} sections (${translated} with Hebrew)`);
