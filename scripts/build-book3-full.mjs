// Merge extracted English sections + authored Hebrew into data/library/book3-full.json.
...
const DIR = path.join(ROOT, "data", "library", "book3");
...
fs.writeFileSync(path.join(ROOT, "data", "library", "book3-full.json"), JSON.stringify(out, null, 2));
...
fs.writeFileSync(path.join(ROOT, "data", "library", "book3-index.json"), JSON.stringify(index));
...
console.log(`✓ book3-full.json — ${out.chapters.length} chapters, ${secs} sections (${translated} with Hebrew)`);
