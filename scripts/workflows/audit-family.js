...
OFFICIAL LOOKUP TOOL: \`node scripts/sap-help-search.mjs "<query>" --size 12 --json\` (add --product SAP_ERP for the ECC scope; JSON: title, deliverable, product, release, versionId, loio, url, date, snippet). help.sap.com topic BODIES, fal.cloud.sap and api.sap.com pages are JS shells: a claim must be supported by a search record's title/snippet, or by a document that was actually read (the portal content service http.svc/pagecontent, a PDF via curl -o + Read). Never claim body text nobody saw.
