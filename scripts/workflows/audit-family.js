...
READ FIRST: audit/s4-enrichment/MANIFEST.md (evidence channels + limits), lib/evidence/types.ts (Evidence / VerificationRecord / S4StatusClaim / S4Status tokens), lib/evidence/validate.ts (the rules incl. URL_ALLOWLIST, CERTAINTY_RE, PLACEHOLDER_RE).
OFFICIAL LOOKUP TOOL: \`node scripts/sap-help-search.mjs "<query>" --size 12 --json\` (add --product SAP_ERP for the ECC scope; JSON: title, deliverable, product, release, versionId, loio, url, date, snippet). FIORI APPS LIBRARY (official, structured, no auth): `node scripts/fal-app.mjs <APPID>` (latest on-premise release S32OP = S/4HANA 2025 FPS01; `--release S27OP` = 2023; `--json`; `--out <dir>` saves the raw JSON) prints role(s) with SAP_BR id, business and technical catalogs, semantic object/action, OData services or V4 service groups, backend/UI software components, supported releases, predecessor/successor apps, RIN note numbers and the app documentation link, all copied from the library's OData service; `node scripts/fal-app.mjs --tcode IW31` lists the apps whose leading transaction code is IW31 and whether the GUI transaction itself is listed as an app. Cite the printed deep link (fioriappslibrary.hana.ondemand.com/...#/detail/Apps('ID')/RELEASE) as the URL, sourceType fiori_apps_library or sap_help as the Evidence type allows, and copy values verbatim. An empty result means 'not in that release of the library', never 'does not exist'.
help.sap.com topic BODIES, fal.cloud.sap and api.sap.com pages are JS shells: a claim must be supported by a search record's title/snippet, or by a document that was actually read (the portal content service http.svc/pagecontent, a PDF via curl -o + Read). Never claim body text nobody saw.
HARD RULES: never invent a table field, parameter, Fiori ID, SAP Note number, release status or successor. S/4 status tokens ONLY from the S4Status union. sap_official_verified requires a help.sap.com / api.sap.com / fioriappslibrary / fal URL. A name that no official record prints stays verification_required with the searches listed; never assert non-existence. Hebrew professional copy, no em dashes. xrefs must exist in the universe (lib/route-manifest.generated.ts families, data/fiori/apps.ts ids, data/exits.ts names, data/enhancements.ts slugs, OBJECT_REGISTRY, IDOC_BASIC_TYPES, bp slugs).`
