...
$('#livecount').textContent=`${DATA.tables.length} TABLES · ${DATA.edges.length} LINKS · ${DATA.shared.length} HUBS`;

// ---- zoom ----
const zoom=d3.zoom().scaleExtent([0.15,7]).on('zoom',e=>{root.attr('transform',e.transform);updateMini(e.transform);});
stage.call(zoom);
function fit(pad=0.92){try{const b=root.node().getBBox();if(!b.width)return;const w=W(),h=H();
  const s=Math.min(w/b.width,h/b.height)*pad;const tx=w/2-s*(b.x+b.width/2),ty=h/2-s*(b.y+b.height/2);
...
$$('#tabs button').forEach(b=>b.onclick=()=>{$$('#tabs button').forEach(x=>x.classList.remove('on'));b.classList.add('on');st.view=b.dataset.v;render();});
...
function render(){hideTip();stage.interrupt();root.interrupt();root.selectAll('*').remove();stopSim();
  ...
  $('#rail').style.display = (st.view==='s4')?'none':'block';
  ({landscape:renderLandscape,business:renderBusiness,functional:renderFunctional,
