...
export const CH14: TextbookChapter = {
  ...
  introHe:
    "פרק זה הוא יחידת-לימוד מלאה לאינטגרציה של SAP IBP (Integrated Business Planning) — פתרון-הענן של SAP לתכנון עסקי משולב — אל מערכות-הליבה כמו SAP S/4HANA, SAP Analytics Cloud ו-SAP Cloud for Customer, בהקשר של תהליך S&OP. תכנון S&OP טוב ככל הנתונים שמזינים אותו: ללא זרימת-נתונים אמינה מ-S/4HANA אל IBP (מלאי, פק\"עות, היסטוריית-מכירות) וחזרה (תחזית, תכנית-אספקה), התכנית נשארת תיאורטית. כל תת-פרק וכל תת-סעיף הורחב ליחידת-לימוד עצמאית בת 18 מקטעים — שלוש רמות הסבר (מנהלים, מתחילים, יועצים), מטרה עסקית, דוגמת-תהליך מקצה-לקצה, דוגמת הארגון (Example Product bottling), ניווט/קונפיגורציה, אובייקטים, תרשים זרימת-נתונים, טעויות נפוצות, פתרון-תקלות, שיטות מומלצות, שאלות-ראיון ומסקנות-מפתח. המטרה: ללמוד את הנושא בלי הספר המקורי.",
  subchapters: [
    // ============================================================ 14.1
    {
      id: "14.1",
    ...
    // ============================================================ 14.2
    {
      ...
      scenarioHe:
        "בארגון: S&OP חודשי לכל קטגוריות-המשקה. CI-DS טוען מכירות-עבר לפי SKU/רשת/שבוע ומלאי-מוגמר. מנהל-הביקוש מזין ידנית דרך ה-Web UI תכניות-קידום (מבצעי-קיץ, חגים) שמקורן בשיווק ולא ב-S/4HANA. התחזית המאושרת מיוצאת ב-OData ל-SAC לדוחות-הנהלה.",
      navHe: [
      ...
      children: [
        {
          ...
          scenarioHe:
            "בארגון: CI-DS Agent רץ ב-data center של המבקבק; Task יומי מושך מכירות-משקאות ומלאי-מחסנים מ-S/4HANA, ממיר ליחידות-מארז ולשבועות, וטוען ל-Planning Area של IBP. delta load מעביר רק חשבוניות-חדשות כדי לקצר זמן-ריצה בעונת-שיא.",
          navHe: [
        ...
        },
        {
          ...
          scenarioHe:
            "בארגון: מנהל-השיווק מזין דרך ה-Web UI את תכנית-המבצעים לקיץ (הנחות-מחיר, 1+1) ל-Key Figure ידני; ב-Excel Add-in צוות-המכירות מתאים ידנית יעדים לפי רשת. נתונים אלה אין להם מקור ב-S/4HANA ולכן מוזנים ידנית, ומופרדים מ-Key Figures שמקורם CI-DS.",
          navHe: [
        ...
        },
        {
          ...
          scenarioHe:
            "בארגון: SAP Analytics Cloud (SAC) ומחסן-הנתונים של המבקבק קוראים את התחזית ותכנית-האספקה מ-IBP דרך OData. מנהלי-הקטגוריות רואים בדשבורד SAC את הביקוש-המוסכם לכל משקה, נמשך ישירות מ-IBP בלי ייצוא-קבצים ידני.",
          navHe: [
    ...
    // ============================================================ 14.3
    {
      ...
      scenarioHe:
        "בארגון בעונת-קיץ: רשת-קמעונאות מגדילה הזמנה לפתע. דרך RTI/SDI ההזמנה מגיעה ל-IBP for Response & Supply תוך דקות; המערכת מזהה מחסור-מלאי במרכז-הפצה אחד ומקצה מ-מרכז-סמוך, או מתעדפת פק\"עת-מילוי. ללא RTI, התגובה הייתה מתעכבת עד הטעינה-הלילית — מאוחר מדי לעונת-שיא.",
      navHe: [
    ...
    // ============================================================ 14.4
    {
      ...
      scenarioHe:
        "בארגון: התכנית-התפעולית למשקאות מוזנת ל-SAC לבדיקת-רווחיות מול תקציב-השנה (עלות-תרכיז, מחיר-מכירה); דשבורד-הנהלה מציג ביקוש מול הכנסה. במקביל, עסקאות-מכר חדשות עם רשתות חדשות ב-C4C מוזרמות ל-IBP ומגדילות את תחזית-הביקוש לאזורים הרלוונטיים.",
      navHe: [
      ...
      children: [
        {
          ...
          scenarioHe:
            "בארגון: תחזית-המשקאות מ-IBP נטענת ל-SAC; SAC מתרגם להכנסה (מחיר-מארז) ולעלות (תרכיז, סוכר, אריזה) ומציג רווחיות לפי קטגוריה מול תקציב-השנה. הנהלת-המבקבק רואה בדשבורד SAC שמבצע-קיץ אגרסיבי מגדיל נפח אך שוחק שוליים — קלט להחלטת-S&OP.",
          navHe: [
        ...
        },
        {
          ...
          scenarioHe:
            "בארגון: צוות-המכירות סוגר עסקה לאספקת משקאות לרשת-מלונות חדשה — מתועד כ-opportunity ב-C4C. האינטגרציה מזרימה אותו ל-IBP, משוקלל בהסתברות, ומגדיל את תחזית-הביקוש לאזור ולקטגוריה — קווי-המילוי מתוכננים מראש לנפח הנוסף.",
          navHe: [
    ...
    // ============================================================ 14.5
    {
      ...
      scenarioHe:
        "בארגון, התמונה המלאה: CI-DS מזין מכירות-משקאות ומלאי מ-S/4HANA; שיווק מזין מבצעי-קיץ ידנית; C4C מזרים עסקאות-רשתות חדשות; IBP מתכנן ביקוש ואספקה עונתיים; SAC מאמת רווחיות מול תקציב; OData מזין דשבורדי-הנהלה; RTI מאפשר תגובה מהירה בעונת-שיא. כך S&OP של המבקבק מבוסס על מציאות מלאה.",
      navHe: [
