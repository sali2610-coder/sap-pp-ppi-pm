...
export const CH7: TextbookChapter = {
  ...
  introHe:
    "פרק זה הוא יחידת-לימוד מלאה לשיתוף-הפעולה ולניהול-לפי-חריגים (Management by Exception) ב-SAP IBP. בתכנון S&OP אמיתי אף מתכנן אינו יכול לבחון כל פריט בכל מחסן; במקום זאת המערכת מתריעה רק על מה שחורג מהמצופה, מנתבת את החריג לאדם הנכון, ומאפשרת לצוותים מבוזרים לעבוד יחד סביב אותה תוכנית-יחידה. כל תת-פרק וכל תת-סעיף הורחב ליחידת-לימוד עצמאית בת 18 מקטעים — שלוש רמות הסבר (מנהלים, מתחילים, יועצים), מטרה עסקית, דוגמת-תהליך מקצה-לקצה, דוגמת הארגון (Example Product bottling), ניווט באפליקציות IBP וב-SAP Build Work Zone, אפליקציות Fiori/IBP ו-Excel add-in, פרטי קונפיגורציה, תרשים-תהליך, טעויות נפוצות, פתרון תקלות, שיטות מומלצות, שאלות-ראיון ומסקנות-מפתח. המטרה: לשלוט בנושא ללא הספר המקורי.",
  subchapters: [
    // ============================================================ 7.1
    {
      id: "7.1",
      ...
      scenarioHe:
        "בארגון: מנהל-S&OP אזורי, מנהלי קווי-המילוי ומחלקת-המכירות חולקים workspace אחד ב-SAP Build Work Zone. כשמבצע-קיץ מקפיץ ביקוש ל-1.5 ליטר, ההתראה והדיון מתנהלים במרחב המשותף, וכל המעורבים רואים את אותה תוכנית-אספקה מעודכנת — מבלי לערבב גרסאות-גיליון מקומיות.",
      navHe: [
      ...
      children: [
        {
          ...
          scenarioHe:
            "בארגון כל אזור-בקבוק מקבל Space משלו ('North', 'South'), ובתוכו Pages לפי תפקיד; מנהל-קו-מילוי רואה רק את אריחי-הקיבולת והחריגים של הקו שלו, ומנהל-המכירות רואה את תצוגות-הביקוש.",
          navHe: [
        ...
        },
        {
          ...
          scenarioHe:
            "בארגון אינטגרציית IBP מאפשרת למנהל-S&OP לפתוח מ-Work Zone את תצוגת-איזון-ההיצע-ביקוש של כל קווי-המילוי בענן, להריץ תרחיש-מבצע, ולשתף את התוצאה עם המכירות — הכל בזרימה אחת.",
          navHe: [
        ...
        },
        {
          ...
          scenarioHe:
            "בארגון מתכנני-המכירות האזוריים עובדים בתבניות-Excel אחידות שהופצו דרך Work Zone; הם מזינים תחזית-מבצעים לכל SKU, וה-write-back מאחד את כולם לתוכנית-ביקוש אחת ב-IBP cloud — בלי לשלוח גיליונות במייל.",
          navHe: [
    ...
    // ============================================================ 7.2
    {
      ...
      scenarioHe:
        "בארגון מוגדרות Alerts ל'מלאי-מוגמר מתחת לבטיחות לפני סוף-שבוע', 'קיבולת-קו-מילוי מעל 95%' ו'סטיית-תחזית-מבצע > 20%'. בעונת-השיא מנהל-S&OP מנהל את כל ה-portfolio דרך מסך-החריגים בלבד — כל פריט שאינו חריג פשוט אינו דורש מבט.",
      navHe: [
      ...
      children: [
        {
          ...
          scenarioHe:
            "בארגון מנהלי-הקווים משתמשים ב-'My Alerts' לראות חריגי-קיבולת, ומנהל-S&OP ב-Dashboard עם Alert Cards אזוריות — אותה שפת-Fiori לכל התפקידים.",
          navHe: [
        ...
        },
        {
          ...
          scenarioHe:
            "בארגון בעונת-השיא מנהל-S&OP פותח את מסך-החריגים פעמיים ביום: כל 'קיבולת > 95%' שמופיעה מנותבת או למשימה (להזיז ייצור לקו אחר) או ל-Case (אם דורשת החלטה בין-מחלקתית), וכל מלאי-בטיחות חורג מטופל מיד.",
          navHe: [
        ...
        },
        {
          ...
          scenarioHe:
            "בארגון ה-key-user מגדיר שלוש Alerts: מלאי-מוגמר < בטיחות (High), ניצולת-קו > 95% (Medium), סטיית-תחזית-מבצע > 20% (Low), כל אחת ברמת-הצבירה הנכונה, ומתזמן חישוב-לילי לקראת ישיבת-ה-S&OP הבוקרת.",
          navHe: [
    ...
    // ============================================================ 7.3
    {
      ...
      scenarioHe:
        "בארגון כשמבצע-קיץ יוצר מחסור-קיבולת חוצה-אזורים, מנהל-S&OP פותח Case 'Summer peak capacity', מצרף את כל המעורבים (מכירות, ייצור, רכש) דרך Work Zone, ומנהל את ההחלטה הבין-אזורית — להזיז ייצור בין מפעלי-בקבוק — עם מעקב-משימות עד סגירה.",
      navHe: [
    ...
    // ============================================================ 7.4
    {
      ...
      scenarioHe:
        "בארגון, בעקבות Case של מחסור-קיבולת-קיץ, נפתחות Tasks אזוריות: 'הזז ייצור 1.5L למפעל-צפון', 'אשר שעות-נוספות בקו-מילוי', 'עדכן הבטחת-אספקה ללקוח-מפתח' — כל אחת עם אחראי ומועד, נעקבות עד השלמה לקראת ישיבת-ה-S&OP.",
      navHe: [
    ...
    // ============================================================ 7.5
    {
      ...
      scenarioHe:
        "בארגון הלולאה הזו היא שגרת-עונת-השיא: חישוב-לילי של חריגי-מלאי-וקיבולת, מסך-חריגים בבוקר, Cases חוצי-אזורים למחסורים-מורכבים, ו-Tasks אזוריות עם אחראים — הכל במרחב-אחד, כך שצוות-S&OP מצומצם מנהל את כל מפעלי-הבקבוק לפי-חריגים בלבד.",
      navHe: [
