/* Project NEO · verification overlay — CDS views (`cds:` ids).
   ...
   Batch 5 (2026-09-24, DATE24): I_ProductSalesData, I_MfgOrderComponent, I_Routing
...
export const CDS_VERIFICATION: VerificationRecord[] = [
    ...
    evidence: [
      {
        ...
        release: "2023.latest",
        accessedAt: DATE24,
        claim: "עמוד ה-VDM הרשמי לכותרת רשימת הפעולות מעל PLKO (מדריך Virtual Data Model and CDS Views, On-Premise 2023 Latest, loio 8c9e297f) נוקב בשם הטכני I_MfgBillOfOperationsChgSt‏ (Data Category: Basic, Dimension; Status: Released) ובמטרה 'retrieve header data of the change state (table PLKO) for a manufacturing bill of operations (routing/recipe)'; גוף העמוד נקרא ב-2026-09-24 דרך scripts/sap-help-body.mjs והציטוט מופיע בו. חיפוש חוזר בשם המדויק 'I_Routing' בשירות החיפוש של help.sap.com‏ (2026-09-24, מוצר SAP_S4HANA_ON-PREMISE) החזיר ארבע רשומות, כולן בתוצר 'Malaysia' ואינן קשורות לתצוגה. במוצר SAP_S4HANA_CLOUD, Old → New: שני עמודי API למחסן ב-2026-09-14, אפס רשומות ב-2026-09-24. לא אותר עמוד רשמי הנוקב בשם I_Routing, ולכן שם זה נשאר ללא אימות רשמי.",
        verificationLevel: "verification_required",
      },
      {
        sourceType: "sap_help",
      ...
      {
        sourceType: "sap_help",
        sourceTitle: "Virtual Data Model and CDS Views",
        url: "https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE/ee6ff9b281d8448f96b4fe6c89f2bdc8/8573b810511948c8a99c0672abc159aa.html?locale=en-US&state=PRODUCTION&version=2023.latest",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.latest",
        accessedAt: DATE24,
        claim: "השאילתה 'I_Routing CDS view Virtual Data Model' בשירות החיפוש של help.sap.com‏ (2026-09-24, מוצר SAP_S4HANA_ON-PREMISE) החזירה 21 רשומות, ואף אחת מהן אינה נוקבת בשם I_Routing בכותרת או בסניפט. בין התוצאות: עמוד המבוא של המדריך (loio 8573b810, 2023 Latest; בסניפט: 'The VDM (virtual data model) forms the basis for data access in SAP S/4HANA in, for example, analytical applications and APIs') והנושא 'CDS Views in Settlement Management' בתוצר What's New in SAP S/4HANA 2022 FPS02 (versionId 2022.002, loio 24360558), שאינו עוסק ברשימות פעולות.",
        verificationLevel: "verification_required",
      },
      {
        sourceType: "sap_help",
        sourceTitle: "Production Routing Header",
        url: "https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE/ee6ff9b281d8448f96b4fe6c89f2bdc8/32d6ac68ef5e4c5589346cae1c2e0ae6.html?locale=en-US&state=PRODUCTION&version=2023.latest",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.latest",
        accessedAt: DATE24,
        claim: "לפי הסניפט, עמוד ה-VDM 'Production Routing Header' (מדריך Virtual Data Model and CDS Views, On-Premise 2023 Latest, loio 32d6ac68) נוקב ב-CDS View Name I_ProductionRoutingHeaderDEX עם Analytical Data Category DIMENSION, ומציין שהתצוגה מייצגת את סוג האובייקט ProductionRoutingHeader. השאילתה 'Routing task list CDS view interface' (2026-09-24, מוצר SAP_S4HANA_ON-PREMISE) החזירה 21 רשומות, ובהן משפחת עמודי Production Routing* (בין היתר Production Routing Main Header‏, loio edee4fb5, וכן Production Routing Sequence‏, Production Routing Component Allocation ו-Production Routing Material Assignment) ואת Maintenance Task List Data ו-Maintenance Task List Operation Data, ולא תצוגה בשם I_Routing. הטענה תחומה לכותרות ולסניפטים; גוף העמוד לא נקרא.",
        verificationLevel: "verification_required",
      },
      {
        sourceType: "repository",
  ...
  },
  {
    ...
    evidence: [
      ...
      {
        sourceType: "sap_help",
        sourceTitle: "help.sap.com search (2026-09-24): \"I_RoutingOperation\", \"Routing Operation Virtual Data Model CDS view\", \"Task List Operations CDS view\" (SAP_S4HANA_ON-PREMISE, default scope)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        accessedAt: DATE24,
        claim: "ממצא שלילי, אימות חוזר ב-2026-09-24: שלוש שאילתות נוספות בשירות החיפוש של help.sap.com אינן מעלות אף רשומה שבה השדה CDS View Name הוא I_RoutingOperation. החיפוש בשם המדויק החזיר חמש רשומות ללא קשר לנושא (Contract Management 1709 FPS01 וארבעה נושאי לוקליזציה למלזיה, כולן עם סניפט ריק). החיפוש 'Routing Operation Virtual Data Model CDS view' (21 רשומות) העלה את Bill of Operations Sequence Operation Assignment, Production Routing Header (I_ProductionRoutingHeaderDEX), Production Routing Main Header ו-Production Order Sequence, ואף אחת מהן אינה נקראת I_RoutingOperation. החיפוש 'Task List Operations CDS view' (21 רשומות) העלה בראש התוצאות תצוגות מצד PM (I_MaintTaskListOpMaterial, I_PckgTaskListOpalLocData, I_MaintTaskListOpData), ובהמשך גם נושאי VDM לניתוב ייצור (Production Routing Operation Subordinate Operation, I_ProdnRoutingOpSubordOpDEX; Manufacturing Bill of Operations Operation Change State; Bill of Operations Sequence Operation Assignment; Production Routing Header ועוד); אף אחד מהם אינו נוקב בשם I_RoutingOperation. התוצאה תואמת את ממצאי 2026-09-14.",
        verificationLevel: "verification_required",
      },
      {
        sourceType: "sap_help",
