/* Project NEO · verification overlay — Fiori apps (`fiori:` ids).
   ...
   transactions where the library prints none). Fourth batch the same day: F5104A, F1339, F2023,
   F2828, F5241 and F2072 (library values copied into data/fiori/apps.ts for all six; each
   status source moved to the record's S32OP library row, the old rows kept or logged as
   Old → New in the notes). Fifth batch the same day: W0029, W0028, F8669, F4587, F5323 and
   F2462 (library values copied into data/fiori/apps.ts for all six, F4587 from its audited
   recommendedAction; W0029 and W0028 stay verification_required on the SAP ERP question; the
   F4587 S32OP library row is one const, F4587_FAL_S32OP, shared with fiori:F3577). Sixth batch
   the same day: F1511A, F0251A, F5460 and W0020 (library values copied into data/fiori/apps.ts
   for all four; F1511A and F5460 move their status source to a new S32OP library const,
   F0251A keeps its app topic as the source and gains the S32OP row, W0020 stays not_available
   on the 2023 deletion record and gains the S24OP = 2022 library row). */
...
const F1339_APP_TOPIC: Evidence = {
  ...
  accessedAt: DATE24,
  claim:
    "תיעוד PP-MRP לגרסת 2025 FPS01 (קישור התיעוד שרשומת הספרייה מדפיסה מפנה ל-topic זה) קובע: 'With this app you can " +
    "create and schedule a job for executing your MRP runs'; תבנית ה-Job החובה היא 'Material Requirements Planning " +
    "(MRP)'; 'This app is available for the roles: SAP_BR_MATL_PLNR_EXT_PROC (Material Planner - External " +
    "Procurement) SAP_BR_PRODN_PLNR (Production Planner)'. SAP_BR_RPLNMT_SPCLST_DC_RFM אינו מוזכר בעמוד זה; הוא " +
    "מופיע ברשומת הספרייה. רכיב לתקלות: PP-MRP; אפליקציות קשורות: Display MRP Master Data Issues, Display MRP Key " +
    "Figures, Monitor Material Coverage.",
  verificationLevel: "sap_official_verified",
};
...
/* ----------------------- 2026-09-24 batch 4: status sources (see records) */

const F5104A_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  ...
  accessedAt: DATE24,
  claim:
    "רשומת ה-xsodata המובנית של ספריית ה-Fiori Apps (נקראה בפועל דרך scripts/fal-app.mjs, לא מעטפת JavaScript) עבור " +
    "F5104A ב-S32OP (ReleaseName 'S/4HANA 2025 FPS01', isPublished 'Published'): ApplicationType 'Transactional', " +
    "UITechnology 'SAP Fiori (SAPUI5)', ApplicationComponent 'PM-FIO-WOC-JC' ('Fiori UI for PM Completion " +
    "Confirmations'). SplitBusinessRole: BusinessRoleName מוביל 'SAP_BR_MAINTENANCE_TECHNICIAN' (RoleID R0090, " +
    "isLeading='X'), נוסף 'SAP_BR_MAINT_SUPERVISOR' (R0198). SplitBusinessCatalog: BusinessCatalogName " +
    "'SAP_EAM_BC_MNTJOB_MNG' ('EAM - Maintenance Job Lists'). SplitTechnicalCatalogs: TechincalCatalog " +
    "'SAP_TC_EAM_COMMON'. SemanticObject/Action (SplitAdditionalIntents): 'MaintenanceJob'/'performJob'. " +
    "RequiredODataServices: 'API_MAINTNOTIFICATION' 0001, 'API_MAINTORDERCONFIRMATION' 0001, " +
    "'UI_MAINTENANCEJOB_MANAGE' 0001 (כולם SoftwareComponentName 'S4CORE 109'). fuzzy.LeadingTransactionCodes " +
    "'IW41'; fuzzy.TransactionCodes (related) 'IW21, IW22, IW23, IW32'. RetrofittedSWCBackend 'S4CORE 109 - SP " +
    "0001' על ProductVersionOfficialNameBackend 'SAP S/4HANA 2025'; RetrofittedSWCUI 'UIS4H 109 - SP 0001'. " +
    "PredecessorDetails: 'W0016' 'Display Job List' (מ-releaseId S26OP), 'W0020' 'Confirm Jobs' (מ-releaseId " +
    "S25OP); NumberofSuccessors=0 (Successors=[]). Notes (RIN): '3493254' (Front-End Server), '3671888' (Back-End " +
    "Server). AppDocumentationLink מפנה לאותו topic id (3da57072a73444f18b5ad8785bc2900e) כמו ראיית ה-sap_help " +
    "הראשונה ברשומה זו. Related_Apps: 'F1511A' 'Create Maintenance Request' (relationType 'Navigation Target').",
  verificationLevel: "sap_official_verified",
};

const F1339_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  sourceTitle: "Schedule MRP Runs - SAP Fiori Apps Reference Library (public OData API, Apps('F1339')/S32OP)",
  product: "SAP S/4HANA",
  edition: "on-premise",
  release: "2025.001",
  ...
  accessedAt: DATE24,
  claim:
    "ספריית SAP Fiori Apps Reference Library (שירות ה-OData הציבורי שלה, דרך scripts/fal-app.mjs) מפרסמת למזהה F1339 " +
    "רשומה בשם 'Schedule MRP Runs', סוג Transactional, טכנולוגיית UI 'SAP Fiori: Generic Job Scheduling Framework', " +
    "סטטוס Published במהדורה S32OP ‏(S/4HANA 2025 FPS01), רכיב PP-FIO-MRP. תפקידים: SAP_BR_MATL_PLNR_EXT_PROC " +
    "‏(R0100-21, Material Planner - External Procurement), SAP_BR_PRODN_PLNR ‏(R0114, Production Planner), " +
    "SAP_BR_RPLNMT_SPCLST_DC_RFM ‏(R0246-176M, Replenishment Specialist - Distribution Center (Retail)). קטלוגים " +
    "עסקיים: SAP_RFM_BC_DC_RPLNMT ‏('Retail Replenishment - Distribution Center') ו-SAP_SCM_BC_MRPRUN " +
    "‏('Production Planning - MRP Runs'); קטלוג טכני SAP_TC_SCM_PP_COMMON; intent: MRPRun-schedule. שירות OData: " +
    "APJ_JOB_MANAGEMENT_SRV גרסה 0001 ‏(SAP_BASIS 816). backend: SAP_BASIS 816 SP 0001 / SAP S/4HANA 2025; UI: " +
    "UIS4H 109 SP 0001. קודמים ויורשים: אין (predecessors: -; successors: -). קודי GUI: מוביל MD01; קשורים MD01N, " +
    "MD02, MD03, MD40, MD41, MD42, MD43, MDBT, MPBT, MSBT. הערות RIN: 3493254 ‏(Front-End Server), 3671888 " +
    "‏(Back-End Server). רשימת המהדורות ברשומה כוללת מהדורות On-Premise מ-S3OP ‏(1511) עד S32OP, מהדורות " +
    "Private Cloud ‏(S29PCE עד S32PCE), ואת S36 ו-S37 ‏(2602, 2608, ללא ציון מהדורה ברשומה). ברשומה הגולמית " +
    "מופיעה אפליקציה קשורה מסוג Required: 'Application Jobs' ‏(F1240).",
  verificationLevel: "sap_official_verified",
};

const F2023_LIBRARY_S32OP: Evidence = {
  sourceType: "fiori_library",
  ...
  accessedAt: DATE24,
  claim:
    "רשומת הספרייה ל-F2023 על S32OP (2025 FPS01), שנקראה דרך ערוץ ה-OData הרשמי (scripts/fal-app.mjs, לא ה-JS shell): " +
    "AppName 'Report and Repair Malfunction', Published, ApplicationType Transactional, UITechnology 'SAP Fiori " +
    "(SAPUI5)', ApplicationComponent PM-FIO-WOC-JC (Fiori UI for PM Completion Confirmations). תפקיד עסקי " +
    "SAP_BR_MAINTENANCE_TECHNICIAN (R0090, Maintenance Technician). קטלוג עסקי SAP_EAM_BC_CORRMAINT_MW (EAM - " +
    "Corrective Maintenance); קטלוג טכני SAP_TC_EAM_COMMON. Semantic Object/Action MaintenanceJob-reportMalfunction. " +
    "שירות OData EAM_MALFUNCTION_MANAGE גרסה 0001 (S4CORE 109). טרנזקציית GUI מובילה IW31, קשורות " +
    "IW21/IW22/IW32/IW41. Backend S4CORE 109 SP0001 / S/4HANA 2025; UI UIS4H 109 SP0001. NumberofPredecessors=0, " +
    "NumberofSuccessors=0. הודעות RIN: 3493254 (Front-End Server), 3671888 (Back-End Server). AppDocumentationLink " +
    "מפנה לנושא 13b3075824570746e10000000a441470, הנושא של רשומת ה-sap_help Report and Repair Malfunction | " +
    "Maintenance Management (F2023_APP_TOPIC) לעיל. בהרצה נפרדת על S27OP (2023, " +
    "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F2023')/S27OP) חוזרים " +
    "אותו תפקיד, אותם קטלוגים ואותן טרנזקציות GUI; ApplicationComponent שם הוא PM-FIO (Fiori User Interface (UI) for " +
    "PM); OData על S4CORE 108, UI UIS4HOP1 900; הודעות RIN 3336823 (Front-End)/3351047 (Back-End). שתי הריצות מציגות " +
    "NumberofPredecessors=0, NumberofSuccessors=0.",
  verificationLevel: "sap_official_verified",
};

const F2828_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  ...
  claim:
    ...
    "Armed Forces'). Business Catalogs: SAP_DFS_BC_MAINTENANCE ('MAINT - Defense Maintenance') ו-SAP_EAM_BC_ORD ('EAM " +
    "- Order'); Technical Catalog SAP_TC_EAM_COMMON. Semantic Object/Action: MaintenanceOrder/monitor. OData Service " +
    "נדרש: EAM_ORDER_MONITOR, Version 0001, Namespace ODATA_EAM_ORD_MON, SoftwareComponentName S4CORE 109. GUI " +
    "Transactions (fuzzy record): LeadingTransactionCodes='IW29', TransactionCodes='IW38'. Backend " +
    "RetrofittedSWCBackend 'S4CORE 109 - SP 0001' / ProductVersionOfficialNameBackend 'SAP S/4HANA 2025'; UI " +
    "RetrofittedSWCUI 'UIS4H 109 - SP 0001'. NumberofPredecessors=0, NumberofSuccessors=0 " +
    ...
    "Database 'HANA DB exclusive'; ICFNodes: EAM_ORD_MONS1 (ראשי) וצמתים נוספים (isAdditional=1) כגון EAM_PO_MONS1 " +
    "ו-EAM_PROCMTS1; רשימת הגרסאות: S12OP=1809 עד S32OP=2025 FPS01 (On-Premise) ו-S32PCE (Private Cloud), וכן " +
    "S36=2602 ו-S37=2608 (SAP S/4HANA Public Cloud).",
  verificationLevel: "sap_official_verified",
};

const F5241_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  sourceTitle: "Manage Maintenance Orders (F5241), SAP Fiori Apps Reference Library, S32OP (S/4HANA 2025 FPS01)",
  product: "SAP S/4HANA",
  edition: "on-premise",
  release: "2025.001",
  ...
  accessedAt: DATE24,
  claim:
    "node scripts/fal-app.mjs F5241 --out /tmp/fal-src (ערוץ ה-OData הרשמי, לא JS shell) מול S32OP (S/4HANA 2025 " +
    "FPS01) החזיר רשומה Published יחידה: ApplicationType 'Transactional', UITechnology 'SAP Fiori elements', " +
    "ApplicationComponent 'PM-FIO-WOC-MO'; BusinessRoleName 'SAP_BR_MAINTENANCE_PLANNER' (RoleID R0088, isLeading " +
    "X); BusinessCatalogName 'SAP_EAM_BC_WORKORD_MNG' (EAM - Work Order Management); TechnicalCatalogName " +
    "'SAP_TC_EAM_COMMON'; SemanticObject 'MaintenanceOrder', SemanticAction 'manageWorkOrder'; " +
    "RequiredODataServiceGroups: serviceGroupName 'UI_MAINTENANCEORDER_MANAGE' (SoftwareComponentVersion S4CORE 109); " +
    "fuzzy.LeadingTransactionCodes 'IW31', fuzzy.TransactionCodes 'IW32, IW33, IW37N, IW38, IW39'; " +
    "RetrofittedSWCBackend 'S4CORE 109 - SP 0001' על ProductVersionOfficialNameBackend 'SAP S/4HANA 2025'; All_Rel " +
    "מדפיס Published על S30OP/S30PCE (2023 FPS03), S31OP/S31PCE (2025), S32OP/S32PCE (2025 FPS01); PredecessorDetails " +
    "מונה F2175 'Find Maintenance Order' (מזהה זה אינו קיים במאגר data/fiori/apps.ts ולכן אינו מקבל xref); Successors " +
    "ריק; Notes: RIN 3493254 (Front-End Server) ו-3671888 (Back-End Server); Related_Apps מונה W0017 'Process " +
    "Maintenance Order' כ-Required (אף הוא לא קיים במאגר). על S27OP (S/4HANA 2023) הבקשה החזירה 'not in this " +
    "release (empty Results)'; זה עקבי עם appFirstRelease S30OP (2023 FPS03) שהספרייה מדפיסה.",
  verificationLevel: "sap_official_verified",
};

const F2072_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  sourceTitle: "SAP Fiori Apps Reference Library: Find Technical Object (F2072), S/4HANA 2025 FPS01",
  product: "SAP S/4HANA",
  edition: "on-premise",
  release: "2025.001",
  ...
  accessedAt: DATE24,
  claim:
    "רשומת ה-OData של ספריית ה-Fiori (fioriId=F2072, releaseId=S32OP, isPublished='Published') מדפיסה: " +
    "ApplicationType 'Transactional', UITechnology 'SAP Fiori elements', ApplicationComponent 'PM-FIO', תפקיד מוביל " +
    "(isLeading=X) SAP_BR_MAINTENANCE_PLANNER (R0088) ועוד עשרה תפקידי SAP_BR נוספים, עשרה קטלוגים עסקיים " +
    "(SAP_EAM_BC_TO, SAP_EAM_BC_TO_DSP, SAP_EAM_BC_TO_MW, SAP_EAM_BC_TO_TMS, SAP_EHS_BC_ENV_MGMT, " +
    "SAP_EHS_BC_HS_HAZMAT_MGMT, SAP_EHS_BC_HS_RSK_MGMT, SAP_EHS_BC_IM_INC_MGMT, SAP_EHS_BC_MOC_CHANGE_MGMT, " +
    "SAP_SOM_BC_SUBSCR_ORDER), קטלוג טכני SAP_TC_EAM_COMMON, SemanticObject/Action " +
    "'MaintenanceObject'/'displayFactSheet', RequiredODataServices עם TechnicalName 'EAM_OBJPG_TECHNICALOBJECT_SRV' " +
    "גרסה 0001 (S4CORE 109), LeadingTransactionCodes 'IE03' ו-TransactionCodes 'IH06, IH08, IL03', ProductVersion " +
    "Backend 'SAP S/4HANA 2025' (RetrofittedSWCBackend S4CORE 109 - SP 0001), PredecessorDetails F0226 (Equipment), " +
    "F0227 (Functional Location), W0011 ו-W0012 (Display Master Data Information Center), NumberofSuccessors=0, " +
    "ReleaseName 'S/4HANA 2025 FPS01'. אותה שאילתה על S27OP (2023) מחזירה Published עם אותו תפקיד מוביל " +
    "SAP_BR_MAINTENANCE_PLANNER, אותם עשרה מזהי קטלוג עסקי (כותרת SAP_EAM_BC_TO שם: 'EAM - Technical Object'), אותו " +
    "שירות OData (S4CORE 108) ואותן טרנזקציות GUI; רשימת התפקידים שם שונה: 14 תפקידים, כולל " +
    "SAP_BR_CUSTOMER_SERVICE_MGR, SAP_BR_CUST_SRVC_MGR_REPA, SAP_BR_CUST_SRVC_REP_REPA, SAP_BR_CUST_SRVC_TEC_REPA " +
    "ו-SAP_BR_SERVICE_PLANNER, וללא SAP_BR_SERVICE_MANAGER ו-SAP_BR_SOLN_BUSINESS_SPCLST; רכיב ה-UI הוא UIS4HOP1 900 " +
    "- SP 0000 ומספרי ה-RIN שונים (RetrofittedSWCBackend S4CORE 108 - SP 0000).",
  verificationLevel: "sap_official_verified",
};

/* ----------------------- 2026-09-24 batch 5: status sources (see records) */

const W0029_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  ...
  accessedAt: DATE24,
  claim:
    "לפי scripts/fal-app.mjs W0029 (ערוץ ה-OData הרשמי של הספרייה, S32OP = S/4HANA 2025 FPS01): 'Process Technical " +
    "Object', ApplicationType ו-UITechnology 'Web Dynpro', isPublished=Published, רכיב EP-PCT-MAN-MW (BP for " +
    "Maintenance Worker). תפקיד עסקי SAP_BR_MAINTENANCE_PLANNER (R0088, Maintenance Planner); קטלוג עסקי SAP_EAM_BC_TO " +
    "'EAM - Technical Object'; קטלוג טכני SAP_TC_EAM_BE_APPS:S4EAM; intent MaintenanceObject-create. " +
    "NumberofOdataServices=0 ו-ODataServices ריק. טרנזקציות GUI: מובילה IE01; קשורות IE02, IE03, IL01, IL02, IL03. " +
    "WDAAppAndConfigName=EAMS_WDA_TECHOBJ_OIF (EAMS_WDA_TECHOBJ_OIF_CFG_V2), " +
    "WebDynproPFCGRole=SAP_COCKPIT_EAMS_GENERIC_FUNC2. backend S4CORE 109 - SP 0001, UI UIS4H 109 - SP 0001. " +
    "predecessors ו-successors: '-'. RIN notes: 3493254 (RIN, Front-End Server), 3671888 (RIN, Back-End Server), " +
    "2488413 (BOM, General Note). All_Rel מונה מהדורות מ-S3OP=1511 ועד S32OP/S32PCE=2025 FPS01 בקבוצת " +
    "releaseGroupId=SOP, וגם S36=2602 ו-S37=2608 בקבוצת releaseGroupId=SC.",
  verificationLevel: "sap_official_verified",
};

const W0028_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  ...
  accessedAt: DATE24,
  claim:
    "scripts/fal-app.mjs W0028 ב-S32OP (S/4HANA 2025 FPS01) מחזיר: Display Technical Object, ApplicationType " +
    "ו-UITechnology = Web Dynpro, isPublished = Published, רכיב EP-PCT-MAN-MW (BP for Maintenance Worker); תפקיד " +
    "SAP_BR_MAINTENANCE_TECHNICIAN (R0090, Maintenance Technician); קטלוג עסקי SAP_EAM_BC_TO_MW ('EAM - Technical " +
    "Object MW'); קטלוג טכני SAP_TC_EAM_BE_APPS:S4EAM; intent MaintenanceObject-display; NumberofOdataServices = 0 ולא " +
    "רשום שירות OData; טרנזקציה מובילה IQ09, ללא טרנזקציות קשורות; backend S4CORE 109 - SP 0001 (SAP S/4HANA 2025), UI " +
    "UIS4H 109 - SP 0001; ללא predecessors ו-successors; ב-All_Rel אפליקציית ה-WDA היא EAMS_WDA_TECHOBJ_OIF " +
    "(EAMS_WDA_TECHOBJ_OIF_MW_CFG_V2); הערות RIN: 3493254 (RIN, Front-End Server), 3671888 (RIN, Back-End Server), " +
    "2488413 (BOM, General Note); קישור התיעוד מפנה ל-topic F11F4D57001C0922E10000000A44147B בגרסה 2025.001.",
  verificationLevel: "sap_official_verified",
};

const F8669_WHATS_NEW_2025_FPS01: Evidence = {
  sourceType: "sap_help",
...
const F4587_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  ...
  accessedAt: DATE24,
  claim:
    "רשומת הספרייה ל-F4587 בגרסת S32OP (2025 FPS01): 'Manage Process Orders', Transactional / SAP Fiori elements, " +
    "סטטוס Published, רכיב PP-FIO-PI (Fiori UI for Process Order). תפקיד: SAP_BR_PRODN_SUPERVISOR_PROC (R0115-11, " +
    "Production Supervisor - Process Manufacturing). קטלוג עסקי: SAP_SCM_BC_PROC_ORD_MGMT 'Production Control " +
    "(Process) - Order Management'; קטלוג טכני: SAP_TC_SCM_PP_COMMON; intent: ProcessOrder-manage. OData: PP_MPE_AOR " +
    "0001 ו-PP_PROCESS_ORDER_MANAGE_SRV 0001 (S4CORE 109), קבוצת V4: PP_MPE_AOR_SRV. שדות טרנזקציית ה-GUI (מובילה " +
    "וקשורה) מודפסים '-'. backend: S4CORE 109 SP 0001; UI: UIS4H 109 SP 0001. רשימת הגרסאות של הרשומה נפתחת ב-S18OP " +
    "(2020) וממשיכה עד S37 (2608), כולל S29PCE עד S32PCE (Private Cloud); הגרסה האחרונה ב-On-Premise ברשימה היא S32OP " +
    "(2025 FPS01). שדות הקודמים והיורשים מודפסים '-'. הערות RIN: 3493254 (Front-End Server), 3671888 (Back-End " +
    "Server).",
  verificationLevel: "sap_official_verified",
};

const F5323_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  sourceTitle: "SAP Fiori Apps Library: F5323 Manage Process Order Operations @ S32OP (S/4HANA 2025 FPS01)",
  product: "SAP S/4HANA",
  edition: "on-premise",
  release: "2025.001",
  ...
  accessedAt: DATE24,
  claim:
    "רשומת הספרייה ל-F5323 בגרסת S32OP: 'Manage Process Order Operations', Transactional / SAP Fiori elements, סטטוס " +
    "Published, רכיב PP-FIO-PI (Fiori UI for Process Order); תפקיד עסקי SAP_BR_PRODN_SUPERVISOR_PROC (R0115-11, " +
    "Production Supervisor - Process Manufacturing); קטלוג עסקי SAP_SCM_BC_PROC_ORD_MGMT (Production Control (Process) " +
    "- Order Management), קטלוג טכני SAP_TC_SCM_PP_COMMON; שירותי OData PP_MPE_AOR ו-PP_PROCESS_ORDER_MANAGE_SRV " +
    "(קבוצת V4 PP_MPE_AOR_SRV, S4CORE 109); טרנזקציית GUI מובילה וקשורה: אין (leading -; related -); backend S4CORE " +
    "109 SP0001, UI UIS4H 109 SP0001; ללא קודמים ויורשים (NumberofPredecessors=0, NumberofSuccessors=0); רשימת הגרסאות " +
    "מתחילה ב-S21OP=2021 וכוללת גם S36=2602 ו-S37=2608; הערות RIN 3493254 (Front-End Server) ו-3671888 (Back-End " +
    "Server); קישור התיעוד: " +
    "https://help.sap.com/http.svc/outputlink?product=SAP_S4HANA_ON-PREMISE&version=2025.001&topic=48d4f057b6a44fb59d3c3d85500a9f42&state=PRODUCTION.",
  verificationLevel: "sap_official_verified",
};

const F2462_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  sourceTitle: "Manage Batches (F2462), SAP Fiori Apps Reference Library, S32OP (S/4HANA 2025 FPS01)",
  product: "SAP S/4HANA",
  edition: "on-premise",
  release: "2025.001",
  ...
  accessedAt: DATE24,
  claim:
    "רשומת הספרייה ל-F2462 על S32OP (2025 FPS01), שנקראה דרך ערוץ ה-OData הרשמי (scripts/fal-app.mjs, לא ה-JS shell): " +
    "Published, ApplicationType Transactional, UITechnology 'SAP Fiori elements', ApplicationComponent LO-BM-FIO " +
    "(Fiori UI for Batch Management). תפקיד עסקי מוביל SAP_BR_INVENTORY_MANAGER (R0082, Inventory Manager); ועוד 18 " +
    "תפקידים ללא דגל 'מוביל' (19 תפקידים בסך הכול; בין השאר SAP_BR_WAREHOUSE_CLERK, SAP_BR_BATCH_MASTER_SPCLST, " +
    "SAP_BR_QUALITY_TECHNICIAN, SAP_BR_PRODN_SUPERVISOR_DISC/PROC/RPTV). חמישה קטלוגים עסקיים: " +
    "SAP_EHS_BC_MOC_CHANGE_MGMT (Health and Safety - Management of Change), SAP_SCM_BC_BATCH_DSP (Batch Management - " +
    "Batch Display), SAP_SCM_BC_BATCH_MGMT (Batch Management - Batches), SAP_SCM_BC_BATCH_MNTR (Batch Management - " +
    "Batch Monitoring), SAP_SCM_BC_MPE_EXEC_MNTR (Production Operation - Execution Monitoring); קטלוג טכני " +
    "SAP_TC_SCM_PP_COMMON. שירות OData: LO_BM_BATCH_SRV גרסה 0001 (S4CORE 109). טרנזקציית GUI מובילה MSC1N, טרנזקציות " +
    "קשורות MSC2N ו-MSC3N. ללא קודמים וללא יורשים (NumberofPredecessors=0, NumberofSuccessors=0). שלוש הערות RIN/BOM: " +
    "2501549 (BOM, General Note, Batch classification in Fiori: updating an existing batch), 3493254 (RIN, Front-End " +
    "Server), 3671888 (RIN, Back-End Server). רשומת S27OP (2023) מדפיסה אותו תפקיד מוביל, אותם חמישה קטלוגים ואותן " +
    "טרנזקציות GUI, עם אותו שירות OData (LO_BM_BATCH_SRV, S4CORE 108) ומספרי הערות RIN שונים (3336823, 3351047).",
  verificationLevel: "sap_official_verified",
};

const F1511A_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  ...
  accessedAt: DATE24,
  claim:
    "רשומת הספרייה ל-F1511A בגרסת S32OP (S/4HANA 2025 FPS01), סטטוס Published, קומפוננטה PM-FIO-WOC-MN (Fiori UI for PM " +
    "Maintenance Notifications): 'Create Maintenance Request', Transactional / SAP Fiori (SAPUI5). תפקידים עסקיים: " +
    "SAP_BR_EMPLOYEE (R0056), SAP_BR_MAINTENANCE_PLANNER (R0088), SAP_BR_MAINTENANCE_TECHNICIAN (R0090), " +
    "SAP_BR_MAINT_SUPERVISOR (R0198), SAP_BR_MD_SPECIALIST_EAM (R0097-180). קטלוגים עסקיים: SAP_EAM_BC_MREQ_MNG 'EAM - " +
    "Maintenance Request', SAP_EAM_BC_TO_MNG 'EAM - Technical Objects Management'; קטלוג טכני SAP_TC_EAM_COMMON. Intent: " +
    "MaintenanceWorkRequest-create. שירות OData: UI_MAINTWORKREQUESTOVW_V2 גרסה 0001 (S4CORE 109); קבוצת V4: " +
    "UI_PRIORITIZATION_PROFILE (S4CORE 109). טרנזקציית GUI מובילה: IW21. קודמת (predecessor): F1511 Request Maintenance; " +
    "אין successor רשום. Backend: S4CORE 109 SP0001 / SAP S/4HANA 2025; UI: UIS4H 109 SP0001. רשימת ה-releases מתחילה " +
    "ב-S21OP=2021 וכוללת גם S36=2602 ו-S37=2608. הערות RIN: 3493254 (Front-End Server), 3671888 (Back-End Server).",
  verificationLevel: "sap_official_verified",
};

const F5460_FAL_S32OP: Evidence = {
  sourceType: "fiori_library",
  sourceTitle: "Advanced Scheduling Board - SAP Fiori Apps Library, structured record (F5460, S32OP)",
  product: "SAP S/4HANA",
  edition: "on-premise",
  release: "2025.001",
  url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F5460')/S32OP",
  accessedAt: DATE24,
  claim:
    "רשומת ספריית היישומים ל-F5460 על S32OP (SAP S/4HANA 2025 FPS01), שנקראה דרך scripts/fal-app.mjs: AppName 'Advanced " +
    "Scheduling Board', Published, Transactional, SAP Fiori (SAPUI5), רכיב יישום SCM-APO-PPS-DS (Detailed Scheduling); " +
    "Description: 'An application to monitor and schedule the capacity of production resources.'. תפקיד עסקי מוביל " +
    "(isLeading) SAP_BR_PRODN_PLNR (R0114, Production Planner); תפקיד נוסף SAP_BR_PRODN_PLNR_APS (R0114-226, Production " +
    "Planner - Advanced Planning). קטלוגים עסקיים SAP_SCM_BC_APS_PROD_PLNG (Advanced Planning - Production Planning) " +
    "ו-SAP_SCM_BC_CAPA_PLAN (Capacity - Capacity Planning); קטלוג טכני SAP_TC_SCM_APS_COMMON. Semantic Object/Action: " +
    "CRPResource-scheduleOrder. שירות OData: UI_SCM_ADVNCD_SCHEDG_BOARD 0001 (S4COREOP 109). טרנזקציית GUI מובילה " +
    "/SAPAPO/CDPS0, ללא טרנזקציות קשורות. backend: S4COREOP 109 - SP 0001; UI: UIS4H 109 - SP 0001. " +
    "NumberofPredecessors=0, NumberofSuccessors=0. הערות RIN: 3632817 ו-3210319 (BOM, General Note), 3493254 (RIN, " +
    "Front-End Server), 3671888 (RIN, Back-End Server). קישור תיעוד: " +
    "https://help.sap.com/http.svc/outputlink?product=SAP_S4HANA_ON-PREMISE&version=2025.001&topic=330bdbd6e033485d8530029a7147c1da&state=PRODUCTION.",
  verificationLevel: "sap_official_verified",
};
...
export const FIORI_VERIFICATION: VerificationRecord[] = [
  ...
  /* -------------------------------------------------------- fiori:F5104A */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "Perform Maintenance Jobs - SAP Fiori Apps Reference Library (F5104A, S27OP, fal-app.mjs)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F5104A')/S27OP",
        accessedAt: DATE24,
        claim:
          "אותה רשומת xsodata עבור F5104A ב-S27OP (ReleaseName 'S/4HANA 2023', isPublished 'Published'): אותם roles " +
          "('SAP_BR_MAINTENANCE_TECHNICIAN' מוביל + 'SAP_BR_MAINT_SUPERVISOR'), אותו BusinessCatalogName " +
          "'SAP_EAM_BC_MNTJOB_MNG', אותם RequiredODataServices ואותם fuzzy.LeadingTransactionCodes/TransactionCodes כמו " +
          "ב-S32OP, אך ApplicationComponent 'PM-FIO' ('Fiori User Interface (UI) for PM'), RetrofittedSWCBackend 'S4CORE " +
          "108 - SP 0000' על ProductVersionOfficialNameBackend 'SAP S/4HANA 2023', RetrofittedSWCUI 'UIS4HOP1 900 - SP " +
          "0000'. Notes (RIN): '3336823' (Front-End Server), '3351047' (Back-End Server). מאשש רציפות זהות האפליקציה בין " +
          "2023 ל-2025 FPS01.",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      status: "s4_native",
      he:
        "Perform Maintenance Jobs (F5104A) היא אפליקציית Fiori שנוספה ב-S/4HANA (What's New 2021, 'App New', פריטי " +
        "היקף 4HH/4HI, רכיב PM), וממשיכה להיות מפורסמת (isPublished 'Published') ומתוחזקת עד S32OP (S/4HANA 2025 " +
        "FPS01). ספריית ה-Fiori Apps (fal-app.mjs, נקראה בפועל ב-S32OP וב-S27OP) מאששת: תפקיד מוביל " +
        "SAP_BR_MAINTENANCE_TECHNICIAN עם SAP_BR_MAINT_SUPERVISOR נוסף, קטלוג עסקי SAP_EAM_BC_MNTJOB_MNG, קטלוג טכני " +
        "SAP_TC_EAM_COMMON, שלושה שירותי OData (API_MAINTNOTIFICATION, API_MAINTORDERCONFIRMATION, " +
        "UI_MAINTENANCEJOB_MANAGE), tcode מוביל IW41 ותקודים קשורים IW21/IW22/IW23/IW32, ושני קודמים (W0016, W0020) " +
        "ללא יורש (NumberofSuccessors=0). המסכים המקבילים ב-SAP GUI לפי נתוני הפרויקט הם IW41/IW42/IW44/IW48 " +
        "לאישורים ו-IK11/IK34 למדידות.",
      edition: "on-premise",
      release: "2025.001",
      source: F5104A_FAL_S32OP,
      recommendedAction:
        "להקצות את האפליקציה (קטלוג עסקי SAP_EAM_BC_MNTJOB_MNG; עד 2026-09-24 רשם הקטלוג המתוחזק " +
        "SAP_EAM_BC_MAINT_WORKER) לתפקיד SAP_BR_MAINTENANCE_TECHNICIAN וכן SAP_BR_MAINT_SUPERVISOR, כנקודת העבודה " +
        "המרכזית לביצוע ולדיווח עבודות (זמן, רכיבים, מדידות ונתוני תקלה), ולוודא שירותי OData API_MAINTNOTIFICATION, " +
        "API_MAINTORDERCONFIRMATION ו-UI_MAINTENANCEJOB_MANAGE פעילים. אין להפנות טכנאים לאפליקציית Confirm Jobs " +
        "(W0020): לפי רשומת What's New 2023 היא נמחקה, וספריית ה-Fiori מאשרת ש-W0020 הוא קודם (predecessor) של " +
        "F5104A. ה-catalogPatch הוחל ב-2026-09-24 על הקטלוג המתוחזק (data/fiori/apps.ts#F5104A), שכולל כעת גם את " +
        "התקודים הקשורים IW21/IW22/IW23/IW32 לצד IW41.",
    },
    xrefs: ["tx:IW41", "tx:IW42", "tx:IW44", "tx:IW48", "tx:IK11", "tx:IK34", "table:AFRU", "table:AUFK", "fm:BAPI_ALM_CONF_CREATE", "fm:BAPI_ALM_ORDER_MAINTAIN", "fiori:F2730", "fiori:F4604", "fiori:W0020", "fiori:F1511A"],
    lastVerifiedAt: DATE24,
    notes:
      ...
      " עדכון 2026-09-24: הרצתי node scripts/fal-app.mjs F5104A --out /tmp/fal-src בשתי המהדורות S32OP ו-S27OP " +
      "(--release S27OP). בניגוד לביקור הקודם (2026-09-02) שבו כתובת הספרייה נקראה כמעטפת JavaScript, שירות ה-xsodata " +
      "החזיר הפעם רשומה מלאה ('found': true, isPublished 'Published') בשתי המהדורות. זה מחליף את ראיית הספרייה הקודמת " +
      "(verification_required) בשתי ראיות sap_official_verified חדשות. הפער העיקרי מול הקטלוג המתוחזק " +
      "(data/fiori/apps.ts#F5104A): עד 2026-09-24 רשם הקטלוג catalog 'SAP_EAM_BC_MAINT_WORKER' עם ההערה 'סביר' (טרם " +
      "אומת) ושדה OData ריק עם הערת 'טרם אומת' ב-explain.technical, בעוד הספרייה מאששת בפועל catalog " +
      "'SAP_EAM_BC_MNTJOB_MNG' ו-3 שירותי OData מפורשים; הקטלוג רשם guiTx יחיד IW41 בעוד הספרייה מוסיפה 4 תקודים " +
      "קשורים (IW21/IW22/IW23/IW32). ה-Related_Apps מונה את F1511A ('Create Maintenance Request', Navigation Target " +
      "בלבד, לא predecessor/successor רשמי); נוסף ל-xrefs לצורך ניווט. הקודמים W0016 ('Display Job List') ו-W0020 " +
      "('Confirm Jobs') מגיעים מ-PredecessorDetails; W0016 אינו קיים בקטלוג data/fiori/apps.ts ולכן לא נכלל ב-xrefs " +
      "(נזכר כאן בטקסט בלבד, לא כיורש/successor נתבע). ה-status נשען כעת על ראיית הספרייה של S32OP כ-source, אך " +
      "תאריך ה-App New המקורי (2021.000, F5104A_WHATS_NEW_2021) לא נמחק; הוא נשמר בהיסטוריית הראיות. Old → New: שורת " +
      "הספרייה הקודמת (fiori_library ברמת verification_required, כתובת " +
      "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/?appId=F5104A שגוף הדף שלה מעטפת JavaScript, " +
      "2026-09-02) הוחלפה בשתי שורות fal-app.mjs (S32OP, S27OP); הקטלוג העסקי שלא אומת, SAP_EAM_BC_MAINT_WORKER, " +
      "הוחלף ב-SAP_EAM_BC_MNTJOB_MNG; מקור הסטטוס עבר מ-F5104A_WHATS_NEW_2021 (release 2021.000) לשורת S32OP (release " +
      "2025.001); המלצת הפעולה הקודמת כללה גם הגדרה של אפשרויות התצורה תחת Define Control Parameters for Perform " +
      "Maintenance Jobs (שיוך עצמי של עבודות, אישור עבודות שהוקצו לטכנאים אחרים) וסימנה את התפקיד העסקי, הקטלוג " +
      "הטכני ושירות ה-OData כדורשים אימות. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל באותו יום על " +
      "data/fiori/apps.ts#F5104A (catalog, odata, guiTx, source, trust, lastReviewed; role, type והשם תאמו), ומשפט " +
      "explain.technical שנקב ב-SAP_EAM_BC_MAINT_WORKER ('סביר') ובהערת 'טרם אומת' ל-OData הוחלף בערכי הספרייה. לא " +
      "בוצעה בדיקה במערכת SAP חיה; הקצאת ה-role והפעלת ה-OData services לא אומתו במערכת.",
  },
  ...
  /* --------------------------------------------------------- fiori:F1339 */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "Schedule MRP Runs - SAP Fiori Apps Reference Library (public OData API, Apps('F1339')/S27OP)",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F1339')/S27OP",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        accessedAt: DATE24,
        claim:
          "במהדורה S27OP ‏(S/4HANA 2023) הספרייה מחזירה את אותו שם וסיווג (Schedule MRP Runs, Transactional, " +
          "Generic Job Scheduling Framework, Published), אותם שלושה תפקידים, אותם שני קטלוגים עסקיים ואותו קטלוג טכני, " +
          "שירות OData ‏APJ_JOB_MANAGEMENT_SRV גרסה 0001 ‏(SAP_BASIS 758), ואותם קודי GUI (מוביל MD01; קשורים " +
          "MD01N, MD02, MD03, MD40, MD41, MD42, MD43, MDBT, MPBT, MSBT). backend: SAP_BASIS 758 SP 0000 / SAP S/4HANA " +
          "2023; UI: UIS4HOP1 900 SP 0000. הערות RIN למהדורה זו: 3336823 ‏(Front-End Server), 3351047 " +
          "‏(Back-End Server).",
        verificationLevel: "sap_official_verified",
      },
      F1339_APP_TOPIC,
      {
        ...
        accessedAt: DATE24,
        claim:
          "מדריך היישום קובע כתנאי מוקדם: 'The user needs authorization to schedule corresponding batch jobs'; אובייקטי " +
          "ההרשאה: S_PROGRAM עם P_GROUP = PPH_MRP, ‏S_PROGNAM עם P_PROGNAM = PPH_MRP_START (בשניהם P_ACTION עם " +
          "BTCSUBMIT, SUBMIT, VARIANT), ו-S_BTCH_JOB עם JOBACTION ‏(DELE, LIST, RELE, SHOW) ו-JOBGROUP; יש לוודא " +
          "שרשומת קטלוג ה-Job‏ SAP_SCM_MRP מתוחזקת בפעילות ה-Customizing 'Activiation of Scope-Dependent Application " +
          "Job Catalog Entries (S/4HANA)' (האיות כפי שמודפס בעמוד). Required SAP Notes: 'Not applicable'. שירות OData " +
          "להפעלה: APJ_JOB_MANAGEMENT_SRV ‏(0001); אפליקציית SAPUI5: NW_APS_APJ; לתפקיד PFCG ניתן להשתמש בקטלוג " +
          "העסקי לדוגמה SAP_SCM_BC_MRPRUN. תחת More Information: 'the roles SAP_BR_MATL_PLNR (Material Planner - " +
          "External Procurement) and SAP_BR_PRODN_PLNR (Production Planner) give a user the authorization to use this " +
          "app', ובנוסף יש להקצות בבקאנד את אובייקט ההרשאה M_MTDI_ORG עבור הפעולות שה-Job מבצע.",
        verificationLevel: "sap_official_verified",
      },
      {
        sourceType: "sap_help",
    ...
    status: {
      status: "s4_native",
      he:
        "אפליקציית Fiori טרנזקציונית לתזמון Job להרצת MRP, מבוססת Generic Job Scheduling Framework; בספרייה קוד ה-GUI " +
        "המוביל הוא MD01, ובתיעוד Retail היא מוצמדת גם ל-MRP Live ‏(MD01N). מפורסמת (Published) בספריית SAP Fiori " +
        "Apps Reference Library במהדורות S/4HANA 2023 ו-2025 FPS01.",
      edition: "on-premise",
      release: "2025.001",
      source: F1339_FAL_S32OP,
      recommendedAction:
        "להקצות SAP_BR_MATL_PLNR_EXT_PROC או SAP_BR_PRODN_PLNR (התפקידים שתיעוד האפליקציה מונה), ואת " +
        "SAP_BR_RPLNMT_SPCLST_DC_RFM כשמדובר בחידוש מלאי במרכז הפצה קמעונאי. לפני ההפעלה: להפעיל את שירות ה-OData‏ " +
        "APJ_JOB_MANAGEMENT_SRV ‏(0001) ואת אפליקציית ה-SAPUI5‏ NW_APS_APJ, לוודא שרשומת קטלוג ה-Job‏ " +
        "SAP_SCM_MRP שמוזכרת במדריך היישום מתוחזקת, ולהקצות את אובייקטי ההרשאה S_PROGRAM, S_PROGNAM, S_BTCH_JOB " +
        "ו-M_MTDI_ORG לפי מדריך היישום.",
    },
    xrefs: ["tx:MD01", "tx:MD01N", "tx:MDBT", "fiori:F0247A", "fiori:F0251"],
    lastVerifiedAt: DATE24,
    notes:
      "עדכון 2026-09-24: scripts/fal-app.mjs החזיר רשומה מובנית מלאה מהספרייה ל-S32OP ‏(2025 FPS01) ול-S27OP " +
      "‏(2023), עם תפקידים, קטלוגים, OData וקודי GUI זהים בשתי המהדורות; שלושת עמודי help.sap.com נקראו מחדש עם " +
      "scripts/sap-help-body.mjs (גרסה 2025.001). Old → New: ראיה [0] הקודמת הייתה רשומת ספרייה ל-S18OP " +
      "‏(https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/#/detail/Apps('F1339')/S18OP, נגישה " +
      "2026-09-02) שאישרה רק את השם 'Schedule MRP Runs' מכותרת התוצאה, כי גוף העמוד נטען כ-JavaScript; היא הוחלפה " +
      "בשתי רשומות ה-OData של הספרייה. status.source עבר מ-topic האפליקציה (fdd11356) לרשומת הספרייה S32OP; status.he " +
      "הקודם קשר את האפליקציה ל-MRP Live ‏(MD01N), והנוכחי מבחין בין קוד ה-GUI המוביל בספרייה (MD01) לבין הצימוד " +
      "ל-MD01N שבעמוד ה-Retail. Old → New (ממצאים קודמים שנשמרים): הקישור ל-MDBT נשען על סקירת הטרנזקציות של CBP " +
      "בתיעוד (MDBT = total planning ברקע) ועל tx-intel של הפרויקט, וכעת MDBT מופיע גם בין קודי ה-GUI הקשורים בספרייה; " +
      "הטבלאות MDKP ו-MDTB שברשומת המאגר אינן ביקום המזהים ולכן אינן ב-xrefs; מקורות נוספים שלא נכללו כרשומות ראיה: " +
      "תיעוד Public Edition 2608 ‏(loio 3e467f54737c8c4ce10000000a4450e5) המציין 'App ID: F1339'; What's New 1809 " +
      "FPS02 ‏(loio 4f972af74c8d42a29a6ae5ba0bb0c8bc) עם אותו צימוד MD01N / F1339; What's New Public Edition 2508 " +
      "המזכיר את הקטלוג העסקי SAP_SCM_BC_MRPRUN_MC ‏('Production Planning - MRP Runs'), ללא אישור לגבי " +
      "On-Premise; בדיקת מבקר (2026-09-07): חיפוש בתיעוד SAP ERP (מוצר SAP_ERP) אחר Schedule MRP Runs app לא העלה " +
      "נושא בשם זה, ההיעדר תומך ב-s4_native אך אינו הוכחה חיובית. אי-התאמה רשמית שלא נפתרה: עמוד 'App " +
      "Implementation: Schedule MRP Runs' כותב את התפקיד 'SAP_BR_MATL_PLNR (Material Planner - External " +
      "Procurement)', בעוד topic האפליקציה והספרייה מדפיסים SAP_BR_MATL_PLNR_EXT_PROC עם אותו תיאור; המקורות אינם " +
      "מבהירים אם מדובר בטעות דפוס או בתפקיד נפרד עם אותו תיאור; יש לבדוק ב-PFCG במערכת. היקף התפקידים: topic " +
      "האפליקציה מונה שני תפקידים והספרייה שלושה (בנוסף SAP_BR_RPLNMT_SPCLST_DC_RFM); זה הבדל בהיקף המקורות ולא " +
      "סתירה. פער מול רשומת המאגר data/fiori/apps.ts (כפי שהייתה עד 2026-09-24): role SAP_BR_MRP_CONTROLLER לא הודפס " +
      "באף אחד מהמקורות הרשמיים שנקראו כאן; הערך catalog 'SAP_SCM_MRP (Job Catalog)' הוא רשומת קטלוג Job המופיעה " +
      "במדריך היישום, אך הקטלוגים העסקיים שהספרייה מפרסמת הם SAP_RFM_BC_DC_RPLNMT ו-SAP_SCM_BC_MRPRUN; guiTx במאגר " +
      "היה MD01N בלבד, והספרייה מובילה ב-MD01. התיקון מוצע ב-catalogPatch. לא אומתו: תצוגות CDS (הספרייה אינה " +
      "מפרסמת CDS לרשומה זו), פירוט פריטי ההיקף (ScopeItems כוללים 1BM ועוד, נצפו בפלט הגולמי בלבד). F1240 " +
      "‏(Application Jobs) אינו ביקום המזהים ולכן אינו ב-xrefs. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch " +
      "הוחל באותו יום על data/fiori/apps.ts#F1339 (role, catalog, odata, guiTx, source, trust, lastReviewed; type " +
      "והשם תאמו), וב-explain של אותה רשומה הוחלפו התפקיד SAP_BR_MRP_CONTROLLER והערת 'טרם אומת' ל-OData בערכי " +
      "הרשומה הזו; קטלוג ה-Job‏ SAP_SCM_MRP נשאר שם כרשומת קטלוג Job. לא בוצעה בדיקה במערכת SAP חיה.",
  },
  ...
  /* --------------------------------------------------------- fiori:F2023 */
  {
    ...
    status: {
      status: "s4_native",
      he:
        "יישום Fiori של S/4HANA: מתועד במדריך Maintenance Management לגרסת 2025 FPS01 (On-Premise), Published בספריית " +
        "Fiori Apps Reference Library הן ב-S32OP (2025 FPS01) והן ב-S27OP (2023), לתפקיד העסקי Maintenance Technician " +
        "(SAP_BR_MAINTENANCE_TECHNICIAN), ונקוב ב-What's New 2022 כאחד היישומים היורשים של Confirm Jobs (W0020).",
      edition: "on-premise",
      release: "2025.001",
      source: F2023_LIBRARY_S32OP,
      recommendedAction:
        "ה-catalogPatch הוחל ב-2026-09-24 על רשומת המאגר data/fiori/apps.ts#F2023: הקטלוג העסקי לפי הספרייה הוא " +
        "SAP_EAM_BC_CORRMAINT_MW (עד 2026-09-24 נרשם שם SAP_EAM_BC_MAINT_WORKER), והקטלוג הטכני SAP_TC_EAM_COMMON " +
        "נוסף לשדה הטכני. הטרנזקציה המובילה לפי הספרייה היא IW31 (עד 2026-09-24 נרשמו שם IW21/IW26); " +
        "IW21/IW22/IW32/IW41 הן טרנזקציות קשורות. שירות ה-OData הוא EAM_MALFUNCTION_MANAGE (במקום הערת 'טרם אומת' " +
        "בשדה הטכני של רשומת המאגר). הספרייה אינה מציגה ליישום קודם או יורש (NumberofPredecessors=0, " +
        "NumberofSuccessors=0). השם, המזהה והתפקיד העסקי תאמו ולא שונו.",
    },
    xrefs: ["tx:IW21", "tx:IW22", "tx:IW24", "tx:IW26", "tx:IW31", "tx:IW32", "tx:IW41", "table:QMEL", "table:AUFK", "table:AFIH", "table:AFRU", "fm:BAPI_ALM_NOTIF_CREATE", "cds:I_MaintenanceNotification", "fiori:F5104A", "fiori:F1511", "enh:badi:NOTIF_EVENT_SAVE", "fiori:W0020"],
    lastVerifiedAt: DATE24,
    notes:
      ...
      " עדכון 2026-09-24 (scripts/fal-app.mjs F2023, S32OP=2025 FPS01 ו-S27OP=2023): שתי הריצות מחזירות Published עם " +
      "אותו תפקיד, אותם קטלוגים ואותה טרנזקציית GUI מובילה, ומיישבות את הפערים שהרשומה סימנה קודם כ'לא אומת' (הספרייה " +
      "נקראה קודם כמעטפת JavaScript). התפקיד SAP_BR_MAINTENANCE_TECHNICIAN אושר (תואם לרשומת המאגר). הקטלוג העסקי לפי " +
      "הספרייה הוא SAP_EAM_BC_CORRMAINT_MW (EAM - Corrective Maintenance), בתוספת קטלוג טכני SAP_TC_EAM_COMMON; זה שונה " +
      "מהקטלוג SAP_EAM_BC_MAINT_WORKER שנרשם עד 2026-09-24 ב-data/fiori/apps.ts#F2023, סתירת מאגר שתוקנה שם " +
      "ב-catalogPatch. שירות ה-OData הוא EAM_MALFUNCTION_MANAGE גרסה 0001 (S4CORE 109 ב-S32OP, S4CORE 108 ב-S27OP), " +
      "והוא נכלל ב-catalogPatch; בכך מאומת גם שם השירות שספר 1 פרק 8 רשם כמקור משני. הטרנזקציה המובילה לפי הספרייה " +
      "היא IW31 (ברשומת המאגר נרשמו עד 2026-09-24 IW21/IW26); IW21, IW22, IW32, IW41 הן טרנזקציות קשורות; נוספו " +
      "ל-xrefs: tx:IW31, tx:IW22, tx:IW32 (tx:IW21 ו-tx:IW41 כבר ברשימה; tx:IW24 ו-tx:IW26 נותרו כהיסטוריה אף שרשומת " +
      "הספרייה אינה מדפיסה אותם). ApplicationComponent שונה בין הגרסאות (PM-FIO-WOC-JC ב-S32OP מול PM-FIO ב-S27OP); " +
      "פרט תיעודי, לא סתירה מהותית. הספרייה אינה מציגה קודם או יורש (NumberofPredecessors=0, NumberofSuccessors=0 " +
      "בשתי הריצות), ולכן לא נוסף xref של קודם או יורש. מקור הסטטוס הועבר מ-F2023_APP_TOPIC (מדריך Maintenance " +
      "Management, 2025.001) לרשומת הספרייה S32OP (2025 FPS01); הניסוח הקודם של הסטטוס והמלצת הפעולה הקודמת (אימות " +
      "התפקיד והקטלוג SAP_EAM_BC_MAINT_WORKER בספרייה, אי הסתמכות על F2730) נשמרים כאן כהיסטוריה. תוספת 2026-09-24 " +
      "(שלב הכתיבה): ה-catalogPatch הוחל באותו יום על data/fiori/apps.ts#F2023 (catalog, odata, guiTx, source, trust, " +
      "lastReviewed; role, type והשם תאמו), ומשפט explain.technical שנקב בהערת 'טרם אומת' ל-OData/CDS הוחלף בשירות " +
      "ובקטלוג הטכני שהספרייה מדפיסה. לא בוצעה בדיקה במערכת SAP חיה.",
  },
  ...
  /* --------------------------------------------------------- fiori:F2828 */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "Fiori Apps Library · App F2828 'Maintenance Planning Overview' (SAP Fiori elements: Overview Page), release S27OP",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F2828')/S27OP",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        accessedAt: DATE24,
        claim:
          "אותה רשומה בספרייה עבור S27OP (S/4HANA 2023, isPublished='Published'): אותם Application Type, UI Technology, " +
          "Business Roles (SAP_BR_MAINTENANCE_PLANNER מוביל, SAP_BR_MAINT_TECH_OFFICER), Business Catalogs " +
          "(SAP_DFS_BC_MAINTENANCE, SAP_EAM_BC_ORD), Technical Catalog SAP_TC_EAM_COMMON, ושירות OData EAM_ORDER_MONITOR " +
          "Version 0001; RetrofittedSWCBackend 'S4CORE 108 - SP 0000', RetrofittedSWCUI 'UIS4HOP1 900 - SP 0000'; " +
          "NumberofPredecessors=0, NumberofSuccessors=0; RIN Notes 3336823 (Front-End Server) ו-3351047 (Back-End " +
          "Server); AppDocumentationLink לגרסת 2023.000.",
        verificationLevel: "sap_official_verified",
      },
      F2828_LIBRARY,
      {
    ...
    status: {
      status: "s4_native",
      he:
        "יישום Fiori אנליטי מסוג Overview Page למתכנן התחזוקה. ספריית SAP Fiori Apps Reference Library " +
        "(scripts/fal-app.mjs) מתעדת אותו כ-'Published' מ-S/4HANA 1809 ועד 2025 FPS01 (On-Premise ו-Private Cloud) וגם " +
        "ב-SAP S/4HANA Public Cloud (2602, 2608); Database 'HANA DB exclusive'; רשימת הגרסאות בספרייה אינה מונה מהדורת " +
        "SAP ERP.",
      edition: "on-premise",
      release: "2025.001",
      ...
      recommendedAction:
        ...
        "S4CORE 108 ב-2023), ולהתאים את פריטי ההיקף שנוקבו ב-What's New 2023 (4HH/4HI/BH1/BH2/BJ2) לתהליך התחזוקה " +
        "שנבחר. היישום דורש HANA DB, ולכן אינו חלק מתרחיש ECC.",
    },
    ...
    lastVerifiedAt: DATE24,
    notes:
      "עדכון 2026-09-24: אומת מחדש דרך הערוץ הרשמי הסקריפטי (scripts/fal-app.mjs F2828, S32OP ו-S27OP); בביקור דפדפן " +
      "קודם (2026-09-02) טבלאות ה-role/catalog/odata הוצגו כ-'No data' ללא התחברות, וכעת ערוץ ה-OData הציבורי מחזיר " +
      "נתונים מלאים ותואמים בין S32OP ל-S27OP. Business Role מוביל SAP_BR_MAINTENANCE_PLANNER תואם את רשומת המאגר " +
      "(data/fiori/apps.ts#F2828), אך ה-Business Catalogs שהספרייה מדפיסה (SAP_DFS_BC_MAINTENANCE, SAP_EAM_BC_ORD) " +
      "שונים מהקטלוג שהיה ברשומת המאגר עד 2026-09-24 (SAP_EAM_BC_ORD_MC); הפער הוצג ב-catalogPatch להחלטת בעל התוכן. " +
      "ה-GUI Transaction המוביל לפי הספרייה הוא IW29 (עם IW38 כקשור), בעוד רשומת המאגר הציגה עד 2026-09-24 guiTx " +
      "IP10/IP30/IW38 (תכניות תחזוקה); התיאור הרשמי (Key Features) עוסק בהודעות שטרם שויכו, פקודות שלא שוחררו " +
      "ודרישות/הזמנות רכש לחלפים, לא בתזמון תוכניות; גם פער זה הוצג ב-catalogPatch להחלטת בעל התוכן. " +
      "NumberofPredecessors=0 ו-NumberofSuccessors=0 בשתי הגרסאות: הספרייה אינה מציגה successor או predecessor, ולכן " +
      "לא נוספו xrefs מסוג fiori. Related_Apps (Navigation Target) מציג F2071/F2173/F2175, ו-Required מציג " +
      "F2827/F3065; אלה יעדי ניווט/תלות ולא predecessor/successor, ולא נוספו כ-xref לפי כלל §1. השם, ה-App ID וה-URL " +
      "הועתקו מילולית מפלטי הסקריפט. מקור הסטטוס עבר מ-F2828_LIBRARY (עמוד הספרייה הקלאסי שנקרא בדפדפן, 2026-09-02) " +
      "לשורת S32OP של fal-app.mjs; הסטטוס (s4_native) וה-release (2025.001) ללא שינוי, ושורת הספרייה הקלאסית נשארה " +
      "בראיות. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל באותו יום על data/fiori/apps.ts#F2828 (role, " +
      "catalog, odata, guiTx, source, trust, lastReviewed; type והשם תאמו), יחד עם explain.consultant, " +
      "explain.technical ו-commonErrors שנקבו ב-SAP_EAM_BC_ORD_MC או בהערת 'טרם אומת' ל-OData; ecc, problem " +
      "ו-relatedTables באותה רשומה (IP10/IP30, MPLA/MHIS) נשארו אצורים. לא בוצעה בדיקה במערכת SAP חיה. " +
      "ממצא קודם (2026-09-02): " +
      ...
      "(תכניות תחזוקה), בעוד התיאור הרשמי עוסק בהודעות, בפקודות ובדרישות/הזמנות רכש לחלפים לא-מלאיים; ה-xrefs כאן " +
      "(IW28/IW29/IW38/IW39, QMEL/AUFK/EBAN) הוגדרו אז כעזרי ניווט של המאגר ולא כמיפוי רשמי (Old → New: הספרייה " +
      "מדפיסה כעת IW29/IW38). רשימת תשעת הכרטיסים (Missing Components, Orders for Planning, Purchase Requisitions Not " +
  ...
  /* ---- 2026-09-23 batch: official App IDs that the records above already named (F5241, F2072,
     ...
     read. F5777 was not added: no SAP_S4HANA_ON-PREMISE record names it. F5241 and F2072 were
     enriched on 2026-09-24 from the library's public OData service (scripts/fal-app.mjs, S32OP
     and S27OP); their 2026-09-23 rows are unchanged. ---- */
  ...
  /* --------------------------------------------------------- fiori:F5241 */
  {
    ...
    status: {
      status: "s4_native",
      he:
        "יישום Fiori של S/4HANA לניהול פקודות תחזוקה, מפורסם (Published) בספריית ה-Fiori בגרסת 2025 FPS01: תפקיד עסקי " +
        "SAP_BR_MAINTENANCE_PLANNER, קטלוג עסקי SAP_EAM_BC_WORKORD_MNG, קטלוג טכני SAP_TC_EAM_COMMON, קבוצת שירות OData " +
        "V4 UI_MAINTENANCEORDER_MANAGE, וטרנזקציית GUI מובילה IW31. נוסף כחדש ב-2023 FPS03 (Type New) ואינו מופיע " +
        "בספרייה עבור 2023 (S27OP).",
      edition: "on-premise",
      release: "2025.001",
      source: F5241_FAL_S32OP,
      recommendedAction:
        "להשתמש במזהה F5241 לכותרת Manage Maintenance Orders, עם תפקיד SAP_BR_MAINTENANCE_PLANNER, קטלוג עסקי " +
        "SAP_EAM_BC_WORKORD_MNG, שירות OData UI_MAINTENANCEORDER_MANAGE וטרנזקציית GUI מובילה IW31. הרשומה המתוחזקת " +
        "F2731 נושאת את אותה כותרת בלי מקור רשמי (ראו fiori:F2731).",
    },
    xrefs: ["fiori:F2731", "fiori:F4604", "tx:IW31", "tx:IW32", "tx:IW33", "tx:IW37N", "tx:IW38", "tx:IW39"],
    lastVerifiedAt: DATE24,
    notes:
      "עודכן 2026-09-24 (מחליף את גרסת 2026-09-23). מה נוסף: הרצת scripts/fal-app.mjs F5241 מול ערוץ ה-OData הרשמי של " +
      "ספריית ה-Fiori (לא JS shell) על S32OP ו-S27OP. זה סוגר את הפערים שנשארו פתוחים ב-2026-09-23 " +
      "(role/catalog/OData/CDS/GUI-tx לא מאומתים): התפקיד, הקטלוג העסקי, הקטלוג הטכני, קבוצת שירות ה-OData והטרנזקציה " +
      "המובילה אומתו כעת ישירות מהספרייה. עדיין לא אומתו: תצוגות CDS ספציפיות (הספרייה אינה מדפיסה CDS views), זמינות " +
      "ב-SAP S/4HANA Cloud Public Edition (All_Rel שנקרא מונה מהדורות On-Premise ו-Private Cloud; PredecessorDetails " +
      "מציג גם קבוצת S37, SAP S/4HANA Public Cloud, ולכן הזמינות לא נקבעה כאן), והתוכן של Description (טקסט תיאורי, " +
      "לא צוטט בשדה status.he). קודם F2175 'Find Maintenance Order' כ-predecessor ו-W0017 'Process Maintenance Order' " +
      "כ-required app מוזכרים בראיה אך אינם נכנסים ל-xrefs כי אינם קיימים ב-data/fiori/apps.ts. History (Old → New): " +
      "ב-2026-09-23 הסטטוס נשען על שלוש רשומות sap_help (app topic, What's New 2023 FPS03, What's New 2025 FPS01) " +
      "ותפקיד/קטלוג/OData סומנו כלא מאומתים ב-notes; ב-2026-09-24 נוספה ראיית fiori_library המאמתת אותם ישירות והוחלף " +
      "source הסטטוס אליה. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל באותו יום על data/fiori/apps.ts#F5241 " +
      "(role, catalog, odata, guiTx, source, lastReviewed; type, trust והשם תאמו), ומשפט explain.technical שאמר " +
      "שהתפקיד, הקטלוג ושירות ה-OData לא נקראו מהספרייה הוחלף בערכי הספרייה. לא בוצעה בדיקה במערכת SAP חיה. נוסח " +
      "2026-09-23: " +
  ...
  /* --------------------------------------------------------- fiori:F2072 */
  {
    ...
    status: {
      status: "s4_native",
      he:
        "יישום Fiori Elements של S/4HANA (App ID F2072, PM-FIO) לאיתור אובייקטים טכניים; מתועד בספריית יישומי ה-Fiori " +
        "כ-Published על S32OP (2025 FPS01) ועל S27OP (2023). ב-S32OP התפקיד העסקי המוביל הוא Maintenance Planner " +
        "(SAP_BR_MAINTENANCE_PLANNER), הקטלוג הטכני SAP_TC_EAM_COMMON ושירות ה-OData EAM_OBJPG_TECHNICALOBJECT_SRV. " +
        "מ-2023 הוא היישום היורש של Display Master Data Information Center (W0011, W0012); ל-F2072 עצמו אין successor " +
        "רשום בספרייה.",
      edition: "on-premise",
      release: "2025.001",
      source: F2072_FAL_S32OP,
      recommendedAction:
        "F2072 מוצג עם התפקיד העסקי, הקטלוגים ושירות ה-OData שהספרייה מדפיסה ל-S32OP (הועתקו ב-2026-09-24 לרשומת " +
        "הקטלוג data/fiori/apps.ts#F2072); להשתמש בו לאיתור, להצגה ולעריכה המונית של אובייקטים טכניים ולניהול המבנה " +
        "(התקנה ופירוק של ציוד). הרשומה המתוחזקת F2730A (Manage Technical Objects) אינה נתמכת במקור רשמי (ראו " +
        "fiori:F2730A). ארבעת ה-predecessors שהספרייה מדפיסה (F0226, F0227, W0011, W0012) אינם קיימים כ-fiori id " +
        "בקטלוג ולכן לא נוספו כ-xrefs.",
    },
    xrefs: ["fiori:F2730A", "fiori:W0028", "tx:IE01", "tx:IE02", "tx:IE03", "tx:IH06", "tx:IH08", "tx:IL03"],
    lastVerifiedAt: DATE24,
    notes:
      ...
      "אובייקט; זה עזר ניווט ולא מיפוי החלפה. תוספת 2026-09-24: scripts/fal-app.mjs F2072 הורץ על S32OP ועל S27OP " +
      "(ערוץ OData של הספרייה); שתי המהדורות Published עם אותו תפקיד מוביל, אותם מזהי קטלוג עסקי, אותו שירות OData " +
      "ואותן טרנזקציות GUI; רשימת התפקידים שונה (11 ב-S32OP, 14 ב-S27OP), וכך גם רכיב ה-UI ומספרי ה-RIN. evidence[6] " +
      "מבוסס על S32OP. source של הסטטוס: F2072_APM_TOPIC (sap_help, ללא נתוני קטלוג) → F2072_FAL_S32OP " +
      "(fiori_library); הסטטוס וה-release (2025.001) ללא שינוי. xrefs חדשים: tx:IH06, tx:IH08, tx:IL03 (הטרנזקציות " +
      "הקשורות שהספרייה מדפיסה). ה-predecessors שהספרייה מדפיסה (F0226 Equipment, F0227 Functional Location, W0011, " +
      "W0012) לא נוספו כ-xrefs כי אינם קיימים כ-fiori id ב-data/fiori/apps.ts. תוספת 2026-09-24 (שלב הכתיבה): " +
      "ה-catalogPatch הוחל באותו יום על data/fiori/apps.ts#F2072 (role, catalog, odata, guiTx, source, lastReviewed; " +
      "type, trust והשם תאמו), ומשפט explain.technical שאמר שהתפקיד, הקטלוג ושירות ה-OData לא נקראו מהספרייה הוחלף " +
      "בערכי הספרייה. לא אומתו: תצוגות CDS וזמינות ב-Public Cloud (S36/S37 מופיעות ב-All_Rel בלבד). לא בוצעה בדיקה " +
      "במערכת SAP חיה.",
  },
  ...
  /* --------------------------------------------------------- fiori:W0029 */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Reference Library: Process Technical Object (W0029), S27OP (S/4HANA 2023)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('W0029')/S27OP",
        accessedAt: DATE24,
        claim:
          "לפי scripts/fal-app.mjs W0029 --release S27OP (S/4HANA 2023): 'Process Technical Object', Web Dynpro, " +
          "isPublished=Published, רכיב EP-PCT-MAN-MW. אותו תפקיד SAP_BR_MAINTENANCE_PLANNER (R0088, Maintenance " +
          "Planner), אותו קטלוג עסקי SAP_EAM_BC_TO ואותו קטלוג טכני SAP_TC_EAM_BE_APPS:S4EAM, OData '-', אותן " +
          "טרנזקציות GUI (מובילה IE01; קשורות IE02, IE03, IL01, IL02, IL03). backend S4CORE 108 - SP 0000, UI UIS4HOP1 " +
          "900 - SP 0000. predecessors ו-successors: '-'. RIN notes: 3336823 (RIN, Front-End Server), 3351047 (RIN, " +
          "Back-End Server), ו-2488413 (BOM, General Note) המשותף לשתי המהדורות.",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      status: "verification_required",
      he:
        "Process Technical Object (W0029) היא אפליקציית Web Dynpro ליצירה ולשינוי של אובייקטים טכניים. בספריית יישומי " +
        "ה-Fiori היא מתועדת כ-Published ב-S32OP (S/4HANA 2025 FPS01) וב-S27OP (S/4HANA 2023), עם תפקיד Maintenance " +
        "Planner (SAP_BR_MAINTENANCE_PLANNER), קטלוג עסקי EAM - Technical Object (SAP_EAM_BC_TO), " +
        "NumberofOdataServices=0 בספרייה, וטרנזקציית GUI מובילה IE01. All_Rel מונה אותה מ-1511 ועד 2025 FPS01, ובנוסף " +
        "2602 ו-2608 בקבוצת releaseGroupId=SC. הספרייה אינה מדפיסה predecessor או successor. תיעוד SAP ERP 6.0 EHP8 " +
        "מתאר כבר אפליקציות Web Dynpro ליצירה, לשינוי ולהצגה של אובייקט טכני ב-Business Package for Generic EAM " +
        "Functions 1.61, ולכן עדיין לא הוכרע אם W0029 חדשה ב-S/4HANA או אותה אפליקציה מ-SAP ERP.",
      edition: "on-premise",
      release: "2025.001",
      source: W0029_FAL_S32OP,
      recommendedAction:
        "להציג את W0029 כזמינה ב-S/4HANA On-Premise עם התפקיד, הקטלוג וטרנזקציות ה-GUI שהספרייה מדפיסה, בלי לסמן אותה " +
        "כחדשה ב-S/4HANA. לציין שזו אפליקציית Web Dynpro ללא שירות OData בספרייה. לפני הכרעה אם היא חדשה או ממשיכה " +
        "אפליקציה מ-ECC, להשוות במערכת את אפליקציית ה-Web Dynpro EAMS_WDA_TECHOBJ_OIF ואת תפקיד ה-PFCG " +
        "SAP_COCKPIT_EAMS_GENERIC_FUNC2 לאפליקציות של Business Package for Generic EAM Functions ב-SAP ERP.",
    },
    xrefs: ["tx:IE01", "tx:IE02", "tx:IE03", "tx:IL01", "tx:IL02", "tx:IL03", "fiori:W0028", "fiori:F2730A"],
    lastVerifiedAt: DATE24,
    notes:
      "נוסף 2026-09-23, עודכן 2026-09-24. שיטה ב-2026-09-23: scripts/sap-help-search.mjs בארבע שאילתות על " +
      "SAP_S4HANA_ON-PREMISE 2025.001 (המזהה, שאילתה ממוקדת לרשומת ה-BAdI, קטלוג הנושא ותיאור תפקיד המתכנן) ובשאילתה " +
      "אחת על SAP_ERP. תוספת 2026-09-24: node scripts/fal-app.mjs W0029 (S32OP) ו-node scripts/fal-app.mjs W0029 " +
      "--release S27OP; בשתי המהדורות אותו תפקיד מוביל, אותו קטלוג עסקי וטכני ואותן טרנזקציות GUI, OData '-' " +
      "ו-predecessors/successors '-'. בזכות זה נסגרו הפערים שסומנו ב-2026-09-23 בתפקיד העסקי, בקטלוג, בטרנזקציות ה-GUI " +
      "ובשם אפליקציית ה-Web Dynpro (EAMS_WDA_TECHOBJ_OIF). לפני 2026-09-24 הקטלוג SAP_EAM_BC_TO נלקח מנושא Process " +
      "Technical Object לפי זהות הכותרת, וה-xref ל-IL02 נשען על רשימת ה-What's New; עתה שניהם נשענים על ספריית " +
      "ה-Fiori. status.source: W0029_WHATS_NEW_2025_FPS01 (sap_help) → W0029_FAL_S32OP (fiori_library); הסטטוס " +
      "וה-release (2025.001) ללא שינוי. הסטטוס נשאר 'נדרש אימות נוסף' ולא 'חדש ב-S/4HANA': All_Rel מונה מהדורות " +
      "S/4HANA (1511 עד 2025 FPS01, 2602, 2608), ולכן אינו מכריע מול הראיה מ-SAP ERP 6.0 EHP8. ה-xrefs לטרנזקציות " +
      "IE01, IE02, IE03, IL01, IL02 ו-IL03 נשענים על שדות טרנזקציות ה-GUI בספרייה; זה עזר ניווט ולא מיפוי החלפה. סוג " +
      "היישום Transactional ברשומת הקטלוג הוא ערך הסכימה הקרוב (אין בסכימה ערך Web Dynpro); הספרייה מדפיסה " +
      "ApplicationType 'Web Dynpro'. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל על data/fiori/apps.ts#W0029 " +
      "(role, guiTx, source, lastReviewed; catalog, type, trust ושם תאמו; odata נשאר ריק), ומשפט explain.technical " +
      "שאמר שהתפקיד ושירות ה-OData לא נקראו הוחלף בערכי הספרייה. לא אומתו: זמינות ב-Public Cloud (S36/S37 בקבוצת SC " +
      "הופיעו ב-All_Rel אך לא נשלפו בנפרד) וגרסת הצגה ראשונה. לא בוצעה בדיקה במערכת SAP חיה.",
  },
  ...
  /* --------------------------------------------------------- fiori:W0028 */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Reference Library, app id W0028 (S27OP lookup)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('W0028')/S27OP",
        accessedAt: DATE24,
        claim:
          "scripts/fal-app.mjs W0028 ב-S27OP (S/4HANA 2023) מחזיר את אותו שם Display Technical Object, סוג Web Dynpro, " +
          "Published, תפקיד SAP_BR_MAINTENANCE_TECHNICIAN, קטלוג עסקי SAP_EAM_BC_TO_MW, intent " +
          "MaintenanceObject-display וטרנזקציה מובילה IQ09, עם backend S4CORE 108 - SP 0000 (SAP S/4HANA 2023), UI " +
          "UIS4HOP1 900 - SP 0000 והערות RIN 3336823 (RIN, Front-End Server), 3351047 (RIN, Back-End Server), 2488413 " +
          "(BOM, General Note). All_Rel מונה 33 שחרורי On-Premise/Private Cloud מ-S3OP (1511) עד S32PCE (2025 FPS01), " +
          "ללא S17OP (1909 FPS02), ובנוסף S36 (2602) ו-S37 (2608) בקבוצת SC (SAP S/4HANA Public Cloud), כולם " +
          "Published.",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      status: "verification_required",
      he:
        "Display Technical Object (W0028) רשומה בספריית יישומי ה-Fiori כ-Web Dynpro מפורסמת ב-S/4HANA 2025 FPS01 " +
        "(S32OP) וב-2023 (S27OP), עם התפקיד SAP_BR_MAINTENANCE_TECHNICIAN, הקטלוג SAP_EAM_BC_TO_MW והטרנזקציה המובילה " +
        "IQ09. תיעוד SAP ERP 6.0 EHP8 מתאר iView מבוסס Web Dynpro באותו שם, ולכן לא נקבע אם היא חדשה ב-S/4HANA או " +
        "ממשיכה את ה-iView של SAP ERP.",
      edition: "on-premise",
      release: "2025.001",
      source: W0028_FAL_S32OP,
      recommendedAction:
        "להציג את W0028 כזמינה ב-S/4HANA On-Premise 2025 FPS01 עם התפקיד SAP_BR_MAINTENANCE_TECHNICIAN, הקטלוג " +
        "SAP_EAM_BC_TO_MW והטרנזקציה המובילה IQ09, בלי לסמן אותה כחדשה ב-S/4HANA, ולזכור שלפי תיעוד ההגירה היא מציגה " +
        "את נתוני שפת ההתחברות בלבד. ההכרעה בין אפליקציה חדשה לבין המשך ה-iView של SAP ERP " +
        "(com.sap.pct.erp.eam.gen.eam_technical_object_display) דורשת מקור שנוקב בשניהם; רשומת הספרייה אינה מכריעה.",
    },
    xrefs: ["tx:IQ09", "tx:IE03", "tx:IL03", "fiori:F2072", "fiori:W0029", "fiori:F2730A"],
    lastVerifiedAt: DATE24,
    notes:
      "נוסף 2026-09-23, עודכן 2026-09-24. Old → New: ברשומה מ-2026-09-23 (שיטה: scripts/sap-help-search.mjs בשתי " +
      "שאילתות על SAP_S4HANA_ON-PREMISE 2025.001, המזהה עם השם וקטלוג הנושא, ובשתי שאילתות על SAP_ERP) מקור הסטטוס היה " +
      "W0028_MIGRATION_EQUIPMENT, ולא אומתו תפקיד עסקי, שם אפליקציית ה-Web Dynpro, זמינות ב-Public Cloud וגרסת הצגה " +
      "ראשונה. כעת scripts/fal-app.mjs W0028 ב-S32OP וב-S27OP מאמת את התפקיד SAP_BR_MAINTENANCE_TECHNICIAN, הקטלוג " +
      "SAP_EAM_BC_TO_MW, הקטלוג הטכני SAP_TC_EAM_BE_APPS:S4EAM, ה-intent MaintenanceObject-display, הטרנזקציה המובילה " +
      "IQ09, סוג היישום Web Dynpro ואפליקציית ה-WDA EAMS_WDA_TECHOBJ_OIF, ומקור הסטטוס עבר לשורת הספרייה S32OP " +
      "(W0028_FAL_S32OP). הסטטוס נשאר verification_required: הספרייה מאמתת זמינות, תפקיד וקטלוג, אך אינה מכריעה אם " +
      "W0028 חדשה ב-S/4HANA; רשומת הספרייה אינה נוקבת ב-iView של SAP ERP לשום כיוון. ברשומת הספרייה " +
      "NumberofOdataServices=0 ולא רשום שירות OData, ולכן השדה odata בקטלוג נשאר ריק; NumberofPredecessors=0 " +
      "ו-NumberofSuccessors=0, ולכן אין successor רשום בספרייה. קישור התיעוד של הספרייה מפנה לנושא Display Technical " +
      "Object (topic F11F4D57001C0922E10000000A44147B), כך שהקישור בין הנושא ל-W0028, שנשען קודם על זהות הכותרת בלבד, " +
      "נשען כעת גם על הספרייה. סוג היישום Transactional ברשומת הקטלוג נשאר ערך הסכימה (FioriType אינו כולל Web " +
      "Dynpro); הספרייה מדפיסה Web Dynpro. לפי חיפושי SAP_ERP מ-2026-09-23 אותו iView מופיע גם ב-Business Package for " +
      "Maintenance Worker 1.61 (SAP ERP 6.0 EHP6 on HANA ו-EHP8). ה-xrefs ל-IE03 ול-IL03 נשענים על קטעי ההגירה של PM - " +
      "Equipment ו-PM - Functional location, וה-xref ל-IQ09 נשען על הטרנזקציה המובילה בספרייה; אלה עזרי ניווט ולא " +
      "מיפוי החלפה. All_Rel מונה גם S36 (2602) ו-S37 (2608) בקבוצה SC; זמינות ב-Public Cloud לא נבדקה בהרצה עם " +
      "--release S37 ואינה נטענת כאן. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל על data/fiori/apps.ts#W0028 " +
      "(role, guiTx, source, lastReviewed; catalog, type, trust ושם תאמו; odata נשאר ריק), ומשפט explain.technical " +
      "שאמר שהתפקיד ושירות ה-OData לא נקראו מהספרייה הוחלף בערכי הספרייה. לא בוצעה בדיקה במערכת SAP חיה.",
  },
  ...
  /* --------------------------------------------------------- fiori:F8669 */
  {
    ...
    evidence: [
      F8669_APP_TOPIC,
      F8669_WHATS_NEW_2025_FPS01,
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Reference Library: Manage Technical Object Structures (F8669), S/4HANA 2025 FPS01",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2025.001",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F8669')/S32OP",
        accessedAt: DATE24,
        claim:
          "רשומת הספרייה ל-F8669 על S32OP (2025 FPS01), שנקראה דרך ערוץ ה-OData הרשמי (scripts/fal-app.mjs, לא מעטפת " +
          "ה-JavaScript): AppName/EnglishTitle 'Manage Technical Object Structures', isPublished Published, " +
          "ApplicationType Transactional, UITechnology 'SAP Fiori elements', ApplicationComponent PM-FIO-EQM-RS " +
          "('Fiori UI for PM Reference Structures'). תפקיד עסקי מוביל (isLeading X): SAP_BR_MD_SPECIALIST_EAM " +
          "(R0097-180, 'Master Data Specialist - Maintenance Management'). קטלוג עסקי SAP_ASM_BC_REFTO_MNG ('ASM - " +
          "Reference Technical Objects'); קטלוג טכני SAP_TC_EAM_COMMON. Semantic Object/Action: " +
          "DraftTechnicalObjectStructure-manage. רשימת RequiredODataServices ריקה; קבוצת שירות V4 " +
          "UI_DRFTTECHOBJSTRUCTURE_MANAGE (S4CORE 109). NumberofPredecessors=0, NumberofSuccessors=0 " +
          "(PredecessorDetails/Successors ריקים), ואין טרנזקציית GUI מובילה או קשורה ברשימה. Backend S4CORE 109 SP " +
          "0001 (SAP S/4HANA 2025); UI UIS4H 109 SP 0001. הודעות RIN: 3493254 (Front-End Server), 3671888 (Back-End " +
          "Server). לא מודפס קישור תיעוד ליישום (docs: -). הרצה נפרדת על S27OP (2023) החזירה Results ריק: F8669 אינו " +
          "בספרייה בגרסת 2023.",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      ...
      he:
        ...
        "אובייקטים טכניים בטיוטה, מתועד ב-Maintenance Management לגרסת 2025 FPS01 תחת App ID F8669. ספריית יישומי " +
        "ה-Fiori (S32OP) מאשרת: Published, תפקיד SAP_BR_MD_SPECIALIST_EAM, קטלוג עסקי SAP_ASM_BC_REFTO_MNG, קבוצת " +
        "שירות V4 UI_DRFTTECHOBJSTRUCTURE_MANAGE, ללא טרנזקציית GUI וללא קודם או יורש; אינו מופיע בספרייה בגרסת 2023 " +
        "(S27OP).",
      edition: "on-premise",
      release: "2025.001",
      source: F8669_WHATS_NEW_2025_FPS01,
      recommendedAction:
        "לשקול את F8669 לבניית מבנים חדשים של אובייקטים טכניים כטיוטה לפני יצירתם במערכת (זמין מ-2025 FPS01, ללא " +
        "טרנזקציית GUI מקבילה בספרייה). היישום אינו מחליף את הרשומה המתוחזקת F2730A (Manage Technical Objects), שאינה " +
        "נתמכת במקור רשמי. תפקיד עסקי, קטלוג עסקי/טכני ושירות OData כעת מאומתים מהספרייה ועודכנו ב-data/fiori/apps.ts.",
    },
    xrefs: ["fiori:F2730A"],
    lastVerifiedAt: DATE24,
    notes:
      "נוסף 2026-09-23, הורחב 2026-09-24. שיטה ב-2026-09-23: scripts/sap-help-search.mjs בשתי שאילתות על " +
      "SAP_S4HANA_ON-PREMISE 2025.001 (המזהה עם השם, ושאילתה ממוקדת לפרטים הטכניים של רשומת ה-What's New). רכיב היישום " +
      "נקטע בקטע ה-What's New אחרי 'PM-EQM-RS'; הספרייה מדפיסה את רכיב ה-UI כ-PM-FIO-EQM-RS. אובייקט ההרשאה I_DRTOS " +
      "(Authorization for Draft Technical Object Structure) מתועד ב-What's New 2025 FPS01 (loio " +
      "0d8840e3e7384c1c9e73bbcb8ea8cd71) עם הפניה ל-Manage Technical Object Structures, אך הקטע אינו קובע שהוא נבדק " +
      "ביישום, ולכן אינו ברשומת הקטלוג. תוספת 2026-09-24: scripts/fal-app.mjs F8669 על S32OP (2025 FPS01, נמצא) ועל " +
      "S27OP (2023, Results ריק); ערוץ ה-OData של הספרייה נקרא ישירות, לא מעטפת ה-JavaScript. Old → New: לפני " +
      "2026-09-24 תפקיד עסקי, קטלוג ושירות OData לא אומתו; ב-2026-09-24 נקראו מהספרייה תפקיד SAP_BR_MD_SPECIALIST_EAM, " +
      "קטלוג עסקי SAP_ASM_BC_REFTO_MNG, קטלוג טכני SAP_TC_EAM_COMMON, קבוצת שירות V4 UI_DRFTTECHOBJSTRUCTURE_MANAGE " +
      "ו-Semantic Object/Action DraftTechnicalObjectStructure-manage. status.source: F8669_APP_TOPIC → " +
      "F8669_WHATS_NEW_2025_FPS01 (הרשומה שמדפיסה Type New ואת הגרסה); הסטטוס s4_native וה-release (2025.001) ללא " +
      "שינוי. הספרייה אינה מדפיסה טרנזקציית GUI מובילה או קשורה, ו-NumberofPredecessors=0, NumberofSuccessors=0, ולכן " +
      "לא נוסף xref. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל על data/fiori/apps.ts#F8669 (role, catalog, " +
      "odata, source, lastReviewed; guiTx נשאר ריק; type, trust ושם תאמו), ומשפט explain.technical שאמר שהתפקיד, " +
      "הקטלוג ושירות ה-OData לא נקראו מספריית ה-Fiori הוחלף בערכי הספרייה. לא אומתו: זמינות ב-Public Cloud (עבור " +
      "release זה הספרייה מדפיסה את S32OP, On-Premise, ואת S32PCE, Private Cloud; רשומת Public Cloud לא נבדקה). לא " +
  ...
  /* --------------------------------------------------------- fiori:F4587 */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Library: F4587 Manage Process Orders @ S27OP (S/4HANA 2023)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F4587')/S27OP",
        accessedAt: DATE24,
        claim:
          "רשומת הספרייה ל-F4587 בגרסת S27OP (2023): אותם תפקיד (SAP_BR_PRODN_SUPERVISOR_PROC), קטלוגים " +
          "(SAP_SCM_BC_PROC_ORD_MGMT / SAP_TC_SCM_PP_COMMON) ו-intent (ProcessOrder-manage) כמו ב-S32OP; OData: " +
          "PP_MPE_AOR 0001 ו-PP_PROCESS_ORDER_MANAGE_SRV 0001 (S4CORE 108), קבוצת V4: PP_MPE_AOR_SRV; backend: S4CORE " +
          "108 SP 0000; UI: UIS4HOP1 900 SP 0000. שדות טרנזקציית ה-GUI, הקודמים והיורשים מודפסים '-'. הערות RIN: " +
          "3336823 (Front-End Server), 3351047 (Back-End Server).",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      ...
      he:
        ...
        "ורשום בספריית יישומי ה-Fiori כ-Published בגרסאות S32OP (2025 FPS01) ו-S27OP (2023). התפקיד העסקי " +
        "SAP_BR_PRODN_SUPERVISOR_PROC, הקטלוג העסקי SAP_SCM_BC_PROC_ORD_MGMT ושירותי ה-OData PP_MPE_AOR " +
        "ו-PP_PROCESS_ORDER_MANAGE_SRV נקראו מהספרייה; שדות טרנזקציית ה-GUI, הקודם והיורש מודפסים בספרייה '-'.",
      edition: "on-premise",
      release: "2025.001",
      source: F4587_FAL_S32OP,
      recommendedAction:
        "להשתמש במזהה F4587 לכותרת Manage Process Orders, עם התפקיד SAP_BR_PRODN_SUPERVISOR_PROC, הקטלוג " +
        "SAP_SCM_BC_PROC_ORD_MGMT ושירותי ה-OData PP_MPE_AOR ו-PP_PROCESS_ORDER_MANAGE_SRV שהספרייה מדפיסה (הועתקו " +
        "ב-2026-09-24 ל-data/fiori/apps.ts#F4587); guiTx נשאר ריק כי שדות טרנזקציית ה-GUI בספרייה מודפסים '-'. המזהה " +
        "F3577 שברשומה המתוחזקת לא נמצא בחיפושי help.sap.com ולא בספריית ה-Fiori ב-S32OP וב-S27OP (ראו fiori:F3577).",
    },
    ...
    notes:
      ...
      " תוספת 2026-09-24: node scripts/fal-app.mjs F4587 (S32OP) ו---release S27OP החזירו רשומה בשתי הגרסאות; תפקיד, " +
      "קטלוגים, intent ושירותי OData זהים, רכיב backend S4CORE 109 מול S4CORE 108, מספרי RIN שונים. שדות טרנזקציית " +
      "ה-GUI, הקודם והיורש מודפסים '-' בשתי הגרסאות. רשימת הגרסאות של הרשומה כוללת גם S29PCE עד S32PCE (Private Cloud) " +
      "ו-S36=2602, S37=2608; לא נבדקה זמינות ב-Public Cloud דרך sap-help-search --product SAP_S4HANA_CLOUD. קודם: מקור " +
      "הסטטוס F4587_FEATURE_COMPARISON (2025.001), תפקיד/קטלוג/OData לא אומתו; עכשיו: מקור הסטטוס רשומת הספרייה S32OP " +
      "(F4587_FAL_S32OP, משותפת עם fiori:F3577), תפקיד/קטלוג/OData נקראו מהספרייה. תוספת 2026-09-24 (שלב הכתיבה): " +
      "לרשומה המבוקרת לא צורף catalogPatch, אך ההמלצה שלה נקבה בערכים; role, catalog ו-odata הועתקו " +
      "ל-data/fiori/apps.ts#F4587 (guiTx נשאר ריק; type, trust ושם תאמו), ומשפט explain.technical שאמר שהם לא נקראו " +
      "הוחלף בערכי הספרייה. recommendedAction נוסח מחדש בהתאם, והמשפט על F3577 תוחם לחיפושים שבוצעו (קודם: 'אינו מופיע " +
      "באף מקור רשמי'). לא בוצעה בדיקה במערכת SAP חיה.",
  },
  ...
  /* --------------------------------------------------------- fiori:F5323 */
  {
    ...
    evidence: [
      F4587_FEATURE_COMPARISON,
      {
        ...
        claim:
          ...
          "operations involved in a process order.' קטע הנושא אינו נוקב במזהה F5323 עצמו.",
        verificationLevel: "sap_official_verified",
      },
      {
        sourceType: "sap_help",
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Library: F5323 Manage Process Order Operations @ S27OP (S/4HANA 2023)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F5323')/S27OP",
        accessedAt: DATE24,
        claim:
          "רשומת הספרייה ל-F5323 בגרסת S27OP (2023): אותו שם, תפקיד, קטלוגים ושירותים (PP_MPE_AOR " +
          "ו-PP_PROCESS_ORDER_MANAGE_SRV, S4CORE 108, UI UIS4HOP1 900); הערות RIN 3336823 (Front-End Server) ו-3351047 " +
          "(Back-End Server).",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      status: "s4_native",
      he:
        "יישום Fiori (SAP Fiori elements, Transactional) של S/4HANA למעקב אחר הזמנות תהליך ברמת הפעולה, חדש ב-SAP " +
        "S/4HANA 2021 לפי What's New 2021 (רכיב PP-FIO-PI, פריט היקף BJ8). ספריית יישומי ה-Fiori מפרסמת אותו " +
        "(Published) עם רשימת גרסאות שמתחילה ב-S21OP=2021; הרשומות S27OP (2023) ו-S32OP (2025 FPS01) נקראו במלואן: " +
        "תפקיד עסקי SAP_BR_PRODN_SUPERVISOR_PROC, קטלוג עסקי SAP_SCM_BC_PROC_ORD_MGMT, שירותי OData PP_MPE_AOR " +
        "ו-PP_PROCESS_ORDER_MANAGE_SRV, ללא קודמים או יורשים וללא טרנזקציית GUI מובילה או קשורה מודפסת.",
      edition: "on-premise",
      release: "2025.001",
      source: F5323_FAL_S32OP,
      recommendedAction:
        "להשתמש ב-F5323 לצד Manage Process Orders (F4587) למעקב ברמת הפעולה; להקצות את התפקיד " +
        "SAP_BR_PRODN_SUPERVISOR_PROC ואת הקטלוג SAP_SCM_BC_PROC_ORD_MGMT, ולהפעיל את שירותי ה-OData PP_MPE_AOR " +
        "ו-PP_PROCESS_ORDER_MANAGE_SRV (קבוצת V4 PP_MPE_AOR_SRV). לבדוק את ההרשאות בפועל ב-Fiori Launchpad של המערכת.",
    },
    ...
    notes:
      ...
      " עדכון 2026-09-24: הרצת scripts/fal-app.mjs F5323 ב-S32OP וב-S27OP החזירה רשומה Published בשתיהן. קודם: המזהה " +
      "נשען על קריאה לפי סדר ערכים בטבלת ההשוואה; עכשיו: מאושר ישירות ברשומת הספרייה. נוספו תפקיד עסקי " +
      "SAP_BR_PRODN_SUPERVISOR_PROC, קטלוג עסקי SAP_SCM_BC_PROC_ORD_MGMT, קטלוג טכני SAP_TC_SCM_PP_COMMON, שירותי " +
      "OData PP_MPE_AOR ו-PP_PROCESS_ORDER_MANAGE_SRV (קבוצת V4 PP_MPE_AOR_SRV), ללא קודמים ויורשים. חיפוש חוזר של " +
      "Manage Process Order Operations ב-sap-help-search.mjs (21 רשומות) לא החזיר צימוד נוסף למזהה. הספרייה אינה " +
      "מדפיסה טרנזקציית GUI מובילה או קשורה, ולכן guiTx ריק. status.source: F4587_FEATURE_COMPARISON → " +
      "F5323_FAL_S32OP; הסטטוס s4_native וה-release (2025.001) ללא שינוי; שורת נושא היישום (loio " +
      "48d4f057b6a44fb59d3c3d85500a9f42) נקראה שוב בחיפוש ב-2026-09-24. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch " +
      "הוחל על data/fiori/apps.ts#F5323 (role, catalog, odata, source, lastReviewed; guiTx נשאר ריק; type, trust ושם " +
      "תאמו), ומשפט explain.technical שאמר שהתפקיד, הקטלוג ושירות ה-OData לא נקראו הוחלף בערכי הספרייה. לא אומתו: " +
      "זמינות ב-Public Cloud (רשימת הגרסאות כוללת S36=2602 ו-S37=2608 בקבוצת SC, אבל לא נקראה רשומת ענן נפרדת). לא " +
      "בוצעה בדיקה במערכת SAP חיה.",
  },
  ...
  /* --------------------------------------------------------- fiori:F2462 */
  {
    ...
    status: {
      status: "s4_native",
      he:
        "יישום Fiori לניהול אצוות; ספריית יישומי ה-Fiori מציגה Published על S32OP (2025 FPS01) ועל S27OP (2023): תפקיד " +
        "עסקי מוביל SAP_BR_INVENTORY_MANAGER, חמישה קטלוגים עסקיים (SAP_EHS_BC_MOC_CHANGE_MGMT, SAP_SCM_BC_BATCH_DSP, " +
        "SAP_SCM_BC_BATCH_MGMT, SAP_SCM_BC_BATCH_MNTR, SAP_SCM_BC_MPE_EXEC_MNTR), שירות OData LO_BM_BATCH_SRV, " +
        "וטרנזקציית GUI מובילה MSC1N עם המקושרות MSC2N ו-MSC3N. אין קודמים ואין יורשים רשומים בספרייה.",
      edition: "on-premise",
      release: "2025.001",
      source: F2462_FAL_S32OP,
      recommendedAction:
        "להשתמש במזהה F2462 לכותרת Manage Batches, עם התפקיד המוביל Inventory Manager, חמשת הקטלוגים העסקיים ושירות " +
        "ה-OData ‏LO_BM_BATCH_SRV. המזהה F1576 שברשומה המתוחזקת משויך בספריית ה-Fiori לאפליקציה אחרת (Supplier " +
        "Evaluation Response; ראו fiori:F1576), ואין להסתמך עליו לכותרת Manage Batches. לפני ההפעלה: לוודא הקצאת " +
        "התפקידים והקטלוגים למשתמשים, ולהפעיל ב-back-end את מחבר החיפוש המתאים לרמת האצווה (למשל BATCH_PLANT_H לרמת " +
        "מפעל).",
    },
    xrefs: ["fiori:F1576", "tx:MSC1N", "tx:MSC2N", "tx:MSC3N"],
    lastVerifiedAt: DATE24,
    notes:
      ...
      " עודכן 2026-09-24: הורץ node scripts/fal-app.mjs F2462 על S32OP (S/4HANA 2025 FPS01) ועל S27OP (S/4HANA 2023): " +
      "האפליקציה Published בשניהם, עם אותו תפקיד מוביל, אותם קטלוגים, אותו שירות OData ואותן טרנזקציות GUI. " +
      "status.source: F2462_APP_TOPIC (sap_help) → רשומת הספרייה S32OP (fiori_library, F2462_FAL_S32OP). xrefs חדשים: " +
      "tx:MSC1N (מובילה), tx:MSC2N ו-tx:MSC3N (קשורות). המשפט שהופיע ב-status.he עד 2026-09-24, שבחיפושי SAP ERP ו-SAP " +
      "Fiori for SAP Business Suite לא נמצאה רשומה בשם Manage Batches (21 רשומות בכל אחד), נשאר הממצא השלילי המתועד, " +
      "תחום לחיפוש. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל על data/fiori/apps.ts#F2462 (role, catalog, " +
      "guiTx, source, lastReviewed; odata, type, trust ושם תאמו); role קיבל את התפקיד המוביל SAP_BR_INVENTORY_MANAGER, " +
      "ו-18 התפקידים האחרים שברשומת הספרייה לא הועתקו; משפט explain.technical שאמר שהתפקיד והקטלוג לא נקראו מהספרייה " +
      "הוחלף. זמינות ב-Private Cloud (S32PCE) לא נקראה כשורה נפרדת; זמינות ב-Public Cloud לא נבדקה; גרסת ההצגה הראשונה " +
      "לא נקבעה מעבר ל-S9OP=1709 שברשימת ה-releases. לא בוצעה בדיקה במערכת SAP חיה.",
  },
  ...
  /* -------------------------------------------------------- fiori:F1511A */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Library: F1511A Create Maintenance Request @ S27OP (S/4HANA 2023)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F1511A')/S27OP",
        accessedAt: DATE24,
        claim:
          "רשומת הספרייה ל-F1511A בגרסת S27OP (S/4HANA 2023), סטטוס Published, קומפוננטה PM-FIO-WOC-MN: 'Create Maintenance " +
          "Request', Transactional / SAP Fiori (SAPUI5). ארבעה תפקידים עסקיים: SAP_BR_EMPLOYEE (R0056), " +
          "SAP_BR_MAINTENANCE_PLANNER (R0088), SAP_BR_MAINTENANCE_TECHNICIAN (R0090), SAP_BR_MAINT_SUPERVISOR (R0198); " +
          "SAP_BR_MD_SPECIALIST_EAM שבשורת S32OP אינו מודפס בשורה זו. קטלוג עסקי יחיד SAP_EAM_BC_MREQ_MNG 'EAM - Maintenance " +
          "Request'; קטלוג טכני SAP_TC_EAM_COMMON. Intent: MaintenanceWorkRequest-create. שירות OData: " +
          "UI_MAINTWORKREQUESTOVW_V2 גרסה 0001 (S4CORE 108); קבוצת V4: UI_PRIORITIZATION_PROFILE (S4CORE 108). טרנזקציית GUI " +
          "מובילה: IW21. קודמת (predecessor): F1511 Request Maintenance; אין successor רשום. Backend: S4CORE 108 SP0000 / SAP " +
          "S/4HANA 2023; UI: UIS4HOP1 900 SP0000. הערות RIN: 3336823 (Front-End Server), 3351047 (Back-End Server).",
        verificationLevel: "sap_official_verified",
      },
      F1511A_APP_TOPIC,
      {
    ...
    status: {
      status: "s4_native",
      he:
        "יישום Fiori של S/4HANA ליצירת בקשות תחזוקה, מאומת דרך ספריית ה-Fiori (S32OP = 2025 FPS01, Published, קומפוננטה " +
        "PM-FIO-WOC-MN) עם חמישה תפקידים עסקיים, שני קטלוגים עסקיים, שירות OData וטרנזקציית GUI מובילה IW21; ב-S27OP = 2023 " +
        "הספרייה מדפיסה את IW21 ואת שירות ה-OData על S4CORE 108, עם ארבעה תפקידים וקטלוג עסקי אחד. יישום נפרד מ-Request " +
        "Maintenance (F1511), שהיא הקודמת (predecessor) שלו לפי הספרייה.",
      edition: "on-premise",
      release: "2025.001",
      source: F1511A_FAL_S32OP,
      recommendedAction:
        "להשתמש במזהה F1511A לכותרת Create Maintenance Request, עם התפקידים, הקטלוגים, שירות ה-OData וטרנזקציית ה-GUI (IW21) " +
        "שהספרייה מדפיסה ל-S32OP; במערכת 2023 להקצות לפי שורת S27OP (ארבעה תפקידים, קטלוג עסקי SAP_EAM_BC_MREQ_MNG). הרשומה " +
        "המתוחזקת F1511 נשאה את הכותרת הזו עד 2026-09-24, אך לפי התיעוד F1511 הוא Request Maintenance (ראו fiori:F1511).",
    },
    xrefs: ["fiori:F1511", "fiori:F4072", "tx:IW21"],
    lastVerifiedAt: DATE24,
    notes:
      "נוסף 2026-09-23. שיטה: scripts/sap-help-search.mjs בארבע שאילתות על SAP_S4HANA_ON-PREMISE (המזהה עם השם בגרסה " +
      ...
      "New'. תוספת 2026-09-24: scripts/fal-app.mjs F1511A רץ על S32OP (2025 FPS01) ועל S27OP (2023); שתי השורות Published " +
      "ונוספו כשתי שורות fiori_library. זהה בשתי השורות: קומפוננטה PM-FIO-WOC-MN, טרנזקציית GUI מובילה IW21, קטלוג טכני " +
      "SAP_TC_EAM_COMMON, intent MaintenanceWorkRequest-create, שם שירות ה-OData UI_MAINTWORKREQUESTOVW_V2 0001 וקבוצת V4 " +
      "UI_PRIORITIZATION_PROFILE, הקודמת F1511 Request Maintenance, ואין successor רשום. שונה ב-S27OP: ארבעה תפקידים מתוך " +
      "החמישה (ללא SAP_BR_MD_SPECIALIST_EAM), קטלוג עסקי אחד SAP_EAM_BC_MREQ_MNG (ללא SAP_EAM_BC_TO_MNG), השירות על S4CORE " +
      "108 במקום 109, Backend S4CORE 108 SP0000 ו-UI UIS4HOP1 900 SP0000, והערות RIN 3336823 ו-3351047 במקום 3493254 " +
      "ו-3671888. רשימת ה-releases בספרייה מתחילה ב-S21OP=2021, בהתאם ל-releaseInfo. היסטוריה (Old → New): status.source: " +
      "F1511A_APP_TOPIC (sap_help) → רשומת הספרייה S32OP (fiori_library); הממצא מ-2026-09-23 'לא אומתו: תפקיד עסקי, קטלוג, " +
      "שירות OData' הוחלף (superseded) בערכי הספרייה; xrefs: fiori:F1511, fiori:F4072 → נוסף tx:IW21; status.he עודכן למונח " +
      "'תחזוקה'; ההנחיה ב-recommendedAction לקרוא מהספרייה את התפקיד, הקטלוג והשירות הוסרה לאחר שנקראו. במהדורת SAP S/4HANA " +
      "Cloud Public Edition (2608.500, loio 78732361f0b94fe1b1711632af4362b3) קטע הנושא של F1511A פותח ב-'Create " +
      "Maintenance Request - Old Version (F1511A) Manage Maintenance Notifications - New Version (F5777)'; זו עדות לענן " +
      "הציבורי בלבד: היא לא נבדקה כאן כראיה ל-On-Premise, ו-F5777 אינו קיים ביקום ה-xrefs. לא אומתו: זמינות ב-Public Cloud, " +
      "תוכן הערות ה-RIN (הוזכרו רק כמספרים שהספרייה מדפיסה). לא בוצעה בדיקה במערכת SAP חיה. תוספת 2026-09-24 (שלב הכתיבה): " +
      "ה-catalogPatch הוחל על data/fiori/apps.ts#F1511A: role (חמשת התפקידים), catalog (שני הקטלוגים העסקיים), odata " +
      "(UI_MAINTWORKREQUESTOVW_V2 וקבוצת ה-V4 UI_PRIORITIZATION_PROFILE), guiTx IW21, source ו-lastReviewed; type, trust " +
      "והשם תאמו. בשדות הקטלוג נרשמו המזהים עצמם, כמו בשאר הרשומות; מזהי ה-R, כותרות הקטלוגים וגרסאות S4CORE נשמרו בשורות " +
      "הספרייה ברשומה זו, והקטלוג הטכני SAP_TC_EAM_COMMON מופיע ב-explain.technical. לפי ה-verdict הוחלף משפט " +
      "explain.technical שאמר שהתפקיד, הקטלוג ושירות ה-OData לא נקראו מספריית ה-Fiori, והשם העברי ברשומת הקטלוג עבר למונח " +
      "'תחזוקה' ('יצירת בקשת תחזוקה').",
  },
  ...
  /* -------------------------------------------------------- fiori:F0251A */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Reference Library: Manage Material Coverage (Version 2) (F0251A), S/4HANA 2025 FPS01",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2025.001",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F0251A')/S32OP",
        accessedAt: DATE24,
        claim:
          "רשומת ספריית ה-Fiori עבור F0251A ב-S32OP (SAP S/4HANA 2025 FPS01), isPublished=Published, type Transactional / SAP " +
          "Fiori elements, component PP-FIO-MRP: roles SAP_BR_MATL_PLNR_EXT_PROC (R0100-21, Material Planner - External " +
          "Procurement) ו-SAP_BR_PRODN_PLNR (R0114, Production Planner); business catalog SAP_SCM_BC_MRPCOCKPIT ('Production " +
          "Planning - MRP Cockpit'); technical catalog SAP_TC_SCM_PP_COMMON; semantic intent MRPMaterial-manageSegment; OData " +
          "service PP_MRP_MANAGE_MATERIAL_SRV גרסה 0001 על S4CORE 109; GUI transaction מוביל MD04, ללא related transactions; " +
          "predecessor F0251 Manage Material Coverage, ללא successor רשום; RIN notes 3493254 (Front-End Server) ו-3671888 " +
          "(Back-End Server). אותם roles, catalogs, OData service (על S4CORE 108) ו-predecessor חוזרים גם ב-S27OP (SAP S/4HANA " +
          "2023), עם RIN notes 3336823/3351047.",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      ...
      he:
        ...
        "Material Coverage (F0251), מתועד ב-PP-MRP לגרסת 2025 FPS01 תחת App ID F0251A. לפי אותו תיעוד הוא עדיין אינו מכסה את " +
        "כל ההיבטים של F0251 (ראו fiori:F0251). ספריית ה-Fiori מציגה את היישום כ-Published ב-S32OP (2025 FPS01) וב-S27OP " +
        "(2023), עם תפקידים SAP_BR_MATL_PLNR_EXT_PROC ו-SAP_BR_PRODN_PLNR, קטלוג SAP_SCM_BC_MRPCOCKPIT, שירות OData " +
        "PP_MRP_MANAGE_MATERIAL_SRV וטרנזקציית GUI מובילה MD04.",
      edition: "on-premise",
      release: "2025.001",
      source: F0251A_APP_TOPIC,
      recommendedAction:
        "לבחון את F0251A לפריסות חדשות של MRP Cockpit רק לאחר בדיקה, לפי טבלת ההשוואה הרשמית, שהתכונות הנדרשות מכוסות בו. לפי " +
        "ספריית ה-Fiori (evidence[3]) ועמוד התיעוד לגרסת 2025 FPS01, התפקידים הם SAP_BR_MATL_PLNR_EXT_PROC " +
        "ו-SAP_BR_PRODN_PLNR והקטלוג הוא SAP_SCM_BC_MRPCOCKPIT; רשומת What's New 2023 מציינת את הקטלוג " +
        "SAP_SCM_BC_MRPCOCKPIT_MC, ולכן יש לוודא במערכת את שם הקטלוג לפני הקצאה.",
    },
    ...
    notes:
      ...
      "שממנו ניתן לנווט ל-F0251 או ל-F0251A; ה-xref ל-MD04 נשען על טבלת ההשוואה. כתובת What's New 2023 נלקחה כלשונה מרשומת " +
      "החיפוש (deliverable f5d3e1005efd4e86acf9a65abf428082). עדכון 2026-09-24: node scripts/fal-app.mjs F0251A (S32OP) " +
      "ו---release S27OP; הספרייה מחזירה רשומה Published בשתי הגרסאות עם אותם תפקידים, קטלוגים ושירות OData (S4CORE 109 " +
      "ב-S32OP, S4CORE 108 ב-S27OP), predecessor F0251, ללא successor. אי-התאמות בין מקורות רשמיים: (1) שם היישום בספרייה " +
      "הוא 'Manage Material Coverage (Version 2)' לעומת 'Manage Material Coverage' בעמוד התיעוד ובקטלוג המתוחזק; השם בקטלוג " +
      "לא שונה, זו החלטת מוצר. (2) גוף רשומת What's New 2023 מציין את הקטלוג העסקי SAP_SCM_BC_MRPCOCKPIT_MC, בעוד הספרייה " +
      "(S32OP ו-S27OP) ועמוד היישום לגרסת 2025 FPS01 מציינים SAP_SCM_BC_MRPCOCKPIT; מה שיכריע: בדיקת הקטלוג ב-launchpad " +
      "content manager במערכת 2023. הספרייה מציגה גם גרסאות S36/S37 (Public Cloud), זמינות Public Cloud לא נבדקה מעבר לכך. " +
      "לא בוצעה בדיקה במערכת SAP חיה. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל על data/fiori/apps.ts#F0251A " +
      "(role, catalog, odata, guiTx MD04, source, lastReviewed; type ו-trust תאמו), והשם בקטלוג נשאר 'Manage Material " +
      "Coverage', לפי ה-verdict. משפט explain.technical שאמר שהתפקיד, הקטלוג ושירות ה-OData לא נקראו מספריית ה-Fiori הוחלף " +
      "בערכי הספרייה ובאי-ההתאמה מול SAP_SCM_BC_MRPCOCKPIT_MC; מחרוזת ה-source בקטלוג שומרת את מקורות ה-SAP Help הקודמים " +
      "ומוסיפה את הספרייה.",
  },
  ...
  /* --------------------------------------------------------- fiori:F5460 */
  {
    ...
    evidence: [
      ...
      {
        sourceType: "fiori_library",
        sourceTitle: "Advanced Scheduling Board - SAP Fiori Apps Library, structured record (F5460, S27OP)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2023.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('F5460')/S27OP",
        accessedAt: DATE24,
        claim:
          "רשומת הספרייה ל-F5460 על S27OP (SAP S/4HANA 2023): Published, עם אותם תפקידים, קטלוגים עסקיים, קטלוג טכני, Semantic " +
          "Object/Action ושירות OData (UI_SCM_ADVNCD_SCHEDG_BOARD 0001, S4COREOP 108), וטרנזקציית GUI מובילה /SAPAPO/CDPS0. " +
          "backend S4COREOP 108 - SP 0000, UI UIS4HOP1 900 - SP 0000. הערות RIN: 3210319 ו-3353782 (BOM, General Note), 3336823 " +
          "(RIN, Front-End Server), 3351047 (RIN, Back-End Server). רשימת השחרורים שהספרייה מדפיסה נעה מ-S22OP (2021 FPS01) עד " +
          "S32PCE (2025 FPS01 Private Cloud). אין קודמים ואין יורשים.",
        verificationLevel: "sap_official_verified",
      },
    ...
    status: {
      status: "s4_native",
      he:
        "יישום Fiori טרנזקציוני של PP/DS ב-S/4HANA. לפי ספריית היישומים (S32OP, SAP S/4HANA 2025 FPS01) F5460 מפורסם " +
        "(Published) עם התפקיד המוביל SAP_BR_PRODN_PLNR ועם SAP_BR_PRODN_PLNR_APS, הקטלוגים העסקיים SAP_SCM_BC_APS_PROD_PLNG " +
        "ו-SAP_SCM_BC_CAPA_PLAN, שירות OData UI_SCM_ADVNCD_SCHEDG_BOARD 0001 וטרנזקציית ה-GUI המובילה /SAPAPO/CDPS0, בלי " +
        "קודמים ובלי יורשים. רשימת השחרורים בספרייה מתחילה ב-S22OP (2021 FPS01), בעוד What's New 2022 מסווג את היישום App " +
        "New; הפער בין שני המקורות לא הוכרע.",
      edition: "on-premise",
      release: "2025.001",
      source: F5460_FAL_S32OP,
      recommendedAction:
        "הערכים שהספרייה מדפיסה הוחלו ב-2026-09-24 על data/fiori/apps.ts#F5460 (guiTx: /SAPAPO/CDPS0; catalog: " +
        "SAP_SCM_BC_APS_PROD_PLNG, SAP_SCM_BC_CAPA_PLAN; odata: UI_SCM_ADVNCD_SCHEDG_BOARD; role: SAP_BR_PRODN_PLNR, " +
        "SAP_BR_PRODN_PLNR_APS). להעריך את F5460 מול Production Scheduling Board (F2176) לפי היקף ה-PP/DS המופעל במערכת; לא " +
        "נמצאה רשומה רשמית הקובעת שאחד מהם מחליף את השני.",
    },
    xrefs: ["fiori:F2176"],
    lastVerifiedAt: DATE24,
    notes:
      ...
      "Cloud. לא בוצעה בדיקה במערכת SAP חיה. עדכון 2026-09-24: node scripts/fal-app.mjs F5460 הורץ על S32OP ועל S27OP " +
      "והחזיר רשומה מפורסמת עם role/catalog/odata/guiTx זהים בשני השחרורים; משתנים רכיבי התוכנה (S4COREOP 108 / UIS4HOP1 " +
      "900 מול S4COREOP 109 / UIS4H 109) ומספרי ה-RIN. מקור ה-status הוחלף: What's New 2025 FPS01 (loio " +
      "65074cb6e3be4f958e16d5c14d83e2a8) -> רשומת הספרייה S32OP; שורת What's New נשמרת כעדות. guiTx בקטלוג הפרויקט היה ריק " +
      "ומעודכן ל-['/SAPAPO/CDPS0'] דרך catalogPatch. הספרייה מקשרת את Scope Items 7UV ('Make-to-Stock Production – Discrete " +
      "Manufacturing') ו-7V9 ('Make-to-Order Production – Discrete Manufacturing'), שלא נכנסו כ-xref כי אין להם canonical " +
      "id. לא בוצעה בדיקה במערכת SAP חיה; זמינות Public Cloud ודרישות liveCache לא אומתו. תוספת 2026-09-24 (שלב הכתיבה): " +
      "הממצא מ-2026-09-23 'לא אומתו: תפקיד עסקי, קטלוג, שירות OData' הוחלף בערכי הספרייה; דרישות liveCache וזמינות ב-Public " +
      "Cloud נשארו לא מאומתות. ה-catalogPatch הוחל על data/fiori/apps.ts#F5460 (role, catalog, odata, guiTx, source, " +
      "lastReviewed; type, trust והשם תאמו); לפי ה-verdict הוחלף משפט explain.technical שאמר שהתפקיד, הקטלוג ושירות ה-OData " +
      "לא נקראו, ו-releaseInfo מציין גם את רשימת השחרורים של הספרייה (מ-S22OP). recommendedAction: ההנחיה לתחזק את " +
      "data/fiori/apps.ts#F5460 לפי ערכי הספרייה נוסחה כפעולה שבוצעה, והמשפט על היחס ל-F2176 תוחם לחיפושים ('לא נמצאה רשומה " +
      "רשמית' במקום 'אין רשומה רשמית', כמו בנוסח 2026-09-23).",
  },
    ...
    evidence: [
      {
        ...
        accessedAt: DATE24,
        claim:
          "גוף הנושא של רשומת What's New לגרסת SAP S/4HANA 2022 (loio af315b2ddb3e488eb3999f4ae144f0ed), שנקרא ב-2026-09-24 דרך " +
          "scripts/sap-help-body.mjs (deliverable 27597741): 'The Confirm Jobs app (W0020) is deprecated and will be deleted " +
          "from the SAP Fiori launchpad in an upcoming release.'; 'Valid as Of SAP S/4HANA 2022 Additional Details The " +
          "following successor apps are available on the SAP Fiori Launchpad: Perform Maintenance Jobs (F5104A) Report and " +
          "Repair Malfunction (F2023)'; בשורת הסיכום: 'See More App Deprecated BH1 BJ2 PM SAP S/4HANA 2022'.",
        verificationLevel: "sap_official_verified",
      },
      W0020_DELETION_2023,
      {
        sourceType: "fiori_library",
        sourceTitle: "SAP Fiori Apps Reference Library: Confirm Jobs (W0020), S/4HANA 2022 (S24OP)",
        product: "SAP S/4HANA",
        edition: "on-premise",
        release: "2022.000",
        url: "https://fioriappslibrary.hana.ondemand.com/sap/fix/externalViewer/index.html#/detail/Apps('W0020')/S24OP",
        accessedAt: DATE24,
        claim:
          "רשומת הספרייה ל-W0020 על S24OP (S/4HANA 2022), שנקראה דרך scripts/fal-app.mjs W0020 --release S24OP: 'Confirm Jobs | " +
          "Web Dynpro / Web Dynpro | Deprecated | component EP-PCT-MAN-MW (BP for Maintenance Worker)'; תפקיד " +
          "SAP_BR_MAINTENANCE_TECHNICIAN (R0090, Maintenance Technician); קטלוג עסקי SAP_EAM_BC_JOB_MW 'EAM - Job MW'; קטלוג " +
          "טכני SAP_TC_EAM_BE_APPS:S4EAM; intent MaintenanceJobConfirmation-create; שדה OData מודפס '-'; טרנזקציות GUI: מובילה " +
          "IW25, קשורות IW41, IW42, IW44; backend 'S4CORE 107 - SP 0000 / SAP S/4HANA 2022'; רשימת הגרסאות מ-S3OP=1511 עד " +
          "S25OP=2022 FPS01; successors: 'F5104A Perform Maintenance Jobs'. אותו סקריפט החזיר Results ריק ל-W0020 ב-S32OP (2025 " +
          "FPS01) וב-S27OP (2023), בהתאמה למחיקה ב-SAP S/4HANA 2023.",
        verificationLevel: "sap_official_verified",
      },
    ],
    status: {
      status: "not_available",
      he:
        "אפליקציית Confirm Jobs (W0020) הוצאה משימוש ב-SAP S/4HANA 2022 ונמחקה מה-SAP Fiori launchpad ב-SAP S/4HANA 2023, לפי " +
        "רשומות What's New של SAP, שמונות כיישומים יורשים את Perform Maintenance Jobs (F5104A) ואת Report and Repair " +
        "Malfunction (F2023). ספריית ה-Fiori מונה את W0020 עד S25OP (2022 FPS01), והבדיקה ב-scripts/fal-app.mjs לא החזירה " +
        "רשומה ל-W0020 בגרסאות S32OP (2025 FPS01) ו-S27OP (2023).",
      edition: "on-premise",
      release: "2023.000",
      source: W0020_DELETION_2023,
      successor: "fiori:F5104A",
      recommendedAction:
        "לא לתכנן תרחישים חדשים על W0020. לביצוע ולדיווח של עבודות שהוקצו לטכנאי התחזוקה להפנות ל-Perform Maintenance Jobs " +
        "(F5104A) או ל-Report and Repair Malfunction (F2023). הרשומה המתוחזקת F2730 בשם Confirm Jobs אינה נתמכת במקור רשמי " +
        "(ראו fiori:F2730).",
    },
    xrefs: ["fiori:F2730", "fiori:F5104A", "fiori:F2023", "tx:IW25", "tx:IW41", "tx:IW42", "tx:IW44"],
    lastVerifiedAt: DATE24,
    notes:
      "נוסף 2026-09-23 כרשומה עצמאית למזהה הרשמי של Confirm Jobs; עד תאריך זה W0020 היה alias של fiori:F2730, והכינוי הוסר " +
      "משם. שיטה (2026-09-23): scripts/sap-help-search.mjs בחמש שאילתות על SAP_S4HANA_ON-PREMISE, נעוצות לגרסאות 2022.000 " +
      "ו-2023.000. שירות החיפוש החזיר אז לאותו loio של 2022 שני מזהי deliverable בכתובת (e296651f454c4284ade361292c633d69 " +
      "ו-f5d3e1005efd4e86acf9a65abf428082); הכתובת כאן זהה לכתובת שברשומת fiori:F2730; גוף הנושא נקרא דרכה (deliverable " +
      "27597741). היורש ברשומה הוא F5104A, היישום הראשון שרשומת המחיקה מונה; F2023 מופיע ב-xrefs. 2026-09-24: גוף שני נושאי " +
      "ה-What's New נקרא דרך scripts/sap-help-body.mjs (2022: deliverable 27597741; 2023: deliverable 32474459); גוף 2022 " +
      "מונה גם את Report and Repair Malfunction (F2023) כיורש, ולכן שורת 2022 נחתמה מחדש ב-2026-09-24 על בסיס הגוף. Old → " +
      "New: שורת 2022 נשענה על סניפט מ-2026-09-23 ומנתה את F5104A בלבד כיורש; כעת היא מצטטת את הגוף עם F5104A ו-F2023. שורת " +
      "2023 (W0020_DELETION_2023) נשארה ללא שינוי. נוספה שורת fiori_library מ-scripts/fal-app.mjs W0020 --release S24OP, " +
      "שמחליפה את הקביעה הקודמת שתפקיד עסקי וקטלוג לא אומתו: הספרייה מדפיסה ב-S24OP תפקיד SAP_BR_MAINTENANCE_TECHNICIAN, " +
      "קטלוג עסקי SAP_EAM_BC_JOB_MW, קטלוג טכני SAP_TC_EAM_BE_APPS:S4EAM, טרנזקציה מובילה IW25 וקשורות IW41/IW42/IW44, " +
      "ורשימת גרסאות שמתחילה ב-S3OP (1511); שדה ה-OData מודפס '-'. ‏S32OP ו-S27OP החזירו Results ריק, תוצאה מתועדת של אותן " +
      "שתי גרסאות ולא קביעה על גרסאות אחרות. בדיקת המבקר ב-2026-09-24 (scripts/fal-app.mjs W0020 --release S30OP / S31OP) " +
      "החזירה Results ריק גם בשתי הגרסאות האלה, תוצאה מתועדת ולא קביעה על היעדר. פער מול הקטלוג: הספרייה מדפיסה ב-S24OP סוג " +
      "יישום וטכנולוגיית UI 'Web Dynpro', ואילו data/fiori/apps.ts#W0020 רושם type 'Transactional' שנגזר מהפונקציה (אישור " +
      "עבודות); הקטלוג לא שונה כאן, והפער מסומן לטיפול. עמוד ספריית ה-Fiori ‏Apps('W0020')/S11OP המוזכר ברשומת fiori:F2730 " +
      "לא נפתח ואינו ראיה כאן. לא אומתו: שירות OData מעבר לערך '-' שהספרייה מדפיסה, וזמינות ב-SAP ERP ובענן הציבורי. לא " +
      "בוצעה בדיקה במערכת SAP חיה. תוספת 2026-09-24 (שלב הכתיבה): ה-catalogPatch הוחל על data/fiori/apps.ts#W0020 (role, " +
      "catalog, guiTx IW25/IW41/IW42/IW44, source, lastReviewed; trust והשם תאמו); odata נשאר ריק כי הספרייה מדפיסה '-', " +
      "ו-type נשאר Transactional, כי ערכי type בסכמת הקטלוג אינם כוללים Web Dynpro. משפט explain.technical שאמר שהתפקיד, " +
      "הקטלוג ושירות ה-OData לא נקראו הוחלף בערכי שורת S24OP.",
  },
