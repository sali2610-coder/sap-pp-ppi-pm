...
- 2026-09-24 22:05 → 2026-09-25 00:31 · batches committed, each re-verified in a clean worktree (tsc 0, tests 211/211): Fiori depth 5-6 (`fc0413f8`, `ffcf23a6`; **Fiori depth chain complete**, 34 apps, L5 24); objects 4 + master recipe repair (`fbd1e277`, `bc4ee690`; **objects family complete**, 16 records, L5 13); best practices catalog batch 4 + three re-drafts (`bb87eaab`, `efc0724c`; **catalog queue complete**, 35 records); CDS re-checks misc 5-8 (`98363370`, `9dc422ab`, `4e420aec`, `2e561357`; **CDS queue complete**, 39/39, L5 23); chain A 13-15 (`ea5387c3`, `a9373cf4`, `21b99735`; MD50 queued for a re-draft); chain B 1-3 (`fc06f0e7`, `3d0fd5f4`, `642ce620`; MCXA queued). SAP fixes: "ללא שינוי" no longer derives a change (`7ac302e4`); CJ01/CJ02/CJ06/CN21/CN22 are perpetual-scope, not replaced by CJ20N (`da45a535`).
- 2026-09-25 00:31 · chain B split in two at a batch boundary so the 241 remaining named codes run side by side, one writer per file: shard C registered (`df0f4c31`); chain B `wf_72553a2c-fd2` (15 batches, 121 codes incl. the MCXA re-draft, transactions-b.ts) and chain C `wf_7ec92edc-a67` (15 batches, 120 codes, transactions-c.ts). Still running: chain A `wf_5cdb3d4b-349`, misc (enhancements) `wf_6efaf4b6-3f6`. Coverage at `642ce620`: transactions verified 582 / L5 83, functions 126 / L5 41, tables 103 / L5 28, cds 38 / L5 23, fiori 26 / L5 24, enhancements 28 / L5 15, objects 16 / L5 13, best practices 35 records.
- 2026-09-25 00:35-01:30 · SAP fact corrections from the chains' recorded repository conflicts and a mechanical sweep of every Fiori id in `data/**` against the Fiori Apps Library (`fal-app.mjs`, S32OP): FIX-17 MFP1 (planned handling units, not a product cost collector); FIX-18 seven MB codes + MB31 "available" vs replaced, MMBE_OLD rebuilt (it carried MM02), eight catalog titles; FIX-19 phantom / misnamed Fiori ids (F2731 → F5241, F2773, F2974/F2929, F1827, F3364, F0247, F0843A, F1600 → F1600A, F2424, F1077 → F1076) and Confirm Jobs (deleted in 2023) replaced by Perform Maintenance Jobs (F5104A) wherever it read as current; FIX-20 MD01 still available (not replaced), Post Goods Movement = MIGO Web GUI app (not F0843), center cards no longer claim replacement; FIX-21 CO01/CO11N/CO03/C201; FIX-22 MM03; FIX-23 F2018 in the PM lesson. Shards D and E registered (`dfac5c25`) for a four-way split of the named-code queue. A repair agent put a personal address in a `reviewer` field (refused at the writer); the draft schema no longer asks for the field (`fa0fb185`).
- 2026-09-25 01:30 · best-practice completeness audit (`scratchpad/fix/bp-fields.mts`): of 33 process records, 30 miss at least one of the 21 fields (interfaces on the 20 records that predate the section, KPIs on 21, the official reference on 5, migration on 2, roles on 1). Backfill run launched: `wf_70be0419-214` (bp-family per file, replace in place, content preserved).
