...
| Web research | `https://help.sap.com/http.svc/deliverableMetadata` then `https://help.sap.com/http.svc/pagecontent` (the JSON services the Help Portal's own topic page loads; same product / deliverable / loio / `version` as the topic URL) | full topic body on the official host, adopted with functions batch 13 (2026-09-23); a record may say a body was "read in full" only when every quote it takes from the body appears in the returned text, and the auditor re-fetches it; version-bound like the search record |
