...
// transactions and idocs catalogs on 2026-09-02 (their per-catalog data
// commits carry sap_official_verified claims and authored statuses, all
// checked by validateRecords above). The remaining four catalogs are still
// foundation-state and stay under the strict repository-only guard.
const FOUNDATION_RECORDS = [
  ...CDS_VERIFICATION, ...FIORI_VERIFICATION, ...ENH_VERIFICATION, ...OBJECT_VERIFICATION,
];

test("foundation catalogs are repository-verified only (tables + functions + transactions + idocs graduated)", () => {
...
test("graduated records (tables + functions + transactions + idocs): every repository claim still carries a repoRef", () => {
  for (const r of [...TABLE_VERIFICATION, ...FM_VERIFICATION, ...TX_VERIFICATION, ...IDOC_VERIFICATION]) {
    for (const e of r.evidence) {
