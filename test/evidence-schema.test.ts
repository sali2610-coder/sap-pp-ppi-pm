...
const DATA_FILES = [
  "data/verification/tables.ts", "data/verification/transactions.ts", "data/verification/transactions-b.ts", "data/verification/transactions-auto.ts", "data/verification/functions.ts",
...
test("data/verification/index.ts imports exactly the ten files this suite loads", () => {
  const names = [...read("data/verification/index.ts").matchAll(/from\s+"\.\/([a-z-]+)"/g)]
    .map((m) => m[1]).sort();
  assert.deepEqual(names, ["cds", "enhancements", "fiori", "functions", "idocs", "objects", "tables", "transactions", "transactions-auto", "transactions-b"]);
});
