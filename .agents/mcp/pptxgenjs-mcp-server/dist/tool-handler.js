export async function handleToolCall(request, manager) {
...
function handleCreatePresentation(args, manager) {
    ...
    if (args.subject)
        pres.subject = args.subject;
    if (args.company)
        pres.company = args.company;
    return {
        content: [
            {
                type: "text",
...
function handleAddSlide(args, manager) {
...
function handleAddText(args, manager) {
    const pres = manager.getPresentation(args.presentationId);
...
function handleAddShape(args, manager) {
    const pres = manager.getPresentation(args.presentationId);
    const slides = pres.slides;
    if (!slides || args.slideNumber < 1 || args.slideNumber > slides.length) {
        throw new Error(`Invalid slide number: ${args.slideNumber}`);
    ...
    if (args.fill)
        options.fill = { color: args.fill };
    if (args.line)
        options.line = { color: args.line };
    if (args.lineSize)
        options.line = { ...options.line, width: args.lineSize };
...
function handleAddImage(args, manager) {
    const pres = manager.getPresentation(args.presentationId);
    const slides = pres.slides;
    if (!slides || args.slideNumber < 1 || args.slideNumber > slides.length) {
        throw new Error(`Invalid slide number: ${args.slideNumber}`);
...
function handleAddTable(args, manager) {
    const pres = manager.getPresentation(args.presentationId);
    const slides = pres.slides;
    if (!slides || args.slideNumber < 1 || args.slideNumber > slides.length) {
        throw new Error(`Invalid slide number: ${args.slideNumber}`);
...
function handleAddChart(args, manager) {
    const pres = manager.getPresentation(args.presentationId);
    const slides = pres.slides;
    if (!slides || args.slideNumber < 1 || args.slideNumber > slides.length) {
        throw new Error(`Invalid slide number: ${args.slideNumber}`);
...
async function handleSavePresentation(args, manager) {
    const filePath = await manager.savePresentation(args.presentationId, args.outputPath);
    return {
        content: [
            {
                type: "text",
...
function handleListPresentations(manager) {
...
function handleDeletePresentation(args, manager) {
