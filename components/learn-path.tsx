...
function Block({ icon, label, color, children, dir: d }: { icon: React.ReactNode; label: string; color: string; children: React.ReactNode; dir?: string }) {
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" style={{ borderInlineStartColor: color, borderInlineStartWidth: 4 }} dir={d}>
      ...
      <div className="px-4 pb-4 pt-2 text-[14px] leading-relaxed text-slate-700">{children}</div>
    </section>
  );
}
  ...
  return (
    <article className="space-y-4" style={{ animation: "fadeUp .25s ease both" }}>
      ...
      <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        ...
        <div className="mt-1.5 flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-2xl font-extrabold leading-tight text-slate-900 sm:text-3xl">{s.titleHe}</h1>
          ...
          <button onClick={onToggle} className={`tap inline-flex shrink-0 items-center gap-1.5 rounded-xl px-4 py-2.5 text-sm font-bold shadow-sm transition active:scale-95 ${isDone ? "text-white" : "border border-slate-200 bg-white text-slate-600 hover:border-brand/40 hover:text-brand"}`} style={isDone ? { background: accent } : undefined}>
      ...
      {related.length > 0 && (
        <Block icon={<Network className="size-3.5" />} label="אובייקטים קשורים · לחיצה לפתיחה" color="#2563eb" dir="rtl">
          <div className="flex flex-wrap gap-1.5" dir="ltr">
            {related.map((r) => <Link key={r} href={`/object/${encodeURIComponent(r)}/`} className="tech rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 font-mono text-[12px] font-bold text-slate-700 transition hover:border-brand/50 hover:bg-white hover:text-brand">{r}</Link>)}
          </div>
        </Block>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        {s.object && <Link href={`/object/${encodeURIComponent(s.object)}/`} className="tap inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:border-brand/40 hover:text-brand"><BookOpen className="size-4" />דף הידע המלא של {s.object}<ArrowLeft className="size-3.5" /></Link>}
      ...
      {/* lesson flow nav */}
      <nav className="flex items-center justify-between gap-2 border-t border-slate-200 pt-4">
        <button onClick={onPrev} disabled={!hasPrev} className="tap inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-600 transition enabled:hover:border-brand/40 enabled:hover:text-brand disabled:opacity-40"><ArrowRight className="size-4" />היחידה הקודמת</button>
        {hasNext
...
export function LearnPathView({ module }: { module: string }) {
  ...
  const CourseMap = (
    <div className="space-y-1">
      <Link href="/learn/" className="mb-2 inline-flex items-center gap-1.5 text-[13px] font-bold text-slate-500 transition hover:text-brand"><ArrowRight className="size-4" />כל המסלולים</Link>
      <div className="px-1 pb-1 text-[11px] font-bold uppercase tracking-wide text-slate-400">תוכן הקורס · {st.total} יחידות</div>
      {p.steps.map((s, i) => {
        const d = done.has(s.id); const on = i === sel; const here = i === st.current && !st.complete;
        return (
          <button key={s.id} onClick={() => go(i)} className={`flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-right transition ${on ? "bg-white shadow-sm ring-1 ring-slate-200" : "hover:bg-white/70"}`}>
            <span className={`grid size-7 shrink-0 place-items-center rounded-full border-2 text-[12px] font-extrabold transition ${d ? "text-white" : "text-slate-400"}`} style={{ background: d ? accent : "#fff", borderColor: d ? accent : "#cbd5e1" }}>{d ? <Check className="size-3.5" /> : s.n}</span>
            <span className="min-w-0 flex-1">
              <span className={`block truncate text-[13px] font-bold ${on ? "text-slate-900" : "text-slate-600"}`}>{s.titleHe}</span>
              {s.object && <span className="tech block font-mono text-[10px] text-slate-400" dir="ltr">{s.object}</span>}
            </span>
  ...
  const Stat = ({ icon, label, value, c }: { icon: React.ReactNode; label: string; value: React.ReactNode; c?: string }) => (
    <div className="flex items-center gap-2.5 rounded-xl border border-slate-100 bg-white px-3 py-2">
      <span className="grid size-8 shrink-0 place-items-center rounded-lg text-white" style={{ background: c || accent }}>{icon}</span>
      <div className="min-w-0"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-400">{label}</div><div className="text-sm font-extrabold text-slate-900">{value}</div></div>
    </div>
  );
  const Dashboard = (
    <div className="space-y-2.5">
      <div className="rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm">
        <div className="text-[11px] font-bold uppercase tracking-wide text-slate-400">השלמת הקורס</div>
        <div className="my-1 text-4xl font-extrabold tabular-nums" style={{ color: accent }}>{st.pct}%</div>
        <div className="h-2 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full transition-all duration-500" style={{ width: `${st.pct}%`, background: accent }} /></div>
      ...
      {storyHref && <Link href={storyHref} className="tap flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-bold text-slate-600 transition hover:border-brand/40 hover:text-brand"><Workflow className="size-4" />סיור מודרך בתהליך</Link>}
      {st.doneCount > 0 && <button onClick={() => resetPath(module)} className="flex w-full items-center justify-center gap-1.5 py-1 text-xs font-bold text-slate-400 transition hover:text-brand"><RotateCcw className="size-3.5" />אפס התקדמות</button>}
    </div>
  );
    ...
    <div className="mx-auto max-w-[1700px]" dir="rtl">
      ...
      <header className="relative overflow-hidden rounded-3xl border p-6 shadow-sm sm:p-8" style={{ borderColor: accent + "33", background: `linear-gradient(135deg, ${accent}14, #fff 60%)` }}>
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400"><GraduationCap className="size-4" style={{ color: accent }} />NEO Academy</div>
            <h1 className="mt-1 text-3xl font-extrabold text-slate-900 sm:text-4xl">{p.he}</h1>
            <p className="mt-1 max-w-xl text-sm text-slate-500">{p.sub}</p>
            <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[13px] font-bold text-slate-600">
              <span className="flex items-center gap-1.5"><Layers className="size-4 text-slate-400" />{st.total} יחידות לימוד</span>
              <span className="flex items-center gap-1.5"><Boxes className="size-4 text-slate-400" />{objCount} אובייקטים</span>
              <span className="flex items-center gap-1.5"><Clock className="size-4 text-slate-400" />{p.durationHe}</span>
            ...
            <div className="mt-2 flex items-center gap-1.5 text-[12px] text-slate-500"><Target className="size-3.5" /><span className="font-bold">מתאים ל:</span> {audienceOf(module)}</div>
          </div>
      ...
      <div className="mt-3 flex items-center gap-2 lg:hidden">
        <button onClick={() => setMapOpen(true)} className="tap inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-600"><Menu className="size-4" />תוכן הקורס</button>
        <div className="flex flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full" style={{ width: `${st.pct}%`, background: accent }} /></div>
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-[252px_minmax(0,1fr)_288px]">
        <aside className="hidden lg:block"><div className="sticky top-4 max-h-[calc(100vh-2rem)] overflow-y-auto rounded-2xl border border-slate-200 bg-slate-50/60 p-2.5">{CourseMap}</div></aside>
      ...
      {mapOpen && (
        <div className="fixed inset-0 z-[60] lg:hidden" dir="rtl">
          ...
          <div className="absolute inset-y-0 end-0 flex w-[86%] max-w-sm flex-col bg-white shadow-2xl" style={{ animation: "drawerIn .3s cubic-bezier(.32,.72,0,1) both" }}>
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <span className="font-extrabold text-slate-900">תוכן הקורס</span>
              <button onClick={() => setMapOpen(false)} className="tap grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-slate-100"><X className="size-5" /></button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto p-3">{CourseMap}</div>
            <div className="border-t border-slate-100 p-3">{Dashboard}</div>
          </div>
        </div>
      )}
    </div>
