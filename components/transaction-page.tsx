...
function Sec({ id, title, icon, children, sub }: { id: string; title: string; icon: React.ReactNode; children: React.ReactNode; sub?: string }) {
  return (
    <section id={id} className="scroll-mt-20 rounded-2xl border border-hairline bg-surface p-5 shadow-sm">
      <div className="mb-3"><h2 className="flex items-center gap-2 text-sm font-extrabold text-ink-1">{icon}{title}</h2>{sub && <p className="mt-0.5 text-[11px] font-medium text-ink-3">{sub}</p>}</div>
      {children}
    </section>
  );
}
...
export function TransactionPage({ code }: { code: string }) {
  ...
  if (!t) return <div className="py-20 text-center text-sm text-ink-3" dir="rtl">T-Code לא נמצא במאגר.</div>;
  const c = mc(t.module);
  ...
  const CodeChip = ({ cd }: { cd: string }) => { const ex = txExists(cd) || !!registryTx(cd); const cls = "tech rounded-lg px-2.5 py-1 text-[12px] font-bold transition"; return <SapTip name={cd} bare>{ex ? <Link href={`/tcode/${encodeURIComponent(cd)}/`} className={`${cls} hover:brightness-110`} style={{ background: c, color: pillInk(c) }} dir="ltr">{cd}</Link> : <span className={`${cls} border border-hairline bg-surface-2 text-ink-3`} dir="ltr">{cd}</span>}</SapTip>; };
  const RelRow = ({ label, items }: { label: string; items: string[] }) => items.length ? (
    <div className="flex flex-wrap items-center gap-2"><span className="min-w-[6.5rem] text-[11px] font-extrabold uppercase tracking-wide text-ink-3">{label}</span><div className="flex flex-wrap gap-1.5">{items.map((x) => <CodeChip key={x} cd={x} />)}</div></div>
  ) : null;
  ...
  return (
    <div className="mx-auto max-w-[1100px] space-y-4" dir="rtl">
      <div className="flex items-center gap-1.5 text-xs text-ink-3">
        ...
        <span className="font-bold text-ink-2" dir="ltr">{t.code}</span>
      </div>
      ...
      {/* in-page nav */}
      <div className="sticky top-[4.25rem] z-20 flex flex-wrap gap-1.5 rounded-2xl border border-hairline bg-surface/90 p-2 backdrop-blur">
        {NAV.map(([id, lbl]) => <button key={id} onClick={() => jump(id)} className="rounded-lg bg-surface-2 px-2.5 py-1 text-[11px] font-bold text-ink-2 transition hover:bg-brand/10 hover:text-brand">{lbl}</button>)}
      </div>

      {/* what am I looking at */}
      <Sec id="s-about" title="מה אני רואה?" icon={<Lightbulb className="size-4" style={{ color: c }} />}>
        <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-3.5"><p className="text-[14px] leading-relaxed text-ink-1">{t.beginner}</p></div>
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          <div className="rounded-xl bg-emerald-50/60 p-3"><div className="mb-1 text-[11px] font-bold uppercase text-emerald-600">מתי להשתמש</div><p className="text-[12.5px] leading-relaxed text-ink-2">{t.whenUse}</p></div>
          <div className="rounded-xl bg-rose-50/60 p-3"><div className="mb-1 text-[11px] font-bold uppercase text-rose-600">מתי לא</div><p className="text-[12.5px] leading-relaxed text-ink-2">{t.whenNot}</p></div>
        </div>
        <details className="group mt-2 rounded-xl border border-hairline bg-surface-2/60 p-3">
          <summary className="flex cursor-pointer items-center gap-1.5 text-[12px] font-bold text-ink-2"><ChevronDown className="size-3.5 transition-transform group-open:rotate-180" />הסבר ליועץ (טכני)</summary>
          <div className="mt-2 space-y-2">
            <p className="text-[13px] leading-relaxed text-ink-2">{t.consultant}</p>
            <p className="text-[12.5px] leading-relaxed text-ink-3"><span className="font-bold text-ink-2">טכני: </span>{t.descTech}</p>
            <p className="text-[12.5px] leading-relaxed text-ink-3"><span className="font-bold text-ink-2">תהליך: </span>{t.process}</p>
            {t.prereq?.length ? <p className="text-[12.5px] text-ink-3"><span className="font-bold text-ink-2">דרישות מקדימות: </span>{t.prereq.join(" · ")}</p> : null}
      ...
      <Sec id="s-rel" title="חוקר קשרים" icon={<Boxes className="size-4" style={{ color: c }} />} sub="הכול לחיץ — טבלאות, טרנזקציות ואובייקטים טכניים">
        ...
        {relCards.length > 0 && <div className="mt-4 grid gap-2.5 sm:grid-cols-2 xl:grid-cols-3">
          {relCards.map((card) => (
            <div key={card.label} className="rounded-xl border border-hairline bg-surface-2/60 p-3">
              ...
              {card.objHref
                ? <div className="flex flex-wrap gap-1.5">{card.items.map((n) => <SapTip key={n} name={n} bare><Link href={`/object/${encodeURIComponent(n)}/`} className="tech rounded-lg bg-surface px-2 py-0.5 text-[11px] font-bold text-ink-2 ring-1 ring-hairline hover:text-brand hover:ring-brand/40" dir="ltr">{n}</Link></SapTip>)}</div>
      ...
      <Sec id="s-notes" title="הערות יועץ" icon={<Wrench className="size-4" style={{ color: c }} />} sub="טעויות · שגיאות · Best Practices · OSS">
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {t.certTips && <div><div className="eyebrow mb-1 text-ink-3">טיפ הסמכה</div><p className="text-[13px] leading-relaxed text-ink-2">{t.certTips}</p></div>}
        </div>
      ...
      <div className="flex flex-wrap gap-2 pb-4">
        ...
        <Link href={`/graph/?node=${encodeURIComponent(t.code)}`} className="inline-flex items-center gap-1.5 rounded-xl bg-surface-2 px-3 py-2 text-[12px] font-bold text-ink-2 hover:bg-hairline"><GitBranch className="size-4" />גרף</Link>
        <Link href={t.module.startsWith("PM") ? "/pm/" : t.module.startsWith("PP") ? "/pp-pi/" : "/sap-infrastructure/"} className="inline-flex items-center gap-1.5 rounded-xl bg-surface-2 px-3 py-2 text-[12px] font-bold text-ink-2 hover:bg-hairline"><Workflow className="size-4" />מודול {t.module}</Link>
        <Link href="/security/" className="inline-flex items-center gap-1.5 rounded-xl bg-surface-2 px-3 py-2 text-[12px] font-bold text-ink-2 hover:bg-hairline"><ArrowRightLeft className="size-4" />אבטחה</Link>
      </div>
      <p className="pb-6 text-center text-[11px] text-ink-3">ידע טרנזקציות SAP סטנדרטי · trust: curated — קודים ואובייקטים אמיתיים בלבד.</p>
    </div>
  );
}
