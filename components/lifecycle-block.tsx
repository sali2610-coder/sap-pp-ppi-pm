...
export function LifecycleBlock({ code }: { code: string }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" dir="rtl">
      <header className="flex flex-wrap items-center gap-2 border-b border-slate-100 bg-gradient-to-l from-slate-50 to-white px-5 py-3">
        <ArrowRightLeft className="size-4 text-slate-500" />
        <h3 className="text-sm font-extrabold tracking-tight text-slate-900">מחזור חיים · Evolution</h3>
      ...
      <div className="grid gap-x-4 gap-y-2.5 p-5 sm:grid-cols-2">
        ...
        {lc.simplification && <Row label="Simplification Item"><span className="text-slate-600">{lc.simplification}</span></Row>}
      ...
      <div className="border-t border-slate-100 px-5 py-3 text-[13px] leading-relaxed text-slate-600"><b className="text-slate-400">Migration Notes:</b> {lc.migration}</div>
    </section>
  );
}
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="flex items-center justify-between gap-2 border-b border-slate-50 pb-1.5"><span className="text-xs font-bold text-slate-400">{label}</span><span className="text-sm">{children}</span></div>;
}
