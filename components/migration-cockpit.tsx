        ...
        <div className="relative">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2.5 sm:grid-cols-4">
            {([["אובייקטי הגירה", MIG_META.objects], ["גישות העברה", MIG_META.approaches], ["תקלות נפוצות", MIG_META.errors], ["שכבות טעינה", MIG_LOAD_LAYERS.length]] as const).map(([l, n]) => (
              <div key={l} className="rounded-2xl border border-white/15 bg-surface/10 px-4 py-3 backdrop-blur-sm"><div className="text-2xl font-extrabold tabular-nums">{n}</div><div className="text-[10px] font-bold uppercase tracking-wide text-white/60">{l}</div></div>
            ))}
          </div>
        </div>
      </header>
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-[210px_minmax(0,1fr)]">
        <aside className="hidden lg:block"><nav className="sticky top-4 space-y-1">
          {SECTIONS.map((s) => { const Ic = s.icon; const on = active === s.id; return (
            <button key={s.id} onClick={() => go(s.id)} className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-right text-sm font-bold transition ${on ? "bg-surface text-ink-1 shadow-sm ring-1 ring-hairline" : "text-ink-3 hover:bg-surface/70 hover:text-ink-1"}`}><Ic className="size-4 shrink-0" style={on ? { color: "#0e7490" } : undefined} />{s.he}</button>
          ); })}
          <div className="!mt-3 space-y-1 border-t border-hairline pt-3">
            {[["מרכז S/4", "/s4hana/"], ["מודל נתונים", "/sap-infrastructure/"], ["ניתוח השפעה", "/impact/"]].map(([l, h]) => <Link key={h} href={h} className="flex items-center gap-2 rounded-xl px-3 py-1.5 text-[13px] font-bold text-ink-3 transition hover:text-brand"><ArrowLeft className="size-3.5" />{l}</Link>)}
          </div>
        </nav></aside>
        ...
        <div ref={scrollRef} className="min-w-0 max-h-[calc(100vh-2rem)] space-y-5 overflow-y-auto lg:pe-1">
          <Card id="explorer" title="Cockpit Explorer · גישות העברה" icon={<Database className="size-4" />} sub="LTMC · Staging · File · Migration Object Modeler · Direct Transfer" accent="#0e7490">
            <div className="grid-adaptive">{APPROACHES.map((a) => (
              <div key={a.id} className="rounded-2xl border border-hairline bg-surface-2/60 p-4">
                <div className="flex items-center justify-between gap-2"><span className="font-extrabold text-ink-1">{a.he}</span>{a.trust === "needs-verification" && <span className="rounded bg-amber-200 px-1.5 py-0.5 text-[8px] font-bold text-amber-950">verify</span>}</div>
                <div className="font-mono text-[11px] text-cyan-700" dir="ltr">{a.en}</div>
                <p className="mt-1.5 text-[12px] leading-relaxed text-ink-2">{a.desc}</p>
                <p className="mt-1 text-[11px] font-bold text-ink-3">מתי: {a.when}</p>
                {a.note && <p className="mt-1 rounded bg-surface px-2 py-1 text-[11px] text-ink-3 ring-1 ring-hairline">{a.note}</p>}
              </div>
            ))}</div>
          </Card>
          ...
          <Card id="library" title="ספריית אובייקטי הגירה" icon={<Boxes className="size-4" />} sub={`${MIG_OBJECTS.length} אובייקטים · לחיצה לפרטים`} accent="#7c3aed">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <div className="relative flex-1"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-ink-3" /><input value={libQ} onChange={(e) => setLibQ(e.target.value)} placeholder="חפש אובייקט…" className="w-full rounded-xl border border-hairline bg-surface py-2 pe-3 ps-9 text-sm outline-none focus:border-violet-400" /></div>
              <div className="flex flex-wrap gap-1">{["all", "Foundation", "Master", "Transactional", "HR"].map((c) => <button key={c} onClick={() => setLibCat(c)} className={`rounded-full px-2.5 py-1 text-[11px] font-bold transition ${libCat === c ? "bg-slate-900 text-white" : "bg-surface-2 text-ink-3 hover:bg-hairline"}`}>{c === "all" ? "הכל" : CAT_HE[c as MigCat]}</button>)}</div>
            </div>
            <div className="grid gap-3 lg:grid-cols-[1fr_320px]">
              ...
              {o && <div className="lg:sticky lg:top-2 lg:self-start"><div className="overflow-hidden rounded-2xl border border-hairline bg-surface shadow-sm" key={o.id}>
                ...
                <div className="space-y-2 p-4 text-[12px]">
                  ...
                  <div><div className="text-[10px] font-bold uppercase text-ink-3">מקור ECC</div><div className="flex flex-wrap gap-1" dir="ltr">{o.ecc.length ? o.ecc.map((t) => <Link key={t} href={`/object/${encodeURIComponent(t)}/`} className="tech rounded-md bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2 transition hover:text-brand">{t}</Link>) : <span className="text-ink-3">—</span>}</div></div>
                  <p className="rounded-lg bg-surface-2 px-2.5 py-1.5 text-[12px] leading-relaxed text-ink-2">{o.note}</p>
                </div>
              </div></div>}
            </div>
          </Card>
            ...
            <div className="grid gap-3 lg:grid-cols-4">{MIG_LOAD_LAYERS.map((layer) => (
              <div key={layer.cat} className="rounded-2xl border-2 p-3" style={{ borderColor: layer.c + "44" }}>
                ...
                <div className="space-y-1.5">{MIG_OBJECTS.filter((m) => m.cat === layer.cat).map((m) => (
                  <button key={m.id} onClick={() => { setSel(m.id); go("library"); }} className="block w-full rounded-lg border border-hairline bg-surface px-2.5 py-1.5 text-right transition hover:border-hairline">
                    <span className="block text-[12px] font-extrabold text-ink-1">{m.he}</span>
                    {m.dependsOn.length > 0 && <span className="block text-[9px] text-ink-3">↳ {m.dependsOn.map((d) => byId[d]?.he || d).join(" · ")}</span>}
                  </button>
                ))}</div>
              </div>
            ))}</div>
          ...
          <Card id="readiness" title="Migration Readiness Score" icon={<Gauge className="size-4" />} sub="סמן מה הושלם — הציון מתעדכן בזמן אמת" accent="#0ea5e9">
            <div className="grid gap-4 lg:grid-cols-[180px_1fr] lg:items-center">
              <div className="text-center"><div className="text-5xl font-extrabold tabular-nums" style={{ color: readyScore >= 80 ? "#16a34a" : readyScore >= 50 ? "#d97706" : "#dc2626" }}>{readyScore}</div><div className="text-[11px] font-bold uppercase text-ink-3">{readyScore >= 80 ? "מוכן" : readyScore >= 50 ? "בתהליך" : "ראשוני"}</div><div className="mt-2 h-2 overflow-hidden rounded-full bg-surface-2"><span className="block h-full rounded-full transition-all" style={{ width: `${readyScore}%`, background: readyScore >= 80 ? "#16a34a" : "#0ea5e9" }} /></div></div>
              <div className="space-y-1.5">{READINESS.map((r, i) => { const on = ready.has(i); return (
                <button key={i} onClick={() => { const n = new Set(ready); n.has(i) ? n.delete(i) : n.add(i); setReady(n); }} className={`flex w-full items-center gap-2.5 rounded-xl border px-3 py-2 text-right transition ${on ? "border-emerald-300 bg-emerald-50" : "border-hairline bg-surface hover:border-hairline"}`}>
                  <span className={`grid size-5 shrink-0 place-items-center rounded-md border-2 ${on ? "border-emerald-500 bg-emerald-500 text-white" : "border-hairline"}`}>{on && <CheckCircle2 className="size-3.5" />}</span>
                  <span className="flex-1 text-[13px] font-bold text-ink-2">{r.he}</span><span className="text-[11px] font-bold text-ink-3">{r.w}%</span>
                </button>
              ); })}</div>
            </div>
          </Card>
          ...
          <Card id="quality" title="מרכז איכות נתונים" icon={<ShieldCheck className="size-4" />} sub="ממדי איכות שיש לאמת לפני טעינה">
            <div className="grid-adaptive">{QUALITY_DIMS.map((d) => (
              <div key={d.he} className="rounded-2xl border border-hairline bg-surface-2/60 p-3"><div className="text-[13px] font-extrabold text-ink-1">{d.he}</div><p className="text-[12px] text-ink-3">{d.sub}</p></div>
            ))}</div>
          </Card>
          ...
          <Card id="errors" title="ספריית תקלות נפוצות" icon={<AlertTriangle className="size-4" />} sub={`${MIG_ERRORS.length} תקלות LTMC נפוצות + פתרון`} accent="#dc2626">
            <div className="relative mb-3"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-ink-3" /><input value={errQ} onChange={(e) => setErrQ(e.target.value)} placeholder="חפש תקלה…" className="w-full rounded-xl border border-hairline bg-surface py-2 pe-3 ps-9 text-sm outline-none focus:border-rose-400" /></div>
            <div className="space-y-2">{errs.map((e, i) => (
              <details key={i} className="group rounded-xl border border-hairline bg-surface [&_summary]:list-none">
                <summary className="flex cursor-pointer items-center gap-2 p-3"><AlertTriangle className="size-4 shrink-0 text-rose-500" /><span className="flex-1 text-[13px] font-extrabold text-ink-1">{e.he}</span></summary>
                <div className="space-y-1.5 border-t border-hairline px-3 py-2 text-[12px]"><p><b>סימפטום:</b> {e.symptom}</p><p><b>סיבה:</b> {e.cause}</p><p className="rounded bg-emerald-50 px-2 py-1 text-emerald-900"><b>פתרון:</b> {e.fix}</p></div>
              </details>
            ))}{errs.length === 0 && <p className="py-6 text-center text-sm text-ink-3">לא נמצא.</p>}</div>
          </Card>

          <Card id="mapping" title="ECC → אובייקט הגירה" icon={<GitCompare className="size-4" />} sub="מאיזה טבלאות ECC כל אובייקט הגירה נטען">
            <div className="overflow-hidden rounded-xl border border-hairline"><table className="w-full text-right text-[13px]"><thead className="bg-surface-2 text-[11px] font-bold uppercase text-ink-3"><tr><th className="px-3 py-2">אובייקט הגירה</th><th className="px-3 py-2">מודול</th><th className="px-3 py-2">טבלאות ECC</th></tr></thead>
              <tbody>{MIG_OBJECTS.filter((m) => m.ecc.length).map((m) => (<tr key={m.id} className="border-t border-hairline"><td className="px-3 py-2 font-bold text-ink-1">{m.he} <span className="font-mono text-[11px] text-ink-3" dir="ltr">{m.name}</span></td><td className="px-3 py-2 text-ink-3">{m.module}</td><td className="px-3 py-2"><span className="flex flex-wrap gap-1" dir="ltr">{m.ecc.map((t) => <Link key={t} href={`/object/${encodeURIComponent(t)}/`} className="tech rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2 transition hover:text-brand">{t}</Link>)}</span></td></tr>))}</tbody></table></div>
          </Card>
          ...
          <Card id="checklist" title="S/4 Migration Checklist" icon={<ListChecks className="size-4" />} sub="תהליך ההגירה מקצה לקצה" accent="#16a34a">
            <div className="mb-2 flex items-center gap-2"><div className="h-2 flex-1 overflow-hidden rounded-full bg-surface-2"><span className="block h-full rounded-full bg-emerald-500 transition-all" style={{ width: `${Math.round((done.size / MIG_CHECKLIST.length) * 100)}%` }} /></div><span className="text-xs font-extrabold tabular-nums text-emerald-600">{done.size}/{MIG_CHECKLIST.length}</span></div>
            <div className="space-y-1.5">{MIG_CHECKLIST.map((c, i) => { const on = done.has(i); return (
              <button key={i} onClick={() => { const n = new Set(done); n.has(i) ? n.delete(i) : n.add(i); setDone(n); }} className={`flex w-full items-center gap-2.5 rounded-xl border px-3 py-2 text-right transition ${on ? "border-emerald-300 bg-emerald-50" : "border-hairline bg-surface hover:border-hairline"}`}>
                <span className={`grid size-5 shrink-0 place-items-center rounded-md border-2 ${on ? "border-emerald-500 bg-emerald-500 text-white" : "border-hairline"}`}>{on && <CheckCircle2 className="size-3.5" />}</span>
                <span className={`flex-1 text-[13px] font-bold ${on ? "text-ink-3 line-through" : "text-ink-2"}`}>{i + 1}. {c}</span>
              </button>
            ); })}</div>
            <div className="mt-3 flex flex-wrap gap-1.5 border-t border-hairline pt-3">{[["מרכז S/4", "/s4hana/"], ["מודל נתונים", "/sap-infrastructure/"], ["ניתוח השפעה", "/impact/"]].map(([l, h]) => <Link key={l} href={h} className="rounded-lg bg-surface-2 px-2.5 py-1 text-[11px] font-bold text-ink-2 transition hover:bg-hairline">{l}</Link>)}</div>
          </Card>
          <RelatedCenters />
          <p className="pb-4 text-center text-[11px] text-ink-3">תוכן מבוסס SAP S/4HANA Migration Cockpit (LTMC/LTMOM) · trust: curated · פריטים לא ודאיים: needs-verification · לא להמציא — אמת מול גרסת היעד ו-SAP Notes.</p>
        </div>
      </div>
    </div>
  );
