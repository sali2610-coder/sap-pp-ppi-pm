...
export function QuickAccess({ counts = {} }: { counts?: Record<string, number> }) {
  ...
  return (
    <section dir="rtl">
      <div className="mb-4 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.22em] text-ink-3"><LayoutDashboard className="size-4 text-brand" />מרכז הפיקוד · התחל כאן</div>
        ...
        {heroes.map((t) => { const Ic = t.Icon; return (
          <motion.div key={t.href} variants={item}>
            <Link href={t.href} onClick={() => playClick()}
              className="group relative flex h-full min-h-[170px] flex-col justify-between overflow-hidden rounded-3xl border border-hairline bg-surface p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
              ...
              <div className="relative mt-4">
                <div className="flex items-center gap-2"><h3 className="text-xl font-extrabold tracking-tight text-ink-1">{t.he}</h3>{t.count != null && <span className="font-mono text-[13px] font-bold text-ink-3">{t.count.toLocaleString()}</span>}</div>
                <p className="mt-1 text-[13px] leading-relaxed text-ink-3">{t.sub}</p>
      ...
      <motion.div variants={grid} initial="hidden" animate="show" className="mt-3 grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-5">
        {more.map((t) => { const Ic = t.Icon; return (
          <motion.div key={t.href} variants={item}>
            <Link href={t.href} onClick={() => playClick()}
              className="lift group flex h-full items-center gap-3 rounded-2xl border border-hairline bg-surface p-3.5 shadow-sm transition-colors hover:border-brand/30">
              ...
              <span className="min-w-0">
                <span className="flex items-center gap-1.5"><span className="truncate text-[14px] font-extrabold text-ink-1">{t.he}</span>{t.count != null && <span className="font-mono text-[11px] font-bold text-ink-3">{t.count.toLocaleString()}</span>}</span>
                <span className="block truncate text-[11px] font-medium text-ink-3">{t.en}</span>
              </span>
              <ArrowLeft className="ms-auto size-3.5 shrink-0 text-ink-3 transition-all duration-300 group-hover:-translate-x-0.5 group-hover:text-brand" />
            </Link>
          </motion.div>
        ); })}
      </motion.div>
