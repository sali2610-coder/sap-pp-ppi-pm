  ...
  return (
    <>
      ...
      <div className="no-print fixed bottom-[5.25rem] start-5 z-40 flex items-center gap-2">
        <button onClick={() => setOpen(true)} className="lift inline-flex items-center gap-1.5 rounded-full border border-hairline bg-surface px-3.5 py-2 text-sm font-bold text-ink-2 shadow-lg transition hover:border-brand/40 hover:text-brand">
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            <motion.div dir="rtl" className="fixed left-1/2 top-1/2 z-[73] w-[440px] max-w-[92vw] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-hairline bg-surface shadow-2xl"
              ...
              <div className="flex items-center justify-between bg-gradient-to-l from-brand-dark to-brand px-4 py-3 text-white">
                ...
                <button onClick={() => setOpen(false)} className="rounded-lg p-1 text-white/80 hover:bg-surface/15"><X className="size-4" /></button>
              </div>
              <div className="max-h-[70vh] space-y-3 overflow-auto p-4 text-[13px] leading-relaxed text-ink-2">
                ...
                <div className="rounded-xl border border-brand/20 bg-brand/5 p-3">
                  ...
                  <ol className="space-y-1">
                    {help.next.map((n, i) => <li key={i} className="flex gap-2 text-[12px] text-ink-2"><span className="grid size-4 shrink-0 place-items-center rounded-full bg-brand/15 text-[9px] font-extrabold text-brand">{i + 1}</span>{n}</li>)}
                  </ol>
                </div>
              </div>
            </motion.div>
...
function Row({ label, children, tone }: { label: string; children: React.ReactNode; tone?: string }) {
  return <div><p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: tone || "#94a3b8" }}>{label}</p><p className="text-ink-2">{children}</p></div>;
}
