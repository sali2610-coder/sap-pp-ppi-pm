  ...
  return (
    <div ref={wrapRef} className="relative" onMouseEnter={(e) => doOpen(e.currentTarget)} onMouseLeave={scheduleClose}>
      <button
        ...
        className={`relative flex shrink-0 items-center gap-1 whitespace-nowrap rounded-lg px-2.5 py-1.5 transition-colors 2xl:px-3 ${triggerActive ? "bg-white text-brand shadow-[0_2px_8px_-2px_rgba(0,0,0,0.25)]" : "text-white/90 hover:bg-white/15 hover:text-white"}`}>
            ...
            <motion.div
              ...
              <div className="mx-auto px-5 py-6 sm:px-8">
                ...
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <span className="hidden items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-500 sm:inline-flex">{CENTERS.length} מרכזים · ידע מאומת</span>
                </div>
                ...
                <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
                      ...
                      return (
                        <motion.div key={c.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.03 * i, duration: 0.2 }}>
                          <Link href={c.href} role="menuitem" onClick={() => { playClick(); setOpen(false); }}
                            ...
                            className={`group block h-full rounded-2xl border-2 bg-white p-4 transition-all hover:-translate-y-0.5 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 ${focused ? "shadow-lg" : "shadow-sm"}`}
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="font-mono text-[9px] text-slate-400">{c.en}</div>
                              </div>
                              <ArrowLeft className="size-4 shrink-0 text-slate-300 transition-all group-hover:-translate-x-0.5" style={{ color: focused ? c.accent : undefined }} />
                            </div>
                            ...
                            <div className="mt-2.5 flex items-center gap-3">
                              {c.metrics.map((m) => (
                                <div key={m.label} className="leading-none">
                                  ...
                                  <span className="mr-1 text-[10px] font-bold text-slate-400">{m.label}</span>
                                </div>
                              ))}
                            </div>
                            <div className="mt-2.5 flex flex-wrap items-center gap-1">
                              {c.inside.slice(0, 3).map((x) => <span key={x} className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-500" dir="ltr">{x}</span>)}
                              <span className="ms-auto text-[9px] font-bold uppercase tracking-wide text-slate-300">עודכן · {c.updated}</span>
                            </div>
                          </Link>
                        </motion.div>
                      );
                  ...
                  <aside className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4" aria-live="polite">
                    <div key={pv.id} style={{ animation: "fadeUp .2s ease both" }}>
                      <div className="flex items-center gap-2.5">
                        ...
                        <div><div className="text-[14px] font-extrabold text-slate-900">{pv.he}</div><div className="font-mono text-[9px] text-slate-400">{pv.en}</div></div>
                      </div>
                      <p className="mt-2 text-[12px] leading-relaxed text-slate-600">{pv.desc}</p>
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">{pv.inside.map((x) => <span key={x} className="rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10.5px] font-bold text-slate-600" dir="ltr">{x}</span>)}</div>

                      <div className="mt-3 text-[10px] font-bold uppercase tracking-wide text-slate-400"><GitBranch className="me-1 inline size-3" />פופולרי</div>
                      <div className="mt-1 flex flex-wrap gap-1">{pv.popular.map((x) => <span key={x} className="rounded-md bg-white px-1.5 py-0.5 text-[10.5px] font-semibold text-slate-500 ring-1 ring-slate-200">{x}</span>)}</div>
