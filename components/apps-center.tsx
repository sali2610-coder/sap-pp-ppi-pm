...
function HitRow({ h }: { h: SearchHit }) {
  ...
  return (
    <Link href={h.href} className="flex items-center gap-2.5 rounded-xl px-3 py-2 transition hover:bg-surface-2">
      ...
      <ArrowLeft className="size-3.5 shrink-0 text-ink-3" />
    </Link>
  );
}
  ...
  return (
    <div dir="rtl" className="space-y-7">
      ...
      <div className="relative">
        <div className="flex items-center gap-3 rounded-2xl border border-hairline bg-surface px-4 py-3.5 shadow-[var(--elev-1)] focus-within:border-brand/40 focus-within:shadow-[var(--elev-2)]">
          ...
          {q && <button onClick={() => { setQ(""); inputRef.current?.focus(); }} className="text-[11px] font-bold text-ink-3 hover:text-brand">נקה</button>}
        </div>
        <AnimatePresence>
          {q.trim() && (
            <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute inset-x-0 top-full z-30 mt-2 max-h-[60vh] overflow-auto rounded-2xl border border-hairline bg-surface p-1.5 shadow-2xl">
              {hits.length ? hits.map((h) => <HitRow key={h.kind + h.code} h={h} />) : <p className="px-3 py-6 text-center text-sm text-ink-3">אין תוצאות ל-"{q}"</p>}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      {!q.trim() && (
        <>
          {/* what you can search */}
          <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-bold text-ink-3">
            <span>חפש לפי:</span>
            {["טרנזקציית ECC", "טרנזקציית S/4", "שם אפליקציית Fiori", "App ID", "טבלה", "תצוגת CDS", "אובייקט עסקי", "תהליך", "מילת מפתח"].map((t) => <span key={t} className="rounded-full bg-surface-2 px-2 py-0.5">{t}</span>)}
          </div>

          {/* featured daily drivers */}
          <section>
            <div className="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-ink-3"><Star className="size-4 text-brand" />שולחן העבודה היומי · אובייקטים מרכזיים</div>
            <motion.div variants={grid} initial="hidden" animate="show" className="grid-adaptive">
              {featured.map((o) => { const crit = criticality(o); const c = mc(o.intel.module); return (
                <motion.div key={o.code} variants={item}>
                  <Link href={`/apps/${encodeURIComponent(o.code)}/`} className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-hairline bg-surface p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div className="relative mt-3 flex-1">
                      <div className="flex items-center gap-2"><h3 className="tech text-xl font-extrabold tracking-tight text-ink-1" dir="ltr">{o.code}</h3>{o.fiori[0] && <span className="inline-flex items-center gap-0.5 rounded-full bg-amber-50 px-1.5 py-0.5 text-[9px] font-bold text-amber-700"><LayoutGrid className="size-2.5" />Fiori</span>}</div>
                      <p className="mt-1 line-clamp-2 text-[12.5px] leading-relaxed text-ink-3">{o.intel.descHe}</p>
                    </div>
          ...
          {/* integration strip */}
          <section className="rounded-3xl border border-hairline bg-gradient-to-bl from-slate-50 to-white p-5">
            <div className="mb-3 flex items-center gap-2 text-sm font-extrabold text-ink-2"><Sparkles className="size-4 text-brand" />מחובר לכל הפלטפורמה</div>
            <div className="flex flex-wrap gap-2">
              {[["/studio/", "Architecture Studio"], ["/tables/", "חוקר טבלאות"], ["/cds/", "חוקר CDS"], ["/idoc/", "חוקר IDoc"], ["/transactions/", "מרכז טרנזקציות"], ["/fiori-apps/", "אפליקציות Fiori"], ["/knowledge/", "מרכז הידע"]].map(([href, label]) => (
                <Link key={href} href={href} className="lift inline-flex items-center gap-1.5 rounded-xl border border-hairline bg-surface px-3 py-2 text-[12px] font-bold text-ink-2 transition hover:border-brand/30 hover:text-brand">{label}<ArrowLeft className="size-3 text-ink-3" /></Link>
              ))}
            </div>
          </section>
        </>
