...
export function StoryMode({ id }: { id: string }) {
  ...
  if (!story) return <p className="p-8 text-center text-slate-500">סיפור לא נמצא.</p>;
  ...
  const QA = ({ icon, label, text, tone }: { icon: React.ReactNode; label: string; text: string; tone?: string }) => (
    <div className={`flex gap-2.5 rounded-xl p-3 ${tone || "bg-slate-50"}`}>
      <span className="mt-0.5 shrink-0">{icon}</span>
      <div><div className="text-[11px] font-extrabold uppercase tracking-wide text-slate-400">{label}</div><p className="text-sm leading-relaxed text-slate-700">{text}</p></div>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4" dir="rtl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <Link href={story.learnPath} className="inline-flex items-center gap-1.5 text-sm font-bold text-slate-500 transition hover:text-brand"><ArrowRight className="size-4" />חזרה למסלול הלמידה</Link>
        <span className="text-xs font-bold text-slate-400">שלב {i + 1} מתוך {total}</span>
      </div>
      ...
      <header className="relative overflow-hidden rounded-3xl border p-5 shadow-sm" style={{ borderColor: accent + "33", background: `linear-gradient(135deg, ${accent}0d, #fff)` }}>
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-extrabold text-slate-900">{story.he}</h1><p className="text-xs text-slate-500">{story.sub}</p></div>
        </div>
        {/* phase rail — you are here */}
        <div className="mt-3 flex items-center gap-1 overflow-x-auto pb-1">
          {story.steps.map((st, idx) => (
            <button key={st.id} onClick={() => setI(idx)} title={st.title}
              className={`flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold transition ${idx === i ? "text-white shadow-sm" : idx < i ? "text-slate-500 bg-slate-100" : "text-slate-400 bg-slate-50"}`}
              style={idx === i ? { background: accent } : undefined}>
              <span className={`grid size-4 place-items-center rounded-full text-[9px] ${idx < i ? "bg-emerald-500 text-white" : idx === i ? "bg-white/25" : "bg-slate-200 text-slate-500"}`}>{idx < i ? "✓" : st.n}</span>
      ...
      <AnimatePresence mode="wait">
        <motion.section key={s.id} initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={reduce ? undefined : { opacity: 0, y: -8 }} transition={{ duration: 0.22 }}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {s.object && <Link href={`/object/${encodeURIComponent(s.object)}/`} className="tech rounded-md bg-slate-100 px-2 py-0.5 font-mono text-xs font-bold text-slate-600 transition hover:bg-slate-200" dir="ltr">{s.object}</Link>}
          </div>
          ...
          {s.object && (parents.length || children.length || tcodes.length || related.length) ? (
            <div className="mt-3 space-y-3 rounded-2xl border border-slate-100 bg-slate-50/60 p-3">
              {(parents.length || children.length) ? (
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <span className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-400"><Network className="size-3.5" />קשרים:</span>
              ...
              <div className="flex flex-wrap gap-2 pt-0.5">
                ...
                <Link href={`/object/${encodeURIComponent(s.object)}/`} className="tap inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-600 transition hover:border-brand/40 hover:text-brand"><BookOpen className="size-3.5" />כרטיס אובייקט מלא</Link>
              </div>
            </div>
          ) : null}
        </motion.section>
      ...
      <div className="flex items-center justify-between gap-2">
        <button onClick={() => go(-1)} disabled={i === 0} className="tap inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-600 transition enabled:hover:border-brand/40 enabled:hover:text-brand disabled:opacity-40"><ArrowRight className="size-4" />הקודם</button>
        {last ? (
...
function Chip({ label, arrow }: { label: string; arrow: string }) {
  return <span className="inline-flex items-center gap-0.5 rounded-md border border-slate-200 bg-white px-1.5 py-0.5 font-mono text-[11px] font-bold text-slate-500" dir="ltr"><span className="text-slate-300">{arrow}</span>{label}</span>;
}
function Row({ icon, label, items }: { icon: React.ReactNode; label: string; items: string[] }) {
  return (
    <div className="flex flex-wrap items-center gap-2 text-xs">
      <span className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-400">{icon}{label}:</span>
      {items.map((x) => <span key={x} className="tech rounded-md border border-slate-200 bg-white px-1.5 py-0.5 font-mono text-[11px] font-bold text-slate-600" dir="ltr">{x}</span>)}
    </div>
  );
}
