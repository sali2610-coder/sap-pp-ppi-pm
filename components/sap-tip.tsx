...
export function SapTip({ name, children, linkless, bare }: { name: string; children: React.ReactNode; linkless?: boolean; bare?: boolean }) {
  ...
  const Row = ({ icon, color, label, text }: { icon: React.ReactNode; color: string; label: string; text: string }) => (
    <span className="block">
      ...
      <span className="block text-[12px] leading-relaxed text-ink-2">{text}</span>
    </span>
  );
    ...
    <span className="relative inline-flex" onMouseEnter={show} onMouseLeave={hide} onFocus={show} onBlur={hide}>
      ...
      {open && (
        <span role="tooltip" dir="rtl" onMouseEnter={show} onMouseLeave={hide}
          ...
          {/* simple explanation */}
          {tip.he && <span className="block text-[12.5px] font-semibold leading-relaxed text-ink-2">{tip.he}</span>}
          ...
          {(tip.ecc || tip.s4) && (
            <span className="block space-y-0.5 rounded-lg bg-surface-2 p-2 text-[11px] leading-relaxed">
              {tip.ecc && <span className="block text-ink-3"><b className="text-ink-3">ECC:</b> {tip.ecc}</span>}
              {tip.s4 && <span className="block text-ink-3"><b className="text-green-600">S/4:</b> {tip.s4}</span>}
            </span>
          )}
          ...
          {tip.related && tip.related.length > 0 && (
            <span className="block">
              <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-ink-3"><GitBranch className="size-3" />קשור</span>
              <span className="mt-0.5 flex flex-wrap gap-1">{tip.related.map((r) => <Link key={r} href={`${relBase}${encodeURIComponent(r)}/`} className="tech rounded-md bg-surface-2 px-1.5 py-0.5 text-[10.5px] font-bold text-ink-2 hover:bg-brand/10 hover:text-brand" dir="ltr">{r}</Link>)}</span>
            </span>
          )}

          {/* footer */}
          <span className="flex items-center justify-between border-t border-hairline pt-2">
            {tip.verified
            ...
            <span className="flex items-center gap-2.5">
              <button type="button" onClick={(e) => { e.preventDefault(); inspectEntity(tip.name); }} className="inline-flex items-center gap-0.5 text-[11px] font-bold text-ink-3 hover:text-brand" aria-label="שלח למפקח"><PanelRight className="size-3" />מפקח</button>
              {tip.graphHref && <Link href={tip.graphHref} className="inline-flex items-center gap-0.5 text-[11px] font-bold text-ink-3 hover:text-brand"><Network className="size-3" />גרף</Link>}
              {linkless
