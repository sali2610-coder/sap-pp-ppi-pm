  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[var(--elev-1)] sm:p-6">
        <h2 className="mb-3 flex items-center gap-2 text-lg font-extrabold text-slate-900"><Sigma className="size-5" style={{ color: ACCENT }} />מודל הנתונים הווירטואלי (VDM)</h2>
        <div className="flex flex-wrap items-center gap-2 text-[12.5px] font-bold">
          <span className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-slate-600">טבלת ECC קלאסית</span>
        ...
        <p className="mt-3 text-[12px] leading-relaxed text-slate-500">ב-S/4HANA הגישה לנתונים עוברת דרך תצוגות CDS במקום קריאה ישירה לטבלה. תצוגת Interface (I_) חושפת את הנתון הגולמי, תצוגת Consumption (C_) מוסיפה היגיון אנליטי, ואפליקציית Fiori צורכת את השרשרת.</p>
      </section>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex flex-1 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 shadow-sm">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="חיפוש תצוגה · טבלה · אפליקציה…" className="w-full bg-transparent text-sm outline-none placeholder:text-slate-400" />
        </div>
        <div className="flex gap-1.5">
          {(["all", "PM", "PP-PI"] as const).map((m) => <button key={m} onClick={() => setMod(m)} className={`tap rounded-xl px-3 py-2 text-xs font-bold transition ${mod === m ? "text-white shadow-sm" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`} style={mod === m ? { background: m === "all" ? ACCENT : MOD_C[m] } : undefined}>{m === "all" ? "הכול" : m}</button>)}
        </div>
      </div>

      <p className="text-xs font-bold text-slate-400">{list.length} תצוגות</p>
...
function ChainCard({ v }: { v: CdsView }) {
  return (
    <div className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-md">
      <div className="flex items-start justify-between gap-2">
        <Link href={`/cds/${encodeURIComponent(v.view)}/`} className="flex items-center gap-2 min-w-0">
          ...
          <span className="min-w-0">
            <span className="tech block truncate font-mono text-[15px] font-extrabold text-slate-900 group-hover:text-brand" dir="ltr">{v.view}</span>
            <span className="block truncate text-[11.5px] font-semibold text-slate-500">{v.he}</span>
          </span>
        </Link>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        <span className="flex items-center gap-1 text-[10px] font-bold uppercase text-slate-400"><Database className="size-3" />טבלאות ECC</span>
        {v.tables.map((t) => (
          <SapTip key={t} name={t} bare>
            <Link href={`/object/${encodeURIComponent(t)}/`} className="tech rounded-md bg-slate-100 px-1.5 py-0.5 font-mono text-[11px] font-bold text-slate-600 transition hover:bg-brand/10 hover:text-brand" dir="ltr">{t}</Link>
          </SapTip>
        ))}
      </div>
      ...
      {(v.consumption || v.fiori) && (
        <div className="mt-2.5 flex flex-wrap items-center gap-1.5 border-t border-slate-100 pt-2.5 text-[11px] font-bold">
          ...
          {v.consumption && v.fiori && <ArrowLeft className="size-3 text-slate-300" />}
