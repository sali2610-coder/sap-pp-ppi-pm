        ...
        {maps.map((m) => { const c = DC[m.domain] || "#64748b"; const on = m.slug === active.slug;
          return (
            <button key={m.slug} onClick={() => { setSel(m.slug); setOpen(0); }}
              className={`lift group relative overflow-hidden rounded-2xl border bg-white p-3.5 text-start shadow-sm transition-all ${on ? "border-transparent ring-2" : "border-slate-200 hover:border-slate-300"}`}
              ...
              <div className="mt-2 text-sm font-extrabold text-slate-900"><Highlight text={m.he} query={q} /></div>
              <div className="text-[11px] font-semibold text-slate-400">{m.title} · {m.steps.length} שלבים</div>
            </button>
          ); })}
        {maps.length === 0 && <p className="col-span-full rounded-xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-400">אין תהליך תואם לחיפוש.</p>}
      </div>

      {/* active map flow */}
      {active && (
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[var(--elev-1)]">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div><h2 className="text-lg font-extrabold tracking-tight text-slate-900">{active.he} · {active.title}</h2><p className="text-xs text-slate-500">{active.summary}</p></div>
            </div>
          ...
          <motion.ol key={active.slug + s} variants={container} initial="hidden" animate="show" className="relative space-y-2.5 border-r-2 border-slate-100 pr-5">
            {steps.map(({ st, i }) => { const isOpen = open === i; const c = DC[active.domain] || "#4338ca";
              return (
                <motion.li key={i} variants={item} className="relative">
                  ...
                  <div className={`overflow-hidden rounded-2xl border bg-white shadow-sm transition-colors ${isOpen ? "border-brand/30" : "border-slate-200 hover:border-slate-300"}`}>
                    <button onClick={() => setOpen(isOpen ? null : i)} className="flex w-full items-center justify-between gap-2 px-4 py-3 text-start">
                      <h3 className="text-sm font-extrabold text-slate-900"><Highlight text={st.he} query={q} /></h3>
                      <span className="flex items-center gap-2">
                        <span className="hidden gap-1 sm:flex">{st.tcodes.slice(0, 3).map((tc) => <span key={tc} className="tech rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-500" dir="ltr"><Highlight text={tc} query={q} /></span>)}</span>
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        <motion.div initial={reduce ? false : { height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={reduce ? undefined : { height: 0, opacity: 0 }} transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}>
                          <div className="grid gap-2.5 border-t border-slate-100 px-4 py-3 sm:grid-cols-2">
            ...
            {steps.length === 0 && <li className="rounded-xl border border-dashed border-slate-200 p-5 text-center text-sm text-slate-400">אין שלב תואם בתהליך זה.</li>}
          </motion.ol>
        </div>
      )}
    </div>
...
function Chips({ items, hrefFn, q, tone }: { items?: string[]; hrefFn?: (s: string) => string; q: string; tone: string }) {
  ...
  return <div className="flex flex-wrap gap-1.5">{items.map((it) => { const h = hrefFn ? hrefFn(it) : ""; return h
    ? <Link key={it} href={h} className="tech rounded-md border bg-slate-50 px-2 py-0.5 text-[11px] font-bold text-slate-600 transition hover:text-brand" style={{ borderColor: tone + "44" }} dir="ltr"><Highlight text={it} query={q} /></Link>
    : <span key={it} className="tech rounded-md border border-dashed border-slate-200 bg-white px-2 py-0.5 text-[11px] font-bold text-slate-500" dir="ltr"><Highlight text={it} query={q} /></span>; })}</div>;
}
