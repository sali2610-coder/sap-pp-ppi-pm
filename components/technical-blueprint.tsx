...
import {
  ...
  AlertTriangle, ArrowLeft, Sparkles, ArrowRightLeft, AppWindow, ListTree, Layers, Gauge, Clock, Signal,
} from "lucide-react";
...
// per-topic aggregation of verified objects (shared by header widgets + body)
function aggregate(topic: SAPTopic) {
...
// UI heuristic (labeled as an estimate — not an SAP fact): complexity + read time
function difficulty(tables: number, s4: number) {
  const score = tables + s4 * 1.5;
  if (score >= 9) return { he: "מתקדם", c: "#dc2626", level: 3 };
  if (score >= 5) return { he: "בינוני", c: "#d97706", level: 2 };
  return { he: "בסיסי", c: "#16a34a", level: 1 };
}
const readMin = (tables: number, funcs: number) => Math.max(3, Math.round(tables * 1.4 + funcs * 0.4));
...
function BlueprintSection({ topic, code, query, index, total, open, onToggle, refCb }: { topic: SAPTopic; code: string; query: string; index: number; total: number; open: boolean; onToggle: () => void; refCb: (el: HTMLElement | null) => void }) {
  const accent = accentFor(code);
  const Icon = topicIcon(topic.title);
  const domain = domainBySlug(TOPIC_DOMAIN[`${code}-${topic.idx}`] || "");
  const agg = useMemo(() => aggregate(topic), [topic]);
  const diff = difficulty(topic.tables.length, agg.s4.length);
  ...
  return (
    <section ref={refCb} id={`bp-${topic.idx}`} data-bp={topic.idx} className="relative scroll-mt-40 pr-0 sm:pr-14">
      {/* journey rail (desktop) */}
      <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-14 sm:block" aria-hidden>
        {index < total - 1 && <span className="absolute right-[27px] top-6 bottom-[-1.25rem] w-0.5 rounded-full bg-gradient-to-b from-slate-200 to-transparent" />}
        ...
        <button onClick={onToggle} aria-expanded={open} className="group flex w-full items-start gap-3.5 p-5 text-start transition hover:bg-slate-50/60">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-2.5 flex flex-wrap gap-1.5">
              <Kpi icon={<Database className="size-3" />} value={topic.tables.length} label="טבלאות" c="#0891b2" />
              <Kpi icon={<Terminal className="size-3" />} value={agg.tcodes.length} label="T-Codes" c="#475569" />
              <Kpi icon={<Cable className="size-3" />} value={agg.funcs.length} label="BAPIs" c="#2563eb" />
              <Kpi icon={<FileCode2 className="size-3" />} value={agg.cds.length} label="CDS" c="#16a34a" />
...
export function TechnicalBlueprint({ module, query }: { module: SAPModuleData; query: string }) {
  ...
  // module-level KPI totals (verified aggregation)
  const kpis = useMemo(() => {
    const tables = new Set<string>(), tc = new Set<string>(), fn = new Set<string>(), cds = new Set<string>(); let s4 = 0;
    topics.forEach((tp) => { const a = aggregate(tp); tp.tables.forEach((t) => tables.add(t.tableName)); a.tcodes.forEach((x) => tc.add(x)); a.funcs.forEach((x) => fn.add(x)); a.cds.forEach((x) => cds.add(x)); s4 += a.s4.length; });
    return { tables: tables.size, tc: tc.size, fn: fn.size, cds: cds.size, s4 };
  }, [topics]);
  ...
  const [active, setActive] = useState<number | null>(topics[0]?.idx ?? null);
  const refs = useRef<Map<number, HTMLElement>>(new Map());
  useEffect(() => {
    const io = new IntersectionObserver((entries) => {
      const vis = entries.filter((e) => e.isIntersecting).sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top)[0];
      if (vis) { const id = Number((vis.target as HTMLElement).dataset.bp); if (!Number.isNaN(id)) setActive(id); }
    ...
    <div className="space-y-4">
      ...
      <div className="sticky top-[4.5rem] z-20 overflow-hidden rounded-3xl border border-slate-200 bg-white/85 shadow-[0_10px_30px_-20px_rgba(15,23,42,.5)] backdrop-blur-xl">
        ...
        <div className="flex flex-wrap gap-1.5 px-4">
          <Kpi icon={<Database className="size-3" />} value={kpis.tables} label="טבלאות" c="#0891b2" />
          <Kpi icon={<Terminal className="size-3" />} value={kpis.tc} label="T-Codes" c="#475569" />
          <Kpi icon={<Cable className="size-3" />} value={kpis.fn} label="BAPIs/FM" c="#2563eb" />
          <Kpi icon={<FileCode2 className="size-3" />} value={kpis.cds} label="CDS" c="#16a34a" />
          <Kpi icon={<ArrowRightLeft className="size-3" />} value={kpis.s4} label="שינויי S/4" c="#d97706" />
          <Kpi icon={<Gauge className="size-3" />} value={topics.length} label="נושאים" c={accent} />
        </div>
        {/* scroll-spy category rail */}
        <div className="chip-rail mt-3 flex flex-nowrap gap-1.5 overflow-x-auto px-4 pb-3 sm:flex-wrap sm:overflow-visible">
          {topics.map((t) => { const on = active === t.idx; return (
      ...
      {/* sections with journey timeline */}
      <div className="space-y-4">
        {topics.map((t, i) => (
          <BlueprintSection key={t.idx} topic={t} code={code} query={query} index={i} total={topics.length}
            open={open.has(t.idx)} onToggle={() => toggle(t.idx)}
            refCb={(el) => { if (el) refs.current.set(t.idx, el); else refs.current.delete(t.idx); }} />
        ))}
      </div>
    </div>
  );
