...
export function RelatedCenters({ exclude, title = "מרכזים קשורים" }: { exclude?: string; title?: string }) {
  ...
  return (
    <section dir="rtl" className="mt-6 rounded-2xl border border-hairline bg-surface-2/60 p-4">
      ...
      <div className="grid-adaptive-sm">
        {items.map((c) => { const Ic = c.Icon; return (
          <Link key={c.id} href={c.href} className="group flex items-start gap-2.5 rounded-xl border border-hairline bg-surface p-3 transition hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-sm">
            <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-brand/10 text-brand"><Ic className="size-4" /></span>
            <span className="min-w-0">
              <span className="block text-[13px] font-extrabold text-ink-1 group-hover:text-brand">{c.he}</span>
              <span className="block text-[11px] leading-snug text-ink-3">{c.desc}</span>
            </span>
          </Link>
        ); })}
      </div>
