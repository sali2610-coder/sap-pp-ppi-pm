...
function LinkChip({ text }: { text: string }) {
  return <SapTip name={text}><span className="tech rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-bold text-slate-600" dir="ltr">{text}</span></SapTip>;
}
...
function SectionView({ s }: { s: CenterSection }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" dir="rtl">
      <h2 className="mb-2.5 flex items-center gap-2 text-sm font-extrabold tracking-tight text-slate-900"><span className="size-2 rounded-full" style={{ background: tone }} />{s.title}</h2>
      <div className="text-[13px] leading-relaxed text-slate-600">
        ...
        {s.type === "chips" && <div className="flex flex-wrap gap-1.5">{(s.items || []).map((it) => <span key={it} className="tech rounded-lg border border-dashed border-slate-200 bg-white px-2.5 py-1 text-xs font-bold text-slate-500" dir="ltr">{it}</span>)}</div>}
  ...
  return (
    <div dir="rtl">
      <nav className="no-print mb-5 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-400">
        ...
        <Link href={base} className="hover:text-slate-700">{backLabel}</Link><ArrowRight className="size-3 rotate-180" />
        <span className="text-slate-600">{item.he}</span>
      </nav>
      <header className="relative mb-5 overflow-hidden rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
        ...
        <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">{item.he} · <span className="tech" dir="ltr">{item.title}</span></h1>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-500">{item.sub}</p>
      </header>
      ...
      {items.map((it) => (
        <Link key={it.slug} href={`${base}${it.slug}/`} className="lift card-premium group relative block overflow-hidden p-5">
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <h3 className="text-base font-extrabold tracking-tight text-slate-900">{it.he}</h3>
              <p className="tech text-xs font-bold text-slate-400" dir="ltr">{it.title}</p>
            </div>
          ...
          <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-slate-500">{it.sub}</p>
