      ...
      {list.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-2.5 lg:hidden">
            {list.map((r) => (
              <Link key={r.name} href={`/object/${encodeURIComponent(r.name)}/`} className="tap block rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm transition active:scale-[0.98]">
                ...
                {r.he && <p className="mt-1 text-[12.5px] leading-snug text-slate-600"><Highlight text={r.he} query={q} /></p>}
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold ${r.rel >= 6 ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{r.rel >= 6 && "★"}<GitBranch className="size-3" />{r.rel}</span>
                  ...
                  <ArrowLeft className="ms-auto size-4 text-slate-300" />
                </div>
                {r.s4 && <p className="mt-1.5 text-[11px] leading-snug text-slate-400">{r.s4}</p>}
              </Link>
            ))}
          </div>
