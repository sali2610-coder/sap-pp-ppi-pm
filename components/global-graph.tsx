...
export function GlobalGraph() {
  ...
  if (!g || !stats) return <div className="flex h-[60vh] items-center justify-center text-ink-3" dir="rtl">בונה את גרף הידע הגלובלי…</div>;
  ...
  return (
    <div className="mx-auto" dir="rtl">
      <header className="relative mb-3 overflow-hidden rounded-2xl bg-gradient-to-l from-slate-900 to-slate-800 px-5 py-3 text-white">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="relative w-full max-w-sm">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="חפש כל אובייקט — MATDOC · I_Product · MM01 · BAPI…" className="w-full rounded-xl border border-white/20 bg-surface/10 py-2 pe-3 ps-9 text-sm text-white placeholder:text-white/50 outline-none focus:bg-surface/20" />
            {searchHits.length > 0 && <div className="absolute z-50 mt-1 max-h-72 w-full overflow-auto rounded-xl border border-hairline bg-surface p-1 text-ink-1 shadow-2xl">{searchHits.map((n) => <button key={n.id} onClick={() => { recenter(n.id); setQ(""); }} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-right hover:bg-surface-2"><span className="size-2 rounded-full" style={{ background: colorFor(n) }} /><span className="tech flex-1 truncate font-mono text-sm font-bold" dir="ltr">{n.label}</span><span className="rounded bg-surface-2 px-1.5 text-[9px] font-bold text-ink-3">{KIND_META[n.kind].he}</span></button>)}</div>}
          </div>
        </div>
      </header>
      ...
      <div className="grid gap-3 lg:grid-cols-[1fr_320px]">
        {/* graph stage */}
        <div className="relative h-[calc(100vh-12rem)] min-h-[520px] overflow-hidden rounded-3xl border border-hairline bg-surface-2/60" style={{ backgroundImage: "radial-gradient(circle at 1px 1px,#d7deea 1px,transparent 0)", backgroundSize: "28px 28px", touchAction: "none" }} onWheel={onWheel} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={() => (drag.current = null)}>
          ...
          {nbrs.length > 24 && <div className="absolute bottom-3 start-3 rounded-lg bg-surface/90 px-2.5 py-1 text-[11px] font-bold text-ink-3 shadow-sm">מוצג 24 מתוך {nbrs.length} שכנים · התמקד בצומת לראות עוד</div>}
          {/* zoom dock */}
          <div className="absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1 rounded-2xl border border-white/60 bg-surface/90 p-1.5 shadow-xl backdrop-blur-md">
            {[[<ZoomOut key="o" className="size-4" />, () => setTr((p) => ({ ...p, k: Math.max(0.4, p.k / 1.2) }))], [<Scan key="f" className="size-4" />, () => setTr({ x: 0, y: 0, k: 1 })], [<ZoomIn key="i" className="size-4" />, () => setTr((p) => ({ ...p, k: Math.min(2.4, p.k * 1.2) }))]].map((b, i) => <button key={i} onClick={b[1] as () => void} className="grid size-9 place-items-center rounded-xl text-ink-2 transition hover:bg-surface-2">{b[0] as React.ReactNode}</button>)}
            <span className="px-2 font-mono text-xs font-bold text-ink-3">{Math.round(tr.k * 100)}%</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {cNode && (() => {
            ...
            return (
              <div className="overflow-hidden rounded-3xl border border-hairline bg-surface shadow-sm" key={cNode.id}>
                <div className="px-4 py-3 text-white" style={{ background: `linear-gradient(135deg,${colorFor(cNode)},${colorFor(cNode)}cc)` }}>
                  <div className="flex items-center gap-2"><span className="tech font-mono text-xl font-extrabold" dir="ltr">{nm}</span><span className="rounded-full bg-surface/20 px-2 py-0.5 text-[10px] font-bold ring-1 ring-white/25">{cNode.kind === "table" ? cNode.module : KIND_META[cNode.kind].he}</span></div>
                ...
                <div className="max-h-[52vh] space-y-2.5 overflow-y-auto p-4 text-[12px]">
                  {k && <div><div className="text-[10px] font-bold uppercase text-ink-3">מהות / מטרה</div><p className="text-ink-2">{k.role} {k.why}</p>{k.whenUsed && <p className="mt-0.5 text-ink-3">מתי: {k.whenUsed}</p>}</div>}
                  ...
                  <div className="flex flex-wrap gap-1.5 border-t border-hairline pt-2">
                    ...
                    {cNode.kind === "tcode" && <Link href={`/tcode/${encodeURIComponent(nm)}/`} className="rounded-lg bg-surface-2 px-2.5 py-1 text-[11px] font-bold text-ink-2 hover:bg-hairline">טרנזקציה</Link>}
          ...
          {/* executive layer */}
          <div className="rounded-3xl border border-hairline bg-surface p-4 shadow-sm">
            <div className="space-y-3 text-[12px]">
              {[["הכי מקושרים", Trophy, "#d97706", stats.topConnected], ["הכי בשימוש", Flame, "#0ea5e9", stats.mostUsed], ["הגירה קריטית", Boxes, "#16a34a", stats.critMig], ["סיכון גבוה", AlertTriangle, "#dc2626", stats.highRisk]].map(([label, Icon, c, ids]) => (
                <div key={label as string}>
                  <div className="mb-1 flex items-center gap-1.5 text-[11px] font-extrabold text-ink-2">{(() => { const I = Icon as React.ElementType; return <I className="size-3.5" style={{ color: c as string }} />; })()}{label as string}</div>
                  <div className="flex flex-wrap gap-1">{(ids as string[]).slice(0, 6).map((id) => { const n = g.nodes.get(id); if (!n) return null; return <button key={id} onClick={() => recenter(id)} className="tech rounded-md border border-hairline bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2 transition hover:border-brand/50 hover:text-brand">{n.label}</button>; })}</div>
                </div>
              ))}
            </div>
          </div>
