  ...
  return (
    <div dir="rtl">
      <div className="flex items-center gap-2 rounded-2xl border-2 border-hairline bg-surface px-4 py-3 shadow-sm focus-within:border-brand">
        ...
        <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder='שאל את NEO — "How do I replace MB1B in S/4?" · "tables for inspection lot" · "production confirmation"' className="w-full bg-transparent text-sm outline-none placeholder:text-ink-3" />
      </div>
      ...
      {!q && <div className="mt-3 flex flex-wrap gap-1.5">{["replace MB1B", "stock transfer", "inspection lot tables", "production confirmation", "backflush COGI"].map((s) => <button key={s} onClick={() => setQ(s)} className="rounded-lg border border-hairline bg-surface-2 px-2.5 py-1 text-xs font-bold text-ink-3 hover:border-brand hover:text-brand">{s}</button>)}</div>}

      {ans && (
        <div className="mt-5 space-y-4">
          {ans.lc && ans.tcode && (
            <section className="rounded-2xl border-2 border-brand/30 bg-surface p-5 shadow-sm">
          ...
          {ans.sols.length > 0 && (
            <section><h3 className="mb-2 text-sm font-extrabold text-ink-1">פתרונות עסקיים</h3><div className="flex flex-wrap gap-1.5">{ans.sols.map((s) => <Link key={s.slug} href={`/solutions/${s.slug}/`} className="rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700 hover:bg-amber-100">{s.he}</Link>)}</div></section>
          )}
          {ans.hits.length > 0 && (
            <section>
              <h3 className="mb-2 text-sm font-extrabold text-ink-1">מקורות מהמאגר ({ans.hits.length})</h3>
              <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
                {ans.hits.map((h) => <Link key={h.kind + h.code} href={h.href} className="lift flex items-center justify-between gap-2 rounded-xl border border-hairline bg-surface px-3 py-2 text-sm shadow-sm"><span className="min-w-0"><span className="tech font-bold text-ink-1" dir="ltr">{h.label}</span><span className="block truncate text-[10px] text-ink-3">{KIND_HE[h.kind]} · {h.sub}</span></span><span className="text-brand">→</span></Link>)}
              </div>
            </section>
          )}
          {ans.none && <p className="text-sm text-ink-3">לא נמצא במאגר NEO. נסה מונח אחר (T-Code, טבלה, תהליך) — אינני עונה מזיכרון כללי.</p>}
        </div>
      )}
    </div>
  );
