      ...
      {det && (
        <>
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <Card title="User Exits / BAdIs" icon={<Cable className="size-4" />}><div className="space-y-2"><div><p className="eyebrow mb-1 text-ink-3">User Exits</p><div className="flex flex-wrap gap-1.5">{det.exits.map((e) => <Chip key={e} text={e} tone={c} />)}</div></div><div><p className="eyebrow mb-1 text-ink-3">BAdIs</p><div className="flex flex-wrap gap-1.5">{det.badis.map((b) => <Chip key={b} text={b} tone={c} />)}</div></div></div></Card>
          </div>
