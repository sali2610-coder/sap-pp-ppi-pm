          ...
          {rows.map((i) => (
            <Link key={i.slug} href={`/troubleshooting/${i.slug}/`} className="lift group rounded-2xl border border-hairline bg-surface p-4 shadow-sm">
              <div className="mb-1.5 flex items-center gap-2">
                <span className="grid size-7 place-items-center rounded-lg bg-brand-soft"><ShieldAlert className="size-4 text-red-500" /></span>
              ...
              {i.error && i.error !== "—" && <p className="tech mt-1.5 rounded bg-brand-soft px-2 py-0.5 text-[10px] font-bold text-brand" dir="ltr">{i.error}</p>}
            </Link>
          ))}
        </div>
      )}
