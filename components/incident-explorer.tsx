    ...
    <div dir="rtl">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex flex-1 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 shadow-sm">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="חיפוש תקלה · תסמין · קוד שגיאה · T-Code…" className="w-full bg-transparent text-sm outline-none placeholder:text-slate-400" />
        </div>
        <div className="flex flex-wrap gap-1.5">
          {MODULES.map((m) => <button key={m} onClick={() => setMod(m)} className={`rounded-xl px-3 py-2 text-xs font-bold transition ${mod === m ? "text-white shadow-sm" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`} style={mod === m ? { background: m === "ALL" ? "#d62027" : MOD_COLOR[m] } : undefined}>{MOD_HE[m]}</button>)}
        </div>
      </div>
      <p className="mb-2 text-xs font-bold text-slate-400">{rows.length} תקלות</p>
      <div className="grid gap-3 sm:grid-cols-2">
        {rows.map((i) => (
          <Link key={i.slug} href={`/troubleshooting/${i.slug}/`} className="lift group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-1.5 flex items-center gap-2">
              ...
              <ArrowLeft className="ms-auto size-3.5 text-slate-300 transition group-hover:-translate-x-1 group-hover:text-brand" />
            </div>
            <h3 className="text-sm font-extrabold tracking-tight text-slate-900">{i.he}</h3>
            <p className="mt-1 line-clamp-2 text-[12px] leading-snug text-slate-500">{i.symptom}</p>
