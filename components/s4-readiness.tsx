...
export function S4Readiness() {
  ...
  if (!tables) return <div className="flex h-[60vh] items-center justify-center text-slate-400" dir="rtl">מחשב מוכנות S/4…</div>;
  ...
  const Mini = ({ title, icon, rows, metric }: { title: string; icon: React.ReactNode; rows: ModuleReadiness[]; metric: (m: ModuleReadiness) => string }) => (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="mb-3 flex items-center gap-2 text-base font-extrabold text-slate-900">{icon}{title}</h3>
      <div className="space-y-1.5">{rows.map((m, i) => (
        <button key={m.mod} onClick={() => setSel(m.mod)} className="flex w-full items-center gap-2.5 rounded-xl bg-slate-50/60 px-3 py-2 text-right transition hover:bg-slate-100">
          <span className="grid size-6 shrink-0 place-items-center rounded-full bg-slate-900 text-[10px] font-bold text-white">{i + 1}</span>
          <span className="flex-1 text-[13px] font-extrabold text-slate-800">{m.he} <span className="font-mono text-[11px] text-slate-400">{m.mod}</span></span>
  ...
  return (
    <div className="mx-auto" dir="rtl">
      ...
      <header className="relative overflow-hidden rounded-[1.75rem] bg-gradient-to-l from-slate-900 via-slate-800 to-[#0f766e] p-6 text-white shadow-xl sm:p-8">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-[12px] font-bold">{Object.entries(BAND_HE).map(([k, v]) => <span key={k} className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-2.5 py-0.5"><span className="size-2 rounded-full" style={{ background: v.c }} />{v.he} · {bandCounts[k] || 0}</span>)}</div>
          </div>
        ...
        {mods.map((m) => { const band = BAND_HE[m.band]; return (
          <button key={m.mod} onClick={() => setSel(m.mod)} className={`group rounded-3xl border bg-white p-4 text-right shadow-sm transition hover:-translate-y-1 hover:shadow-lg ${sel === m.mod ? "border-emerald-300 ring-1 ring-emerald-200" : "border-slate-200"}`}>
            <div className="flex items-start justify-between gap-2">
              <div><div className="font-mono text-lg font-extrabold text-slate-900">{m.mod}</div><div className="text-[12px] font-bold text-slate-500">{m.he}</div></div>
            ...
            <div className="mt-2 grid grid-cols-3 gap-1 text-center text-[10px] font-bold text-slate-400">
              <div><div className="text-[13px] tabular-nums text-slate-700">{m.fioriPct}%</div>Fiori</div>
              <div><div className="text-[13px] tabular-nums text-slate-700">{m.cdsPct}%</div>CDS</div>
              <div><div className="text-[13px] tabular-nums text-slate-700">{m.tables}</div>טבלאות</div>
            </div>
            <div className="mt-2 flex items-center justify-between border-t border-slate-100 pt-2 text-[10px] font-bold">
              <span className="rounded px-1.5 py-0.5 text-white" style={{ background: RISK_C[m.risk] }}>{RISK_HE[m.risk]}</span>
              <span className="text-slate-400">מורכבות {m.complexity} · {m.effort}</span>
            </div>
          </button>
        ); })}
      </div>
      ...
      {/* executive dashboard */}
      <h2 className="mb-3 mt-7 flex items-center gap-2 text-xl font-extrabold text-slate-900"><TrendingUp className="size-5 text-emerald-600" />לוח הנהלה</h2>
      ...
      {o && (
        <div className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm" style={{ animation: "fadeUp .2s ease both" }}>
          ...
          <div className="grid gap-4 p-6 lg:grid-cols-2">
            ...
            <div>
              <h3 className="mb-2 flex items-center gap-1.5 text-sm font-extrabold text-slate-900"><Network className="size-4" />ארכיטקטורת יעד · {o.mod} → S/4</h3>
              ...
              <p className="mt-2 text-[12px] text-slate-500">דוגמה: {o.mod} → CDS → Fiori → {o.band === "Cloud Ready" ? "Datasphere/SAC" : "Embedded Analytics + MATDOC/ACDOCA"}.</p>
            </div>
            {/* factors + deep links */}
            <div>
              <h3 className="mb-2 flex items-center gap-1.5 text-sm font-extrabold text-slate-900"><ShieldCheck className="size-4" />גורמי מוכנות</h3>
              <div className="space-y-1.5 text-[13px]">
                {[["כיסוי Fiori", o.fioriPct], ["כיסוי CDS", o.cdsPct], ["איתות S/4 (impact)", o.s4Pct]].map(([l, v]) => (
                  <div key={l as string} className="flex items-center gap-2"><span className="w-28 shrink-0 font-bold text-slate-600">{l}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full" style={{ width: `${v}%`, background: o.color }} /></div><span className="w-9 text-left text-[12px] font-extrabold tabular-nums text-slate-500">{v}%</span></div>
                ))}
                <div className="flex flex-wrap gap-x-4 gap-y-1 pt-1 text-[12px] font-bold text-slate-500"><span>אובייקטי הגירה: {o.migrationObjs}</span><span>Simplification: {o.simplification}</span><span>השפעת קוד: {o.customCodeImpact}</span></div>
              </div>
              <div className="mt-3 flex flex-wrap gap-1.5 border-t border-slate-100 pt-3">
                <Link href="/s4hana/" className="rounded-lg bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-600 transition hover:bg-slate-200">מרכז S/4</Link>
      ...
      <p className="py-6 text-center text-[11px] text-slate-400">הציונים מחושבים בזמן ריצה ממטא-דאטה (Fiori/CDS/S4_IMPACT/catalog/migration/landscape) — לא אחוזים קשיחים. <Boxes className="inline size-3" /></p>
    </div>
  );
}
