...
import { onTint, onTintDark, forWhiteText } from "@/lib/contrast";
  ...
  return (
    <div dir="rtl">
      ...
      <div className="mb-2 flex flex-wrap items-center gap-1.5">
        ...
        {modules.map((m) => { const on = mod === m; const col = mc(m); return <button key={m} onClick={() => setMod(on ? "" : m)} className={`tint-ink rounded-lg px-2.5 py-1 text-[11px] font-bold transition`} style={on ? { background: forWhiteText(col), color: "#fff" } : { background: col + "14", "--tint-ink-light": onTint(col, 0x14 / 255), "--tint-ink-dark": onTintDark(col, 0x14 / 255) } as React.CSSProperties}>{m} · {stats.byModule[m]}</button>; })}
      </div>
      {facets.topics.length > 0 && (
        <div className="mb-2 flex flex-wrap items-center gap-1.5">
          ...
          {facets.topics.map((t) => { const on = topic === t; return <button key={t} onClick={() => setTopic(on ? "" : t)} className={`rounded-lg px-2.5 py-1 text-[11px] font-bold transition ${on ? "bg-brand text-brand-foreground" : "bg-surface-2 text-ink-3 hover:bg-hairline"}`}>{t}</button>; })}
        </div>
      )}
      {facets.objects.length > 0 && (
        <div className="mb-4 flex flex-wrap items-center gap-1.5">
          ...
          {facets.objects.map((o) => { const on = obj === o; return <button key={o} onClick={() => setObj(on ? "" : o)} className={`rounded-lg px-2.5 py-1 text-[11px] font-bold transition ${on ? "bg-brand text-brand-foreground" : "bg-surface-2 text-ink-3 hover:bg-hairline"}`}>{o}</button>; })}
        </div>
      )}
              ...
              <Link href={t.href} className="block">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5"><span className="font-mono text-[15px] font-extrabold text-ink-1" dir="ltr">{t.code}</span><span className="tint-ink rounded px-1.5 py-0.5 text-[9px] font-bold" style={{ background: col + "1a", "--tint-ink-light": onTint(col), "--tint-ink-dark": onTintDark(col) } as React.CSSProperties}>{t.module}</span></div>
                    <div className="truncate text-[11px] font-bold text-ink-3">{t.he}</div>
                  </div>
                </div>
                <div className="mt-2 flex flex-wrap items-center gap-1.5">
                  {deep ? <span className="tint-ink rounded-md bg-brand/10 px-1.5 py-0.5 text-[10px] font-bold" style={{ "--tint-ink-light": onTint("#d62027"), "--tint-ink-dark": "var(--brand)" } as React.CSSProperties}>מתועד לעומק</span> : <span className="rounded-md bg-surface-2 px-1.5 py-0.5 text-[10px] font-bold text-ink-3">מאומת</span>}
