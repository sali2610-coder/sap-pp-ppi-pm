...
export function VerifiedObjectView({ o }: { o: VerifiedObject }) {
  ...
  const tcodeChip = (code: string) => hasApp(code)
    ? <Link key={code} href={`/apps/${encodeURIComponent(code)}/`} className="tech rounded-md bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2 transition hover:bg-brand/10 hover:text-brand" dir="ltr">{code}</Link>
    : <Link key={code} href={`/tcode/${encodeURIComponent(code)}/`} className="tech rounded-md bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2 transition hover:bg-brand/10 hover:text-brand" dir="ltr">{code}</Link>;

  return (
    <div dir="rtl" className="mx-auto max-w-[1200px] space-y-4 pb-10">
      <div className="flex items-center gap-2 text-xs text-ink-3"><Link href="/apps/" className="hover:text-brand">מרכז אובייקטים</Link><ArrowLeft className="size-3" /><span className="font-bold text-ink-2">{o.name}</span></div>
      ...
      {o.ppPi && (
        <Section icon={<FlaskConical className="size-5" />} title="חיבור לזרימת PP-PI (ייצור תהליכי)" accent="#6d28d9">
          <p className="text-[13.5px] leading-relaxed text-ink-2">{o.ppPi}</p>
          <div className="mt-3 flex flex-wrap items-center gap-2 text-[12px]">
            ...
            <Link href="/studio/" className="inline-flex items-center gap-1 rounded-lg bg-surface-2 px-2.5 py-1 font-bold text-ink-2 transition hover:text-brand">מפת הקשרים · Studio<ArrowLeft className="size-3.5" /></Link>
          </div>
        </Section>
      )}
      ...
      {domain && (
        <Section icon={<Network className="size-5" />} title={`שכבת לוגיסטיקה משולבת · ${domain.he} (${domain.id})`} accent="#0e7490">
          <p className="text-[13px] leading-relaxed text-ink-2">{domain.summary}</p>
          <p className="mt-1.5 text-[11px] font-bold text-ink-3">רכיב SAP: {domain.component}</p>
          ...
          <p className="mt-3 text-[11px] text-ink-3">LO-HU שייך ל-Logistics General — מוצג כשכבה משולבת, לא כטבלת PP-PI מזויפת.</p>
        </Section>
      )}
      ...
      {o.useCases && o.useCases.length > 0 && (
        <Section icon={<Lightbulb className="size-5" />} title="שימושי יועץ נפוצים" accent="#d97706">
          <ul className="space-y-1.5">{o.useCases.map((u, i) => <li key={i} className="flex gap-2 text-[13px] leading-relaxed text-ink-2"><span className="mt-1.5 size-1.5 shrink-0 rounded-full bg-amber-500" />{u}</li>)}</ul>
        </Section>
      )}
      ...
      <Section icon={<ArrowRightLeft className="size-5" />} title="ECC ↔ S/4HANA" accent="#2563eb">
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-2xl border border-hairline bg-surface-2/60 p-4"><div className="text-[10px] font-bold uppercase text-ink-3">ECC</div><p className="mt-1 text-[13px] leading-relaxed text-ink-2">{o.ecc}</p></div>
          <div className="rounded-2xl border border-hairline bg-surface-2/60 p-4"><div className="text-[10px] font-bold uppercase text-green-600">S/4HANA</div><p className="mt-1 text-[13px] leading-relaxed text-ink-2">{o.s4}</p></div>
        </div>
      </Section>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {o.related.length > 0 && (
          <Section icon={<GitBranch className="size-5" />} title="אובייקטים קשורים" accent={c}>
            <div className="flex flex-wrap gap-1.5">{o.related.map((r) => { const rv = verifiedObject(r); return <Link key={r} href={`/object/${encodeURIComponent(r)}/`} className="tech rounded-md bg-surface-2 px-2 py-0.5 font-mono text-[12px] font-bold text-ink-2 transition hover:bg-brand/10 hover:text-brand" dir="ltr" title={rv?.en}>{r}</Link>; })}</div>
          </Section>
        )}
      ...
      <Section icon={<Tag className="size-5" />} title="שמות וכינויים לחיפוש" accent="#0891b2">
        <div className="flex flex-wrap gap-1.5">
          {[o.name, o.en, ...o.aliases].map((a, i) => <span key={i} className="rounded-full bg-surface-2 px-2.5 py-0.5 text-[11.5px] font-bold text-ink-2">{a}</span>)}
        </div>
      </Section>

      {/* honesty / source footer */}
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-hairline bg-surface-2/60 p-4 text-[12px] text-ink-3">
        <Layers className="size-4 text-ink-3" />
        <span>מקור: <b className="text-ink-2">רפרנס SAP מאומת</b> · אובייקט תקני מחוץ לבלוּפרינט PM/PP-PI (לוגיסטיקה/מלאי/מכירות/פיננסי).</span>
        <span className="inline-flex items-center gap-1 text-ink-3"><CircleAlert className="size-3.5" />שדות שאינם מאומתים אינם מוצגים — ללא המצאה.</span>
      </div>
    </div>
  );
}
