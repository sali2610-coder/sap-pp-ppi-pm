...
function Chip({ name, kind }: { name: string; kind: "table" | "cds" | "bapi" | "tcode" | "raw" }) {
  ...
  const cls = "tech rounded-md bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink-2 transition hover:bg-brand/10 hover:text-brand";
  ...
  return (
    <div dir="rtl" className="mx-auto space-y-4 pb-10">
      ...
      <div className="flex items-center gap-2 text-xs text-ink-3">
        ...
        <span className="font-bold text-ink-2">{o.code}</span>
      </div>
      ...
      <section id="identity" className="relative overflow-hidden rounded-[2rem] p-6 text-white shadow-[0_30px_60px_-24px_rgba(15,23,42,0.5)] sm:p-8" style={{ background: `linear-gradient(135deg, ${c}, ${c}cc 60%, #0f172a)` }}>
        ...
        <div className="relative">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[12px] font-bold">
            <span className="inline-flex items-center gap-1.5 rounded-xl bg-surface/15 px-3 py-1.5 backdrop-blur"><Terminal className="size-3.5" />SAP GUI {o.lc.ecc ? "· ECC" : ""} {o.lc.s4 ? "· S/4" : ""}</span>
            {fiori && <span className="inline-flex items-center gap-1.5 rounded-xl bg-surface/15 px-3 py-1.5 backdrop-blur"><LayoutGrid className="size-3.5" />Fiori: {fiori.app}</span>}
          </div>
        </div>
      </section>
      ...
      {(fiori || t.fiori) && (
        <Section id="successor" icon={<Route className="size-5" />} title="היורש המומלץ" sub="מסלול האבולוציה של האובייקט" accent={c}>
          <div className="flex flex-wrap items-center gap-3">
            <div className="rounded-2xl border-2 border-hairline bg-surface-2 px-4 py-3 text-center"><div className="text-[10px] font-bold text-ink-3">ECC GUI</div><div className="tech text-lg font-extrabold text-ink-1" dir="ltr">{o.code}</div></div>
            <ArrowLeft className="size-5 text-ink-3" />
            <div className="rounded-2xl border-2 px-4 py-3 text-center" style={{ borderColor: STATUS_C[o.lc.status] + "55", background: STATUS_C[o.lc.status] + "0c" }}><div className="text-[10px] font-bold text-ink-3">S/4HANA</div><div className="text-[13px] font-extrabold" style={{ color: STATUS_C[o.lc.status] }}>{o.lc.s4 ? (o.lc.alt || o.code) : "הוסר"}</div></div>
            {(fiori || t.fiori) && <><ArrowLeft className="size-5 text-ink-3" /><div className="rounded-2xl border-2 border-amber-200 bg-amber-50 px-4 py-3 text-center"><div className="text-[10px] font-bold text-amber-600">Fiori</div><div className="text-[13px] font-extrabold text-amber-700">{fiori?.app || t.fiori}</div>{fiori?.appId && <div className="tech text-[10px] font-bold text-amber-500" dir="ltr">{fiori.appId}</div>}</div></>}
          </div>
          {o.fiori.length > 1 && <div className="mt-3 flex flex-wrap gap-1.5"><span className="text-[10px] font-bold uppercase text-ink-3">אפליקציות נוספות:</span>{o.fiori.slice(1).map((f) => <span key={f.appId} className="rounded-md bg-amber-50 px-1.5 py-0.5 text-[11px] font-bold text-amber-700">{f.app} ({f.appId})</span>)}</div>}
        </Section>
      )}
          ...
          <div className="flex flex-wrap items-center gap-2">
            {flow.map((step, i) => { const hot = step.toUpperCase().includes(o.code); return (
              <div key={i} className="flex items-center gap-2">
                <span className={`rounded-xl border-2 px-3 py-2 text-[12px] font-bold ${hot ? "text-white shadow-sm" : "border-hairline bg-surface-2 text-ink-2"}`} style={hot ? { background: c, borderColor: c } : undefined}>{step}</span>
                {i < flow.length - 1 && <ArrowLeft className="size-4 shrink-0 text-ink-3" />}
              </div>
            ); })}
          </div>
        </Section>
        ...
        <Section id="fiori" icon={<LayoutGrid className="size-5" />} title="Fiori" sub={fiori.app} accent="#d97706">
          <div className="grid gap-2.5 sm:grid-cols-2">
            {[["App ID", fiori.appId], ["App Name", fiori.app], ["Business Role", fiori.role], ["Business Catalog", fiori.catalog], ["OData Service", fiori.odata], ["CDS Source", fiori.cds], ["Backend (GUI)", fiori.gui.join(" / ")]].filter(([, v]) => v).map(([k, v]) => (
              <div key={k} className="rounded-xl border border-hairline bg-surface-2/60 p-3"><div className="text-[10px] font-bold uppercase text-ink-3">{k}</div><div className="tech mt-0.5 break-words text-[13px] font-bold text-ink-1" dir="ltr">{v}</div></div>
            ))}
          </div>
        </Section>
      )}
      ...
      {t.authObjects?.length ? (
        <Section id="auth" icon={<ShieldCheck className="size-5" />} title="הרשאות" sub="אובייקטי הרשאה + תקלות נפוצות" accent="#dc2626">
          ...
          <p className="mt-3 flex gap-1.5 text-[12px] leading-relaxed text-ink-3"><AlertTriangle className="mt-0.5 size-3.5 shrink-0 text-amber-500" />כשל הרשאה אופייני: בדוק SU53 מיד אחרי השגיאה, ואז STAUTHTRACE לאיתור האובייקט החסר. ודא הקצאת תפקיד (PFCG){fiori ? ` + Business Catalog ${fiori.catalog || ""} ל-Fiori` : ""}.</p>
        </Section>
      ) : null}
      ...
      {t.commonErrors?.length ? (
        <Section id="errors" icon={<AlertTriangle className="size-5" />} title="שגיאות נפוצות" sub="תסמין · גורם · OSS" accent="#dc2626">
          <div className="space-y-1.5">
            {t.commonErrors.map((e) => <div key={e} className="flex items-start gap-2 rounded-xl border border-hairline bg-surface-2/60 p-2.5 text-[12.5px] text-ink-2"><AlertTriangle className="mt-0.5 size-3.5 shrink-0 text-rose-500" />{e}</div>)}
          </div>
          {t.mistakes?.length ? <div className="mt-3"><div className="mb-1 text-[10px] font-bold uppercase text-ink-3">טעויות נפוצות</div><div className="flex flex-wrap gap-1.5">{t.mistakes.map((m) => <span key={m} className="rounded-md bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-700">{m}</span>)}</div></div> : null}
          {t.ossKeywords?.length ? <div className="mt-3 flex flex-wrap items-center gap-1.5"><span className="text-[10px] font-bold uppercase text-ink-3">חיפוש OSS:</span>{t.ossKeywords.map((k) => <span key={k} className="tech rounded bg-surface-2 px-1.5 py-0.5 text-[10.5px] font-bold text-ink-3" dir="ltr">{k}</span>)}</div> : null}
        </Section>
      ) : null}
      ...
      <Section id="migration" icon={<RefreshCw className="size-5" />} title="מיגרציה ECC → S/4HANA" accent="#2563eb">
        ...
        <p className="mt-2 flex gap-1.5 text-[12.5px] leading-relaxed text-ink-2"><Route className="mt-0.5 size-3.5 shrink-0 text-blue-500" />{o.lc.migration}</p>
        {t.s4 && <p className="mt-2 text-[12.5px] leading-relaxed text-ink-3">{t.s4}</p>}
      </Section>
        ...
        <Section id="compare" icon={<GitCompare className="size-5" />} title="השוואה — ECC מול Fiori" sub={`${o.code} מול ${fiori.app}`} accent="#0f172a">
          ...
          <AnimatePresence>{compare && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="grid gap-px overflow-hidden rounded-2xl border border-hairline bg-hairline text-[12px] sm:grid-cols-[140px_1fr_1fr]">
                {[
                  ...
                  <div key={i} className="contents">
                    <div className="bg-surface-2 p-2.5 font-bold text-ink-3">{row[0]}</div>
                    <div className="bg-surface p-2.5 text-ink-2">{row[1]}</div>
                    <div className="bg-amber-50/40 p-2.5 text-ink-2">{row[2]}</div>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      <Section id="ai" icon={<Sparkles className="size-5" />} title="עוזר חכם" sub="הסבר מותאם — ממאגר הידע המאומת בלבד, ללא הזיות" accent="#0f172a">
        <div className="mb-3 flex flex-wrap gap-1.5">
          {PERSONAS.map((p) => { const Ic = p.icon; const on = persona === p.id; return (
            <button key={p.id} onClick={() => setPersona(p.id)} className={`tap inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-[12px] font-bold transition ${on ? "text-white shadow-sm" : "bg-surface-2 text-ink-3 hover:bg-hairline"}`} style={on ? { background: p.c } : undefined}><Ic className="size-3.5" />{p.he}</button>
          ); })}
        </div>
        <AnimatePresence mode="wait">
          <motion.div key={persona} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}
            className="rounded-2xl border border-hairline bg-surface-2/60 p-4 text-[13px] leading-relaxed text-ink-2">
            {personaText || "אין מידע ייעודי לפרסונה זו."}
          </motion.div>
        </AnimatePresence>
        <p className="mt-2 flex items-center gap-1 text-[10.5px] text-ink-3"><CheckCircle2 className="size-3 text-green-500" />כל התשובות נגזרות ישירות מהמטא-דאטה של {o.code} — לא מזיכרון AI כללי.</p>
      </Section>
...
function QA({ icon, c, label, items }: { icon: React.ReactNode; c: string; label: string; items: string[] }) {
  return (
    <div className="rounded-2xl border border-hairline p-4">
      ...
      <ul className="space-y-1">{items.map((x, i) => <li key={i} className="flex gap-1.5 text-[12px] leading-relaxed text-ink-2"><span className="mt-1.5 size-1 shrink-0 rounded-full" style={{ background: c }} />{x}</li>)}</ul>
    </div>
  );
}
