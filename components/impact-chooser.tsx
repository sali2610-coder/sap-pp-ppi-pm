...
export function ImpactChooser() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5" dir="rtl">
      ...
      <div className="relative">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="חפש טבלה (AUFK, QMEL, MARA…) או תיאור" className="w-full rounded-2xl border border-hairline bg-surface py-3 pe-4 ps-10 text-sm font-medium shadow-sm outline-none focus:border-brand/40" />
        <Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-ink-3" />
      </div>

      {!q && <p className="text-xs font-bold text-ink-3">אובייקטי ליבה לניתוח מהיר:</p>}
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        {rows.length === 0 && <p className="col-span-full py-6 text-center text-sm text-ink-3">לא נמצאו תוצאות.</p>}
      </div>
    </div>
  );
}
