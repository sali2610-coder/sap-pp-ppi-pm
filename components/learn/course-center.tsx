...
function ListUl({ items, color }: { items: string[]; color: string }) {
  return <ul className="space-y-1">{items.map((x, i) => <li key={i} className="flex gap-1.5 text-[13px] leading-relaxed text-ink-2"><span className="mt-1.5 size-1.5 shrink-0 rounded-full" style={{ background: color }} />{x}</li>)}</ul>;
}
const Empty = () => <p className="text-[12px] text-ink-3">— אין תוכן לנושא זה בלשונית זו</p>;
...
export function CourseCenter({ config }: { config: CourseConfig }) {
    ...
    <div id="topic-workspace" className="scroll-mt-4 space-y-3">
      <div className="flex flex-wrap gap-1.5">
        {config.topics.map((x) => { const on = x.id === sel; const d = done.includes(x.id); return (
          <button key={x.id} onClick={() => pick(x.id)} className={`flex items-center gap-1 rounded-xl border-2 px-2.5 py-1 text-[12px] font-bold transition ${on ? "text-white" : "border-hairline bg-surface text-ink-2 hover:border-hairline"}`} style={on ? { background: x.color, borderColor: x.color } : undefined}>{d && <span className="size-1.5 rounded-full bg-emerald-400" />}{x.he}</button>
        ); })}
      </div>
        ...
        <div className="p-4">
          ...
          <div key={a.id + tab} className="mt-4" style={{ animation: "fadeUp .2s ease both" }}>
            ...
            {tab === "visual" && <div className="space-y-3">
              {a.visual?.layers?.length ? <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{a.visual.layers.map((l, i) => (
                <div key={i} className="rounded-xl border-2 p-3 transition" style={{ borderColor: l.hot ? c : "#e2e8f0", background: l.hot ? c + "0d" : "#fff", opacity: l.hot ? 1 : 0.55 }}>
                  ...
                  <ul className="mt-1 space-y-0.5">{l.items.map((it, j) => <li key={j} className="text-[11px] text-ink-3">• {it}</li>)}</ul>
                </div>
              ))}</div> : null}
              ...
              {a.visual?.note && <p className="text-[12px] text-ink-3">{a.visual.note}</p>}
              {!a.visual?.layers?.length && !a.visual?.flow?.length && !a.visual?.note && <Empty />}
            </div>}

            {tab === "examples" && <div className="space-y-2">
              {a.examples?.scenario && <div className="rounded-xl border border-hairline bg-surface-2/60 p-3"><div className="mb-1 text-[11px] font-bold uppercase text-ink-3">תרחיש טיפוסי</div><p className="text-[13px] leading-relaxed text-ink-2">{a.examples.scenario}</p></div>}
            ...
            {tab === "transactions" && <div className="space-y-3">
              {a.transactions?.tcodes?.length ? <div><div className="eyebrow mb-1 text-ink-3">טרנזקציות</div><Chips items={a.transactions.tcodes} color={c} hrefBase="/tcode/" /></div> : null}
              {a.transactions?.tables?.length ? <div><div className="eyebrow mb-1 text-ink-3">טבלאות</div><Chips items={a.transactions.tables} color="#64748b" hrefBase="/object/" /></div> : null}
              {a.transactions?.tools?.length ? <div><div className="eyebrow mb-1 text-ink-3">כלים</div><Chips items={a.transactions.tools} color="#0891b2" /></div> : null}
            ...
            {tab === "related" && <div className="space-y-3">
              {a.related?.length ? <div className="flex flex-wrap gap-1.5">{a.related.map((l) => l.href.startsWith("#") ? <span key={l.label} className="rounded-lg bg-surface-2 px-2.5 py-1 text-[11px] font-bold text-ink-3">{l.label}</span> : <Link key={l.label} href={l.href} className="rounded-lg bg-surface-2 px-2.5 py-1 text-[11px] font-bold text-ink-2 hover:bg-hairline">{l.label}</Link>)}</div> : null}
              <div className="flex flex-wrap gap-1.5 border-t border-hairline pt-2">{config.crossLinks.map((l) => <Link key={l.href} href={l.href} className="rounded-lg bg-surface-2 px-2.5 py-1 text-[11px] font-bold text-ink-2 hover:bg-hairline">{l.label}</Link>)}</div>
            </div>}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-4" dir="rtl">
      ...
      {!focus && <p className="pb-4 text-center text-[11px] text-ink-3">קורס אינטראקטיבי · ידע SAP סטנדרטי · trust: curated. ההתקדמות נשמרת מקומית.<Link href="/knowledge/" className="ms-1 font-bold text-brand">כל המרכזים</Link></p>}
    </div>
  );
}
