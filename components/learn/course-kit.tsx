      ...
      <div className="grid-adaptive">
        {items.map((c) => (
          <button key={c.id} onClick={() => onPick(c.id)} className="group flex items-start gap-3 rounded-2xl border-2 border-slate-200 bg-white p-4 text-right transition hover:-translate-y-0.5 hover:shadow-lg" style={{ borderColor: c.color + "33" }}>
            ...
            <span className="min-w-0"><span className="block text-[14px] font-extrabold text-slate-900 group-hover:text-brand">{c.he}</span><span className="block text-[12px] leading-snug text-slate-500">{c.sub}</span></span>
          </button>
        ))}
      </div>
    </section>
...
export function LearningLadder({ levels, doneIds, onPick }: { levels: LadderLevel[]; doneIds: string[]; onPick: (id: string) => void }) {
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6" dir="rtl">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400"><GraduationCap className="size-4 text-brand" />מסלול למידה · Beginner → Expert</div>
        <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-extrabold text-emerald-600">{done}/{total} · {pct}%</span>
      </div>
      <div className="mb-4 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-l from-emerald-400 to-emerald-500 transition-all" style={{ width: `${pct}%` }} /></div>
      <div className="grid gap-3 lg:grid-cols-4">
        {levels.map((lv, i) => { const ld = lv.topics.filter((t) => doneIds.includes(t.id)).length; return (
          <div key={lv.id} className="rounded-2xl border-2 p-3" style={{ borderColor: lv.color + "44" }}>
            <div className="mb-1.5 flex items-center justify-between"><span className="text-[13px] font-extrabold" style={{ color: lv.color }}>{i + 1}. {lv.label}</span><span className="text-[10px] font-bold text-slate-400">{ld}/{lv.topics.length}</span></div>
            <ul className="space-y-1">{lv.topics.map((t) => { const d = doneIds.includes(t.id); return (
              <li key={t.id}><button onClick={() => onPick(t.id)} className="flex w-full items-center gap-1.5 rounded-lg px-1.5 py-1 text-right text-[12px] font-bold text-slate-600 transition hover:bg-slate-50">
                {d ? <Check className="size-3.5 shrink-0 text-emerald-500" /> : <Circle className="size-3.5 shrink-0 text-slate-300" />}<span className="min-w-0 flex-1 truncate">{t.he}</span></button></li>
            ); })}</ul>
          </div>
        ); })}
      </div>
...
export function TopicTabs({ tabs, active, onChange }: { tabs: TopicTab[]; active: string; onChange: (id: string) => void }) {
  return (
    <div className="flex snap-x gap-1 overflow-x-auto rounded-2xl border border-slate-200 bg-white/90 p-1 shadow-sm backdrop-blur [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" dir="rtl">
      {tabs.map((t) => (
        <button key={t.id} onClick={() => onChange(t.id)} className={`flex min-h-[40px] shrink-0 snap-start items-center gap-1.5 rounded-xl px-3 py-1.5 text-[13px] font-bold transition active:scale-95 ${active === t.id ? "bg-brand text-white shadow-sm" : "text-slate-500 hover:bg-slate-100"}`}>{t.icon}{t.label}</button>
      ))}
    </div>
  );
}
...
export function TopicIntro({ fields }: { fields: IntroField[] }) {
  return (
    <div className="grid gap-2.5 sm:grid-cols-2" dir="rtl">
      {fields.map((f) => f.text ? (
        <div key={f.label} className="rounded-xl border border-slate-100 p-3" style={{ background: f.color + "08" }}>
          <div className="mb-1 text-[11px] font-bold uppercase tracking-wide" style={{ color: f.color }}>{f.label}</div>
          <p className="text-[13px] leading-relaxed text-slate-700">{f.text}</p>
        </div>
      ) : null)}
    </div>
  );
...
export function CompletionChecklist({ course, topicId, items }: { course: string; topicId: string; items: string[] }) {
  ...
  return (
    <div className="rounded-2xl border-2 border-emerald-200 bg-emerald-50/40 p-4" dir="rtl">
      ...
      <ul className="space-y-1.5">{items.map((x, i) => <li key={i} className="flex gap-2 text-[13px] leading-relaxed text-slate-700"><Check className="mt-0.5 size-4 shrink-0 text-emerald-500" />{x}</li>)}</ul>
      <button onClick={() => toggle(topicId)} className={`mt-3 inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-[12px] font-extrabold transition active:scale-95 ${allDone ? "bg-emerald-500 text-white" : "bg-white text-emerald-700 ring-1 ring-emerald-300 hover:bg-emerald-50"}`}>
    ...
    <div className="space-y-3" dir="rtl">
      ...
      {interview && interview.length > 0 && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="mb-1.5 flex items-center gap-1.5 text-[12px] font-extrabold text-slate-700"><GraduationCap className="size-4" />שאלות ראיון טיפוסיות</div>
          <ul className="space-y-1.5">{interview.map((q, i) => <li key={i} className="flex gap-2 text-[13px] leading-relaxed text-slate-700"><ChevronLeft className="mt-0.5 size-4 shrink-0 text-brand" />{q}</li>)}</ul>
        </div>
      )}
    </div>
  );
...
export function FocusToggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button onClick={onToggle} className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-[12px] font-bold transition active:scale-95 ${on ? "bg-brand text-white" : "bg-white text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50"}`}>
