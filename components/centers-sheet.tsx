      ...
      {open && (
        <>
          ...
          <motion.div dir="rtl" role="dialog" aria-label="מרכזי ייעוץ"
            className="fixed inset-x-0 bottom-0 z-[81] max-h-[88vh] overflow-y-auto rounded-t-3xl bg-surface pb-[max(env(safe-area-inset-bottom),1rem)] shadow-2xl xl:hidden"
            ...
            <div className="sticky top-0 z-10 bg-surface/95 backdrop-blur">
              ...
              <div className="flex items-center justify-between px-5 py-3">
                <span className="flex items-center gap-2 text-[13px] font-extrabold text-ink-1"><Compass className="size-4 text-brand" />מרכזי ייעוץ · Command Hub</span>
                <button onClick={onClose} aria-label="סגור" className="rounded-lg p-1 text-ink-3 hover:bg-surface-2"><X className="size-5" /></button>
              </div>
            </div>
            <div className="space-y-2.5 px-4 pt-1">
              {CENTERS.map((c) => { const Ic = c.Icon; const on = path.startsWith(c.href); return (
                <Link key={c.id} href={c.href} onClick={() => { playClick(); onClose(); }}
                  className="block rounded-2xl border-2 bg-surface p-3.5 transition active:scale-[0.98]" style={{ borderColor: on ? c.accent : "#e2e8f0" }}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="ms-auto text-[9px] font-bold uppercase tracking-wide text-ink-3">{c.updated}</span>
                      </div>
                      <p className="mt-0.5 text-[12px] leading-relaxed text-ink-3">{c.desc}</p>
                      <div className="mt-1.5 flex items-center gap-3">
                        {c.metrics.map((m) => <div key={m.label} className="leading-none"><span className="font-mono text-sm font-extrabold" style={{ color: c.accent }}>{m.value}</span><span className="mr-1 text-[10px] font-bold text-ink-3">{m.label}</span></div>)}
                      </div>
                      <div className="mt-1.5 flex flex-wrap gap-1">{c.inside.slice(0, 4).map((x) => <span key={x} className="rounded-md bg-surface-2 px-1.5 py-0.5 text-[10px] font-bold text-ink-3" dir="ltr">{x}</span>)}</div>
                    </div>
                  </div>
                </Link>
              ); })}
              <div className="flex flex-wrap gap-2 border-t border-hairline px-1 pb-2 pt-3 text-[12px] font-bold text-ink-3">
