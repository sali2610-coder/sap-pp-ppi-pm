...
export function OnboardingDrawer() {
  ...
  const Step = ({ s }: { s: { n: number; title: string; desc: string; href: string } }) => (
    <Link href={s.href} onClick={close} className="lift flex items-center gap-3 rounded-xl border border-hairline bg-surface p-3 shadow-sm transition hover:border-brand/30">
      ...
      <span className="min-w-0 flex-1"><span className="block text-sm font-extrabold text-ink-1">{s.title}</span><span className="block truncate text-[11px] text-ink-3">{s.desc}</span></span>
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.aside dir="rtl" role="dialog" aria-modal="true" aria-label="ברוך הבא ל-NEO" className="fixed inset-y-0 start-0 z-[71] flex w-[460px] max-w-[92vw] flex-col bg-surface shadow-2xl"
            ...
            <div className="relative shrink-0 bg-gradient-to-l from-brand-dark via-brand to-brand px-5 py-4 text-white">
              <button onClick={close} aria-label="סגור" className="absolute end-4 top-4 rounded-lg p-1 text-white/80 hover:bg-surface/15"><X className="size-5" /></button>
            ...
            <div className="shrink-0 border-b border-hairline px-5 py-3">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {ROLE_TASKS[role].map((t) => (
                  <Link key={t.he} href={t.href} onClick={close} className="inline-flex items-center gap-1 rounded-lg border border-hairline bg-surface-2 px-2 py-1 text-[11px] font-bold text-ink-2 transition hover:border-brand/40 hover:text-brand">
                    {t.he}<ArrowLeft className="size-3 text-ink-3" />
                  </Link>
                ))}
              </div>
            </div>

            <div className="flex shrink-0 gap-1 border-b border-hairline px-5 py-2">
            ...
            <div className="min-h-0 flex-1 overflow-auto p-5">
              {tab === "welcome" && (
                <div className="space-y-3">
                  ...
                  <p className="text-sm font-bold text-ink-2">למי זה מיועד: מיישמי PM/PP-PI, יועצי SAP, צוותי QA ומיגרציה.</p>
                  <div className="space-y-2">
                    {GLOSSARY.map((g) => (
                      <div key={g.term} className="rounded-xl border border-hairline bg-surface p-3">
                        <div className="text-sm font-extrabold text-ink-1">{g.term}</div>
                        <div className="text-[12px] leading-relaxed text-ink-2">{g.he}</div>
                      </div>
                    ))}
                  </div>
              ...
              {tab === "pm" && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between"><h3 className="flex items-center gap-1.5 text-sm font-extrabold text-ink-1"><GraduationCap className="size-4 text-brand" />מסלול התחלה · PM</h3>
              ...
              {tab === "pppi" && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between"><h3 className="flex items-center gap-1.5 text-sm font-extrabold text-ink-1"><GraduationCap className="size-4 text-brand" />מסלול התחלה · PP-PI</h3>
              ...
              {tab === "work" && (
                <div className="space-y-2">
                  <h3 className="flex items-center gap-1.5 text-sm font-extrabold text-ink-1"><Compass className="size-4 text-brand" />מה אתה צריך לעשות?</h3>
                  {WORKFLOWS.map((w) => (
                    <Link key={w.he} href={w.href} onClick={close} className="lift flex items-center gap-3 rounded-xl border border-hairline bg-surface p-3 shadow-sm transition hover:border-brand/30">
                      <span className="min-w-0 flex-1"><span className="block text-sm font-extrabold text-ink-1">{w.he}</span><span className="block text-[11px] text-ink-3">{w.sub}</span></span>
                      <ArrowLeft className="size-3.5 shrink-0 text-ink-3" />
                    </Link>
                  ))}
                </div>
            ...
            <div className="shrink-0 border-t border-hairline px-5 py-2.5 text-center">
              <button onClick={close} className="text-xs font-bold text-ink-3 hover:text-brand">דלג — אפשר לפתוח שוב דרך ❓ בכל עמוד</button>
            </div>
          </motion.aside>
        </>
      )}
